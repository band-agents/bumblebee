...
export const ISSUES: Issue[] = [
  ...
  { slug: "signed-out-by-admin", area: "Signing in", symptom: "I was signed out and the sign-in page says an admin suspended or removed my login",
    cause: "An owner or admin suspended or removed your login. This signs you out of every device within seconds.", fix: ["Suspended: ask an owner or admin to choose Re-activate on your profile in Users & Access.", "Removed: the login can't be used again — ask them to create a new one if you still need access."], link: "users-and-access" },
  { slug: "login-suspended", area: "Signing in", symptom: "Sign-in is refused for a suspended or removed login", message: "This login is suspended or was removed. Ask your admin.",
    cause: "The login is suspended or was removed.", fix: ["Ask an owner or admin to Re-activate it, or to create a new login."], link: "users-and-access" },
  ...
  { slug: "missing-menu", area: "Access", symptom: "A module is missing from my sidebar",
    cause: "Your role, or the modules your admin picked for you, don't include it.", fix: ["Ask an admin to open your panel in Users & Access and set that module to View, Edit or Full. It appears within seconds."], link: "users-and-access" },
  { slug: "view-only-save", area: "Access", symptom: "Saving is refused in a module I can open", message: "Your access to this module doesn't allow this change. Ask an admin for Edit or Full access.",
    cause: "You have View access to that module, which can read but not add or change.", fix: ["Ask an admin to set the module to Edit or Full on your panel in Users & Access."], link: "users-and-access" },
  { slug: "sees-too-much", area: "Access", symptom: "A colleague can open modules I didn't want them to",
    cause: "They're using their role's module list, which includes those modules — or they're an admin, who opens everything.", fix: ["Open their panel, choose Pick modules myself, set everything they shouldn't open to Off, and Save access.", "If their role is Admin, change it to a normal role first."], link: "users-and-access" },
  { slug: "no-finance-invoice", area: "Access", symptom: "Create invoice or Record payment isn't there", message: "Your access doesn't include Finance, so you can't issue invoices or receipts",
    cause: "Issuing invoices and receipts needs Edit access to Finance.", fix: ["Ask an admin to give you Finance access, or ask a finance colleague to issue it."], link: "roles" },
  { slug: "cant-remove", area: "Access", symptom: "I can't remove or change a login", message: "The owner can't be removed",
    cause: "Nobody can remove or change the owner, remove themselves or change their own access, and only the owner manages admins.", fix: ["Ask the owner if it's an admin; the owner account itself can't be removed."], link: "users-and-access" },
  { slug: "db-needs-update", area: "Access", symptom: "Creating a login or saving access fails with a database message", message: "The database needs the latest access update. Run 4-access-control.sql in the Supabase SQL editor, then try again.",
    cause: "The app was updated but the matching database update hasn't been run yet (older message: violates check constraint \"workspace_members_role_check\").", fix: ["Open the Supabase SQL editor, paste the contents of 4-access-control.sql and run it.", "Try again — nothing needs to be re-entered except the form."], link: "users-and-access" },
  ...
  { slug: "void-not-allowed", area: "Invoices & payments", symptom: "Void is refused", message: "Voiding needs Finance approval access",
    cause: "Voiding money documents needs approval rights in Finance.", fix: ["Ask a Finance Manager, the General Manager, an admin or the owner to void it."], link: "fixing-mistakes" },
...
export const FAQS: Faq[] = [
  ...
  { group: "Accounts & access", q: "Someone left the company. How do I remove their access?", a: "Users & Access → select them → **Remove login** → confirm. They're signed out of every device and the login can never be used again; everything they created stays. If you might bring them back, choose **Suspend & sign out** instead." },
  { group: "Accounts & access", q: "What's the difference between suspend and remove?", a: "Suspend signs them out everywhere and blocks sign-in until you choose Re-activate — reversible. Remove closes the login for good and frees the username. Neither deletes anything they created." },
  { group: "Accounts & access", q: "How do I give someone only one or two modules?", a: "Open their panel in Users & Access → **Pick modules myself** → set those modules to View or Work and everything else to Off → **Save access**. Only those modules appear for them. See [Users and access](/docs/users-and-access#modules)." },
  { group: "Accounts & access", q: "Does a suspended person get signed out straight away?", a: "Yes. Within a few seconds on every device and tab, and they can't sign back in until re-activated." },
  { group: "Accounts & access", q: "Which ready-made roles are there?", a: "Over 30, grouped by department: General Manager, Viewer/Auditor, Sales Manager, Sales Executive, Store Manager, Cashier, E-commerce Manager, Customer Service, Production Manager, Production & Purchasing Lead, Production Planner, Line Supervisor, Pattern & Cutting Master, Designer, Quality Manager, QC Inspector, Warehouse Manager, Storekeeper (data entry), Receiving Clerk, Purchasing Manager, Buyer, Import Coordinator, Dispatcher, Driver, Finance Manager, Accountant, HR Manager, HR Officer and Data Entry Clerk — plus Owner and Admin. See [Roles](/docs/roles)." },
  { group: "Accounts & access", q: "Can one person have more than one role?", a: "Yes. In **Roles** on the create form or their panel, choose **Add role** and tick as many as they need — for example Storekeeper and Cashier. They get every module from all their roles, at the highest level any of them gives. The first role is shown as their main one." },
  { group: "Accounts & access", q: "Can I change what a ready-made role opens for just one person?", a: "Yes. Pick the closest role, then open their panel → **Pick modules myself** and adjust. The role itself stays the same for everyone else." },
  ...
  { group: "Invoices & payments", q: "Who can void invoices and receipts?", a: "Anyone with approval rights in Finance: the Finance Manager and General Manager roles, admins and the owner. Accountants can issue but not void." },
  ...
  { group: "Data & security", q: "Who can see salaries?", a: "Only people with the **HR & Employees** module (and owners and admins). The database doesn't return employee records to anyone else." },
  { group: "Data & security", q: "Is access only hidden in the menu, or really blocked?", a: "Really blocked. The same module list is checked by the database, so a pasted link, a report or a direct request can't return data from a module someone doesn't have." },
