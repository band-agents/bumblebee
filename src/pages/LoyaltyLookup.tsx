    ...
    <div className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${tm.gradient} border border-border/40 p-6`}>
      ...
      <div className="flex items-start justify-between mb-5">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl flex items-center justify-center text-body-lg font-bold text-white shadow-sm" style={{ backgroundColor: member.avatarColor }}>
          ...
          <div>
            ...
            <p className="text-micro text-muted-foreground font-mono mt-0.5">{member.memberNumber}</p>
          </div>
        </div>
        <div className={`px-3 py-1 rounded-full text-micro font-bold tracking-wide ${tm.pill}`}>
      ...
      {/* Points */}
      <div className="mb-5">
        <p className="text-micro text-muted-foreground mb-1">{ar ? "الرصيد الحالي" : "Current Balance"}</p>
        <p className="text-display font-medium text-foreground leading-none tabular-nums" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.03em" }}>
          {fmtPts(member.currentPoints)}
        </p>
        <p className="text-caption text-muted-foreground mt-1">{ar ? "نقطة" : "points"}</p>
      </div>

      {/* Tier progress */}
      {prog.next && (
        <div className="mb-5">
          <div className="flex items-center justify-between text-micro text-muted-foreground mb-1.5">
      ...
      <div className="grid grid-cols-3 gap-3 pt-4 border-t border-border/30">
        <div>
          <p className="text-micro text-muted-foreground">{ar ? "إجمالي الإنفاق" : "Total Spend"}</p>
          <p className="text-body-lg font-medium text-foreground tabular-nums">{fmtCurrency(member.totalSpend)}</p>
        </div>
        <div>
          <p className="text-micro text-muted-foreground">{ar ? "الطلبات" : "Orders"}</p>
          <p className="text-body-lg font-medium text-foreground tabular-nums">{member.orderCount}</p>
        </div>
        <div>
          <p className="text-micro text-muted-foreground">{ar ? "آخر شراء" : "Last Purchase"}</p>
          <p className="text-body-lg font-medium text-foreground">{timeAgo(member.lastPurchaseAt, ar)}</p>
        </div>
      </div>
      ...
      <div className="relative bg-background border border-border/60 rounded-2xl shadow-xl w-full max-w-[420px] overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5 border-b border-border/40">
          <h2 className="text-title font-medium text-foreground" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.02em" }}>
            <Gift size={16} className="inline -mt-0.5 me-2 text-brand-ink" />
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="px-6 py-5 space-y-4">
            <div className="bg-muted/30 rounded-xl px-4 py-3 flex items-center justify-between">
              <span className="text-caption text-muted-foreground">{ar ? "الرصيد المتاح" : "Available"}</span>
              <span className="text-body-lg font-medium text-foreground tabular-nums">{fmtPts(member.currentPoints)} {ar ? "نقطة" : "pts"}</span>
            </div>
            ...
            {numPoints >= 500 && !tooHigh && (
              <div className="bg-primary/5 border border-primary/15 rounded-xl px-4 py-3">
                <p className="text-body font-medium text-brand-ink">{fmtCurrency(discount)} {ar ? "خصم" : "discount"}</p>
                <p className="text-micro text-brand-ink/70 mt-0.5">
          ...
          <div className="px-6 py-4 border-t border-border/40 flex items-center justify-end gap-3">
            ...
            <button type="submit" disabled={!valid || confirmed}
              className="h-9 px-5 rounded-xl bg-primary text-primary-foreground text-body font-medium flex items-center gap-2 hover:opacity-90 disabled:opacity-50 transition-opacity">
              {confirmed
      ...
      <div className="relative bg-background border border-border/60 rounded-2xl shadow-xl w-full max-w-[420px] overflow-hidden">
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="px-6 py-5 space-y-4">
            <div>
              ...
              <input ref={inputRef} type="number" value={amount} onChange={e => setAmount(e.target.value)}
                ...
                className="w-full h-11 px-4 rounded-xl border border-border/80 bg-card text-body-lg text-foreground tabular-nums placeholder:text-muted-foreground focus:outline-none focus:border-primary/40 transition-colors"
              />
            </div>
            ...
            {numAmount >= 10 && (
              <div className="bg-emerald-500/5 border border-emerald-500/15 rounded-xl px-4 py-3">
                <p className="text-body font-medium text-emerald-700">+{fmtPts(finalPoints)} {ar ? "نقطة" : "points"}</p>
                <p className="text-micro text-emerald-600/70 mt-0.5">
          ...
          <div className="px-6 py-4 border-t border-border/40 flex items-center justify-end gap-3">
            ...
            <button type="submit" disabled={!valid || confirmed}
              className="h-9 px-5 rounded-xl bg-emerald-600 text-white text-body font-medium flex items-center gap-2 hover:opacity-90 disabled:opacity-50 transition-opacity">
              {confirmed
                ? <><CheckCircle2 size={14} />{ar ? "تم" : "Done"}</>
...
function MemberTransactions({ transactions, lang }: { transactions: LoyaltyTransactionDemo[]; lang: "en" | "ar" }) {
  ...
  if (transactions.length === 0) return (
    <p className="text-caption text-muted-foreground/50 text-center py-6">{ar ? "لا توجد معاملات" : "No transactions yet"}</p>
  );
      ...
      {transactions.map(tx => {
        ...
        return (
          <div key={tx.id} className="flex items-center gap-3 px-1 py-3">
            ...
            <span className={`text-body font-medium tabular-nums ${isPositive ? "text-emerald-600" : "text-foreground"}`}>
      ...
      <>
        <div className="min-h-full py-6 px-6 md:px-10 max-w-[600px] mx-auto">
          ...
          <button onClick={() => setSelectedMember(null)}
            className="flex items-center gap-1.5 text-caption text-muted-foreground hover:text-foreground mb-5 transition-colors">
          ...
          <div className="grid grid-cols-2 gap-3 mt-5">
            <button onClick={() => setRedeemOpen(true)}
              className="h-12 rounded-xl bg-primary text-primary-foreground text-body-lg font-medium flex items-center justify-center gap-2.5 hover:opacity-90 active:opacity-80 transition-opacity shadow-sm">
              <Gift size={16} strokeWidth={2} />
              {ar ? "استبدال النقاط" : "Redeem Points"}
            </button>
            <button onClick={() => setPurchaseOpen(true)}
              className="h-12 rounded-xl bg-emerald-600 text-white text-body-lg font-medium flex items-center justify-center gap-2.5 hover:opacity-90 active:opacity-80 transition-opacity shadow-sm">
          ...
          {selectedMember.favoriteCategories.length > 0 && (
            <div className="mt-5">
              <p className="text-micro text-muted-foreground mb-2">{ar ? "الفئات المفضلة" : "Favorite Categories"}</p>
              <div className="flex flex-wrap gap-1.5">
                {selectedMember.favoriteCategories.map(c => (
                  <span key={c} className="text-micro px-2.5 py-1 rounded-full bg-muted text-muted-foreground border border-border/40">{c}</span>
                ))}
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-3 mt-5">
            <div className="bg-background border border-border/40 rounded-xl px-3 py-3 text-center">
              <p className="text-micro text-muted-foreground">{ar ? "مكتسبة" : "Lifetime"}</p>
              <p className="text-body-lg font-medium text-foreground tabular-nums mt-1">{fmtPts(selectedMember.lifetimePoints)}</p>
            </div>
            <div className="bg-background border border-border/40 rounded-xl px-3 py-3 text-center">
              <p className="text-micro text-muted-foreground">{ar ? "مستبدلة" : "Redeemed"}</p>
              <p className="text-body-lg font-medium text-foreground tabular-nums mt-1">{fmtPts(selectedMember.redeemedPoints)}</p>
            </div>
            <div className="bg-background border border-border/40 rounded-xl px-3 py-3 text-center">
              <p className="text-micro text-muted-foreground">{ar ? "منتهية" : "Expired"}</p>
              <p className="text-body-lg font-medium text-foreground tabular-nums mt-1">{fmtPts(selectedMember.expiredPoints)}</p>
            </div>
          </div>
          ...
          <div className="mt-6">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate(`/loyalty/members/${selectedMember.id}`)} className="text-micro text-brand-ink hover:underline flex items-center gap-1">
    ...
    <div className="min-h-full flex flex-col items-center py-12 px-6">
      ...
      <div className="text-center mb-8">
        <div className="w-14 h-14 rounded-2xl bg-primary/8 flex items-center justify-center mx-auto mb-4">
          <User size={24} strokeWidth={1.5} className="text-brand-ink" />
        </div>
        <h1 className="text-display font-medium text-foreground" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.025em" }}>
      ...
      <div className="w-full max-w-[480px] mb-6">
        <div className="relative">
          ...
          <input
            ...
            className="w-full h-14 ps-12 pe-12 rounded-2xl border-2 border-border/80 bg-card text-title text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:border-primary/50 transition-colors tabular-nums"
            autoComplete="off"
          />
          {search && (
      ...
      <div className="w-full max-w-[480px]">
        {search.trim().length >= 2 && results.length === 0 && (
          <div className="text-center py-10">
            <p className="text-body text-muted-foreground">{ar ? "لم يتم العثور على نتائج" : "No members found"}</p>
            <p className="text-micro text-muted-foreground/50 mt-1">{ar ? "تحقق من رقم الهاتف أو البريد الإلكتروني" : "Check the phone number or email"}</p>
          </div>
        )}
            ...
            {results.map(m => {
              ...
              return (
                <button key={m.id} onClick={() => setSelectedMember(m)}
                  ...
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-caption font-bold text-white shrink-0" style={{ backgroundColor: m.avatarColor }}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className={`text-micro font-semibold px-2 py-0.5 rounded-full shrink-0 ${tm.pill}`}>
                        {ar ? tm.ar : tm.en}
                      </span>
                    </div>
                    <p className="text-micro text-muted-foreground">{m.phone} · {m.email}</p>
                  </div>
                  <div className="text-end shrink-0">
                    <p className="text-body-lg font-medium text-foreground tabular-nums">{fmtPts(m.currentPoints)}</p>
                    <p className="text-micro text-muted-foreground">{ar ? "نقطة" : "pts"}</p>
                  </div>
                  <ChevronRight size={16} strokeWidth={1.75} className="text-muted-foreground/30 group-hover:text-brand-ink/50 transition-colors shrink-0" />
                </button>
              );
            })}
          </div>
        ...
        {search.trim().length < 2 && (
          <div className="text-center pt-4">
            <p className="text-caption text-muted-foreground/40">{ar ? "اكتب رقم الهاتف للبحث" : "Type a phone number to search"}</p>
          </div>
        )}
      </div>
    </div>
