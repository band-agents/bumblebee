    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl flex items-center justify-center text-display"
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <span className={`text-micro font-semibold px-2 py-0.5 rounded-full ${sm.color} ${sm.bg}`}>
                {ar ? sm.ar : sm.en}
              </span>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="text-center">
          <p className="text-title font-medium text-foreground tabular-nums">{formatNumber(app.total_downloads)}</p>
          <p className="text-micro text-muted-foreground">{ar ? "تنزيلات" : "Downloads"}</p>
        </div>
        <div className="text-center">
          <p className="text-title font-medium text-foreground tabular-nums">{formatNumber(app.active_users)}</p>
          <p className="text-micro text-muted-foreground">{ar ? "مستخدمون نشطون" : "Active Users"}</p>
        </div>
        <div className="text-center">
          <p className="text-title font-medium text-foreground tabular-nums">{app.avg_session_duration}m</p>
          <p className="text-micro text-muted-foreground">{ar ? "مدة الجلسة" : "Session"}</p>
        </div>
      </div>

      <div className="flex items-center justify-between mb-4 pb-4 border-b border-border/25">
        <div className="flex items-center gap-2">
          <span className={`text-micro font-semibold px-2 py-0.5 rounded-full ${bsm.color} ${bsm.bg}`}>
            {ar ? bsm.ar : bsm.en}
          </span>
          {app.last_build_date && (
            <span className="text-micro text-muted-foreground">{formatDate(app.last_build_date, ar)}</span>
          )}
        </div>
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => onNavigate("/mobile-apps/analytics")} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-primary/10 text-brand-ink text-micro font-medium hover:bg-primary/20 transition-colors">
          <BarChart3 size={12} />
          {ar ? "التحليلات" : "Analytics"}
        </button>
        <button onClick={() => onNavigate("/mobile-apps/notifications")} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-muted/40 text-foreground text-micro font-medium hover:bg-muted/60 transition-colors">
          <Send size={12} />
          {ar ? "إشعار" : "Notify"}
        </button>
        <button onClick={() => onNavigate(`/mobile-apps/config/${app.id}`)} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-muted/40 text-foreground text-micro font-medium hover:bg-muted/60 transition-colors">
          <Settings size={12} />
...
function NotificationRow({ notif, ar, index, onNavigate }: { notif: PushNotification; ar: boolean; index: number; onNavigate: (path: string) => void }) {
  ...
  return (
    ...
    >
      <span className="text-title shrink-0">{tm.icon}</span>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <p className="text-body font-medium text-foreground truncate">{ar ? notif.title_ar : notif.title}</p>
          <span className={`text-micro font-semibold px-2 py-0.5 rounded-full shrink-0 ${tm.color} ${tm.bg}`}>
            {ar ? tm.ar : tm.en}
          </span>
        </div>
        <p className="text-micro text-muted-foreground truncate">{ar ? notif.message_ar : notif.message}</p>
      </div>
      <div className="text-end shrink-0">
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          <span className="text-micro text-muted-foreground">{formatTime(notif.sent_at || notif.scheduled_at, ar)}</span>
        </div>
      </div>
    </motion.div>
  );
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-muted/40">
            <span className="text-body-lg">{feature.icon}</span>
          </div>
          <div>
            <div className="flex items-center gap-1.5">
              <p className="text-body font-medium text-foreground">{ar ? feature.name_ar : feature.name}</p>
              {feature.premium && (
                <Crown size={10} className="text-warning" />
              )}
            </div>
            <p className="text-micro text-muted-foreground">{ar ? feature.description_ar : feature.description}</p>
          </div>
        </div>
...
function BuildRow({ build, ar, index }: { build: AppBuild; ar: boolean; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <p className="text-body font-medium text-foreground font-mono">v{build.version}</p>
          <span className={`text-micro font-semibold px-2 py-0.5 rounded-full ${bsm.color} ${bsm.bg}`}>
            {ar ? bsm.ar : bsm.en}
          </span>
        </div>
      ...
      <div className="text-end shrink-0">
        <p className="text-micro text-muted-foreground">{formatDate(build.completed_at || build.started_at, ar)}</p>
        <p className="text-micro text-muted-foreground/60">{build.triggered_by}</p>
      </div>
    </motion.div>
  );
}
...
function QuickActionBtn({ icon: Icon, label, labelAr, color, onClick }: {
  ...
  return (
    ...
    >
      ...
      <span className="text-micro font-medium text-foreground">{labelAr}</span>
    </motion.button>
  );
}
    ...
    <div className="min-h-full px-6 md:px-8 py-6 max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <motion.button
          ...
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary text-primary-foreground text-body font-medium hover:opacity-90 transition-opacity"
        >
          <Plus size={15} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-background border border-border/40 rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-border/30 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Bell size={14} strokeWidth={1.75} className="text-brand-ink" />
            ...
            <motion.button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary/10 text-brand-ink text-micro font-medium hover:bg-primary/20 transition-colors"
            >
              <Send size={11} />
        ...
        <div className="bg-background border border-border/40 rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-border/30">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Zap size={14} strokeWidth={1.75} className="text-warning" />
                <h3 className="text-body font-medium text-foreground" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.01em" }}>
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto">
              {featureCategories.map(cat => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-micro font-medium whitespace-nowrap transition-colors ${
                    featureFilter === cat
      ...
      {/* ─── Quick Actions ──────────────────────────────── */}
      <div>
        <h3 className="text-body font-medium text-foreground mb-3" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.01em" }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          <QuickActionBtn
            ...
            color="bg-primary/10 text-brand-ink"
            onClick={() => navigate("/mobile-apps/notifications")}
          />
          <QuickActionBtn
            icon={Link2}
