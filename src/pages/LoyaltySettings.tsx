  ...
  return (
    <div className="flex items-center justify-between gap-4 py-3.5 border-b border-border/25 last:border-0">
      ...
      <input type={type} defaultValue={value} readOnly
        className="w-[140px] h-8 px-3 rounded-lg border border-border/80 bg-card text-body text-foreground text-end tabular-nums focus:outline-none focus:border-primary/40 transition-colors"
      />
    </div>
  );
}
      ...
      <div className="flex items-center gap-1 border-b border-border/40 mb-6">
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex items-center gap-2 px-4 py-3 text-body font-medium border-b-2 transition-colors ${tab === t.id ? "text-brand-ink border-primary" : "text-muted-foreground border-transparent hover:text-foreground"}`}>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-background border border-border/40 rounded-xl px-5">
        <h3 className="text-body-lg font-medium text-foreground py-4 border-b border-border/30" style={{ fontFamily: "var(--app-font-serif)" }}>
            ...
            <div className="px-5 py-4 flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-micro font-medium px-2 py-0.5 rounded-full ${tm.pill}`}>
                    {tm.multiplier}x
                  </span>
                </div>
              ...
              <div className="text-end">
                <p className="text-micro text-muted-foreground">{ar ? "مضاعف الاكتساب" : "Earning multiplier"}</p>
                <p className="text-title font-medium text-foreground" style={{ fontFamily: "var(--app-font-serif)" }}>{tm.multiplier}x</p>
              </div>
            </div>
          </div>
        );
        ...
        {rules.map(rule => {
          ...
          return (
            <div key={rule.id} className="flex items-center gap-4 px-5 py-4">
              <div className={`w-2 h-2 rounded-full shrink-0 ${rule.active ? "bg-emerald-500" : "bg-warning"}`} />
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <span className={`text-micro font-medium px-2 py-0.5 rounded-full ${tl.pill}`}>
                    {ar ? tl.ar : tl.en}
                  </span>
                  <h4 className="text-body font-medium text-foreground truncate">{ar ? rule.nameAr : rule.nameEn}</h4>
                </div>
                <p className="text-micro text-muted-foreground">{ar ? rule.descAr : rule.descEn}</p>
              </div>
              <span className={`text-micro font-medium ${rule.active ? "text-emerald-600" : "text-warning"}`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-end">
        <a href="/shopify/integration" className="text-micro text-brand-ink hover:underline flex items-center gap-1">
      ...
      <div className={`border rounded-xl p-6 ${connected ? "border-emerald-200 bg-emerald-50/30" : "border-border/40 bg-background"}`}>
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h3 className="text-title font-medium text-foreground" style={{ fontFamily: "var(--app-font-serif)" }}>
              {connected
        ...
        <div className="bg-background border border-border/40 rounded-xl px-5">
          ...
          <div className="py-4 space-y-4">
            ...
            <div className="flex items-center gap-3 pt-2">
              <button className="h-9 px-5 rounded-xl bg-primary text-primary-foreground text-body font-medium flex items-center gap-2 hover:opacity-90 transition-opacity">
              ...
              <a href="https://shopify.dev/docs/apps/build/authentication/access-tokens" target="_blank" rel="noopener noreferrer"
                className="text-micro text-brand-ink hover:underline flex items-center gap-1">
      ...
      <div className="bg-muted/30 border border-border/30 rounded-xl px-5 py-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-caption font-medium text-foreground">{ar ? "ملاحظة أمنية" : "Security Note"}</p>
          <p className="text-micro text-muted-foreground mt-0.5">
            {ar
