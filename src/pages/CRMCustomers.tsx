    ...
    <div className="min-h-full px-6 md:px-8 py-6 max-w-[1400px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-caption text-muted-foreground mt-0.5">{filtered.length} {ar ? "عميل" : "customers"}</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="relative">
            <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground/40" />
            <input value={searchQ} onChange={e => setSearchQ(e.target.value)}
              className="w-64 h-9 pl-9 pr-3 rounded-lg border border-border bg-background text-caption placeholder:text-muted-foreground/40 focus:outline-none focus:ring-1 focus:ring-brand-ink/30"
              placeholder={ar ? "بحث بالاسم، الهاتف، المدينة..." : "Search name, phone, city..."} />
          </div>
          <button onClick={() => setFilterOpen(!filterOpen)} className={`h-9 px-3 rounded-lg border text-micro font-medium flex items-center gap-1.5 transition-colors ${filterOpen ? "border-primary bg-primary/5 text-brand-ink" : "border-border text-muted-foreground hover:bg-muted"}`}>
      ...
      {filterOpen && (
        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} className="mb-4 p-4 rounded-xl border border-border/40 bg-muted/20 overflow-hidden">
          <div className="flex items-center gap-3 flex-wrap">
            <select value={vipFilter} onChange={e => setVipFilter(e.target.value)} className="h-8 px-2 rounded-lg border border-border bg-background text-micro appearance-none cursor-pointer">
            ...
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="h-8 px-2 rounded-lg border border-border bg-background text-micro appearance-none cursor-pointer">
            ...
            <select value={riskFilter} onChange={e => setRiskFilter(e.target.value)} className="h-8 px-2 rounded-lg border border-border bg-background text-micro appearance-none cursor-pointer">
            ...
            <select value={sortBy} onChange={e => setSortBy(e.target.value as typeof sortBy)} className="h-8 px-2 rounded-lg border border-border bg-background text-micro appearance-none cursor-pointer">
          ...
          {filtered.map((c, i) => (
            <motion.div key={c.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
              ...
              <div className="flex items-start gap-3 mb-3">
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-micro font-bold text-white shrink-0" style={{ backgroundColor: c.avatar_color }}>
                  {c.name.split(" ").map(w => w[0]).join("")}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-body font-medium text-foreground truncate">{c.name}</p>
                  <p className="text-micro text-muted-foreground truncate">{c.phone}</p>
                </div>
                {c.vip_level !== "none" && <span className={`text-micro px-1.5 py-0.5 rounded-full font-semibold uppercase ${VIP_COLORS[c.vip_level]}`}>{c.vip_level}</span>}
              </div>
              <div className="grid grid-cols-3 gap-2 mb-3">
                <div className="text-center">
                  <p className="text-caption font-semibold text-foreground">{formatEGP(c.total_spend)}</p>
                  <p className="text-micro text-muted-foreground">{ar ? "الإجمالي" : "Spend"}</p>
                </div>
                <div className="text-center">
                  <p className="text-caption font-semibold text-foreground">{c.total_orders}</p>
                  <p className="text-micro text-muted-foreground">{ar ? "طلبات" : "Orders"}</p>
                </div>
                <div className="text-center">
                  <p className="text-caption font-semibold text-brand-ink">{c.loyalty_points.toLocaleString()}</p>
                  <p className="text-micro text-muted-foreground">{ar ? "نقاط" : "Points"}</p>
                </div>
              </div>
              <div className="flex items-center gap-1.5 flex-wrap mb-2">
                {c.tags.slice(0, 2).map(tag => (
                  <span key={tag} className="text-micro px-1.5 py-0.5 rounded-full bg-primary/10 text-brand-ink">{tag}</span>
                ))}
                {c.churn_risk === "high" && <span className="text-micro px-1.5 py-0.5 rounded-full bg-rose-100 text-rose-600">{ar ? "خطر" : "Risk"}</span>}
              </div>
              <div className="flex items-center justify-between pt-2 border-t border-border/30">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-micro text-muted-foreground">{c.last_activity}</span>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        <div className="space-y-1.5">
          {filtered.map((c, i) => (
            <motion.div key={c.id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }}
              ...
              <div className="w-9 h-9 rounded-full flex items-center justify-center text-micro font-bold text-white shrink-0" style={{ backgroundColor: c.avatar_color }}>
                {c.name.split(" ").map(w => w[0]).join("")}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {c.vip_level !== "none" && <span className={`text-micro px-1.5 py-0.5 rounded-full font-semibold uppercase ${VIP_COLORS[c.vip_level]}`}>{c.vip_level}</span>}
                  <span className={`text-micro px-1.5 py-0.5 rounded-full font-medium ${STATUS_COLORS[c.status]}`}>{c.status}</span>
                </div>
              ...
              <div className="text-right shrink-0 grid grid-cols-3 gap-4">
                <div>
                  <p className="text-caption font-semibold">{formatEGP(c.total_spend)}</p>
                  <p className="text-micro text-muted-foreground">{ar ? "الإجمالي" : "Spend"}</p>
                </div>
                <div>
                  <p className="text-caption font-semibold">{c.total_orders}</p>
                  <p className="text-micro text-muted-foreground">{ar ? "طلبات" : "Orders"}</p>
                </div>
                <div>
                  <p className="text-caption font-semibold text-brand-ink">{c.loyalty_points.toLocaleString()}</p>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-body text-muted-foreground mb-1">{ar ? "لا يوجد عملاء" : "No customers found"}</p>
          <p className="text-micro text-muted-foreground/60 mb-4">{searchQ ? (ar ? "جرب البحث بكلمات مختلفة" : "Try different search terms") : (ar ? "ابدأ بإضافة عميل جديد" : "Start by adding a new customer")}</p>
          {!searchQ && (
            <button className="h-8 px-4 rounded-lg bg-primary text-primary-foreground text-micro font-medium hover:opacity-90 transition-opacity flex items-center gap-1.5 mx-auto">
