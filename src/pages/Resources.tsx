    ...
    <div onClick={onClick} className="bg-background border border-border/40 rounded-xl p-5 flex flex-col gap-3.5 hover:shadow-sm hover:border-border/70 transition-all cursor-pointer group">
      <div className="flex items-start justify-between gap-3">
        <div className="w-10 h-10 rounded-xl bg-primary/8 flex items-center justify-center shrink-0">
          <Icon size={16} strokeWidth={1.75} className="text-brand-ink" />
        </div>
        <span className={`text-micro font-medium px-2 py-0.5 rounded-full ${tm.pill}`}>{ar ? tm.ar : tm.en}</span>
      </div>

      <div className="min-w-0">
        <h3 className="text-body-lg font-medium text-foreground leading-snug group-hover:text-brand-ink transition-colors line-clamp-2" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.01em" }}>
      ...
      {/* Inventory extras */}
      {sku && (
        <p className="text-micro text-muted-foreground font-mono">{sku} · {qty ?? 0} {ar ? "وحدة" : "units"}</p>
      )}
...
function ResourceListView({ items, lang, onNavigate }: { items: ResRow[]; lang: "en" | "ar"; onNavigate: (id: string) => void }) {
  ...
  return (
    <div className="border border-border/40 rounded-xl overflow-hidden bg-background divide-y divide-border/25">
        ...
        return (
          <div key={res.id} onClick={() => onNavigate(res.id)} className="flex items-center gap-4 px-5 py-3.5 hover:bg-muted/20 transition-colors cursor-pointer group">
            <div className="w-9 h-9 rounded-xl bg-primary/8 flex items-center justify-center shrink-0">
              <Icon size={15} strokeWidth={1.75} className="text-brand-ink" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2.5 mb-0.5">
                <span className={`text-micro font-medium px-2 py-0.5 rounded-full ${tm.pill}`}>{ar ? tm.ar : tm.en}</span>
                <h4 className="text-body font-medium text-foreground truncate group-hover:text-brand-ink transition-colors">{ar ? (res.name_ar || res.name_en) : res.name_en}</h4>
              </div>
      ...
      {items.length === 0 && <div className="px-6 py-14 text-center"><p className="text-body text-muted-foreground/60">{ar ? "لا توجد موارد" : "No resources found"}</p></div>}
    </div>
  );
}
    ...
    <div className="min-h-full">
      ...
      <div className="border-b border-border/40 px-8 md:px-10 py-8" style={{ background: "linear-gradient(160deg, hsl(var(--muted)/0.3) 0%, hsl(var(--background)) 60%)" }}>
        <div className="max-w-[1100px]">
          <p className="text-micro text-muted-foreground/60 tracking-[0.08em] uppercase mb-2">{ar ? "الموارد" : "Resources"}</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {[
              ...
              <div key={i} className="bg-background border border-border/40 rounded-xl px-4 py-3.5">
                <div className="flex items-center gap-2 mb-2"><m.icon size={14} strokeWidth={1.75} className={m.color} /><p className="text-micro text-muted-foreground">{m.label}</p></div>
      ...
      <div className="px-8 md:px-10 py-6 max-w-[1100px]">
        <div className="flex flex-wrap items-center gap-2.5 mb-5">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {TYPE_FILTERS.map((f) => (
              <button key={f.value} onClick={() => setTypeFilter(f.value as ResourceType | "all")}
                className={`h-7 px-3 rounded-lg text-caption font-medium border transition-all ${typeFilter === f.value ? "bg-primary/8 text-brand-ink border-primary/25" : "bg-card border-border text-muted-foreground hover:text-foreground"}`}>
                {ar ? f.ar : f.en}
              </button>
            ))}
          </div>
          ...
          {hasFilters && <button onClick={() => { setSearch(""); setTypeFilter("all"); setStatusFilter("all"); }} className="flex items-center gap-1 h-7 px-2.5 rounded-lg text-caption text-muted-foreground hover:text-foreground hover:bg-accent border border-transparent hover:border-border transition-all"><X size={11} strokeWidth={2} />{ar ? "مسح" : "Clear"}</button>}
          <div className="flex-1" />
          <div className="flex items-center border border-border rounded-xl overflow-hidden bg-card">
            <button onClick={() => setView("card")} className={`w-8 h-8 flex items-center justify-center transition-colors ${view === "card" ? "bg-primary/10 text-brand-ink" : "text-muted-foreground hover:text-foreground"}`}><LayoutGrid size={14} strokeWidth={1.75} /></button>
            <div className="w-px h-4 bg-border/60" />
            <button onClick={() => setView("list")} className={`w-8 h-8 flex items-center justify-center transition-colors ${view === "list" ? "bg-primary/10 text-brand-ink" : "text-muted-foreground hover:text-foreground"}`}><List size={14} strokeWidth={1.75} /></button>
          </div>
        </div>

        <p className="text-caption text-muted-foreground mb-4">{ar ? `${filtered.length} مورد` : `${filtered.length} resource${filtered.length !== 1 ? "s" : ""}`}</p>
