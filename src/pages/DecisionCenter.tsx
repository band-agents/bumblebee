...
function SectionTitle({ children, sub }: { children: React.ReactNode; sub?: React.ReactNode }) {
  return (
    <div className="mb-5">
      <h2 className="text-title font-medium text-foreground leading-tight" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.02em" }}>
        {children}
      </h2>
...
function Pill({ children, color }: { children: React.ReactNode; color: string }) {
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-micro font-medium ${color}`}>
      {children}
    </span>
  );
}
  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="text-body-lg font-semibold text-foreground tabular-nums" style={{ fontFamily: "var(--app-font-serif)" }}>{score}</span>
      </div>
    </div>
  );
}
...
function EmptyState({ icon: Icon, titleEn, titleAr, ar }: { icon: React.ElementType; titleEn: string; titleAr: string; ar: boolean }) {
  return (
    <div className="flex flex-col items-center justify-center py-10 text-center">
      ...
      <p className="text-body text-muted-foreground">{ar ? titleAr : titleEn}</p>
    </div>
  );
}
...
function DecisionsTab({ ar, navigate }: { ar: boolean; navigate: (p: string) => void }) {
  ...
  const severityDot: Record<string, string> = {
    high: "bg-rose-500", medium: "bg-warning", low: "bg-stone-400",
  };
    ...
    <div className="space-y-8">
          ...
          <div className="border border-border/40 rounded-xl overflow-hidden divide-y divide-border/25">
            {data.blockers.map((b) => (
              <div key={b.id} className="flex items-start gap-3.5 px-5 py-3.5 hover:bg-muted/10 transition-colors">
                <div className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${b.severity === "high" ? "bg-rose-500" : "bg-warning"}`} />
                <div className="flex-1 min-w-0">
                  <p className="text-body font-medium text-foreground">{ar ? b.titleAr : b.titleEn}</p>
                  <p className="text-micro text-muted-foreground/70 mt-0.5 leading-relaxed">{ar ? b.descAr : b.descEn}</p>
                </div>
                <Pill color={b.severity === "high" ? "bg-rose-50 text-rose-600 border border-rose-200" : "bg-warning/10 text-warning border border-warning/30"}>
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-body font-medium text-foreground">{ar ? "المشاريع المتوقفة" : "Stalled Projects"}</h3>
          {data.stalled.length > 0 && (
            <span className="text-micro bg-warning/10 text-warning border border-warning/30 px-2 py-0.5 rounded-full font-medium">{data.stalled.length}</span>
          )}
        </div>
        {data.stalled.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {data.stalled.map((s) => (
              <div key={s.id} className="border border-warning/30 bg-warning/10 rounded-xl px-4 py-3.5">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <p className="text-body font-medium text-foreground leading-snug">{ar ? s.titleAr : s.titleEn}</p>
                  <Pill color="bg-warning/15 text-warning border border-warning/30">{ar ? s.statusAr : s.statusEn}</Pill>
                </div>
                {s.progress !== undefined && (
                  <div className="flex items-center gap-2 mb-2">
                    <ScoreBar score={s.progress} color="bg-warning" />
                    <span className="text-micro text-muted-foreground shrink-0">{s.progress}%</span>
                  </div>
                )}
                <p className="text-micro text-muted-foreground/70">{ar ? s.severityReasonAr : s.severityReason}</p>
              </div>
            ))}
          </div>
        )}
            ...
            {data.unresolved.map((u) => (
              <div key={u.id} className="flex items-start gap-3.5 px-5 py-3.5 hover:bg-muted/10 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-micro text-muted-foreground mt-0.5">{ar ? u.waitingOnAr : u.waitingOnEn}</p>
                  <p className="text-micro text-muted-foreground/50 mt-0.5">{ar ? u.ageAr : u.ageEn}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        clusters.map((c) => (
          <div key={c.id} className="border border-border/40 rounded-xl p-5 hover:shadow-sm transition-shadow bg-background">
            <div className="flex items-start justify-between gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-micro text-muted-foreground/60 tracking-wide uppercase mb-0.5">{ar ? c.titleAr : c.titleEn}</p>
                <p className="text-body font-medium text-foreground leading-snug">{ar ? c.descAr : c.descEn}</p>
              </div>
            ...
            {c.items.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-3">
                {c.items.map((item, i) => (
                  <span key={i} className="inline-flex items-center gap-1 text-micro bg-muted/50 border border-border/40 rounded-full px-2.5 py-1 text-muted-foreground">
            ...
            <div className="flex items-center gap-1.5 text-micro font-medium text-brand-ink cursor-pointer hover:opacity-80 transition-opacity">
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <div className="bg-background border border-border/40 rounded-xl px-4 py-3.5">
          <p className="text-micro text-muted-foreground mb-1">{ar ? "معدل الإنجاز" : "Completion Rate"}</p>
          <p className="text-display font-medium text-foreground tabular-nums leading-none" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.02em" }}>{data.successRate}%</p>
        </div>
        <div className="bg-background border border-border/40 rounded-xl px-4 py-3.5">
          <p className="text-micro text-muted-foreground mb-1">{ar ? "زخم السرعة" : "Velocity Trend"}</p>
          <div className="flex items-center gap-1.5">
            {(() => { const Icon = trendIcon; return <Icon size={18} strokeWidth={1.75} className={trendColor} />; })()}
            <p className="text-body-lg font-medium text-foreground">{ar ? trendLabelAr : trendLabelEn}</p>
          </div>
        </div>
        <div className="bg-background border border-border/40 rounded-xl px-4 py-3.5">
          <p className="text-micro text-muted-foreground mb-1">{ar ? "أعضاء الفريق" : "Team Members"}</p>
          <p className="text-display font-medium text-foreground tabular-nums leading-none" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.02em" }}>{data.assignees.length}</p>
        </div>
      </div>

      {/* Patterns */}
      <div>
        <h3 className="text-body font-medium text-foreground mb-3">{ar ? "الأنماط المكتشفة" : "Detected Patterns"}</h3>
            ...
            return (
              <div key={p.id} className={`border ${patternColor[p.type]} rounded-xl px-5 py-4`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-body font-medium text-foreground">{ar ? p.titleAr : p.titleEn}</p>
                    <p className="text-micro text-muted-foreground/70 mt-1 leading-relaxed">{ar ? p.descAr : p.descEn}</p>
                    <p className="text-micro font-medium text-brand-ink mt-2">{ar ? p.metricAr : p.metric}</p>
                  </div>
                </div>
              </div>
            );
      ...
      {data.assignees.length > 0 && (
        <div>
          <h3 className="text-body font-medium text-foreground mb-3">{ar ? "أداء الفريق" : "Team Performance"}</h3>
          <div className="border border-border/40 rounded-xl overflow-hidden divide-y divide-border/25">
            {data.assignees.slice(0, 6).map((a, i) => (
              <div key={a.name} className="flex items-center gap-4 px-5 py-3 hover:bg-muted/10 transition-colors">
                <span className="text-micro text-muted-foreground/50 w-4 shrink-0 tabular-nums">{i + 1}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-body font-medium text-foreground truncate">{ar ? a.nameAr : a.name}</p>
                  <p className="text-micro text-muted-foreground">{a.assigned} {ar ? "مُعيّن" : "assigned"} · {a.completed} {ar ? "مكتمل" : "done"}</p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <div className="w-20">
                    <ScoreBar score={a.completionRate} color={a.completionRate >= 60 ? "bg-emerald-500" : a.completionRate >= 30 ? "bg-warning" : "bg-rose-500"} />
                  </div>
                  <span className="text-caption font-medium text-foreground tabular-nums w-8">{a.completionRate}%</span>
                </div>
              </div>
            ))}
          </div>
...
function GoalDriftTab({ ar, industry }: { ar: boolean; industry?: string }) {
  ...
  const driftStyle: Record<string, { bg: string; border: string; dot: string; labelEn: string; labelAr: string }> = {
    ...
    drifting:  { bg: "bg-warning/10",   border: "border-warning/30",   dot: "bg-warning",   labelEn: "Drifting",  labelAr: "ينحرف" },
          ...
          <div key={item.id} className={`border ${s.border} ${s.bg} rounded-xl px-5 py-4`}>
            <div className="flex items-start justify-between gap-3 mb-2">
              <div className="flex items-center gap-2">
                ...
                <p className="text-body-lg font-medium text-foreground" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.01em" }}>
                  {ar ? item.goalAr : item.goalEn}
                </p>
              </div>
              <Pill color={`${s.bg} ${s.border} ${item.driftLevel === "on_track" ? "text-emerald-700" : item.driftLevel === "drifting" ? "text-warning" : "text-rose-700"}`}>
                {ar ? s.labelAr : s.labelEn}
              </Pill>
            </div>
            <p className="text-caption text-muted-foreground mb-2">{ar ? item.currentStateAr : item.currentStateEn}</p>
            <p className="text-micro text-muted-foreground/70 leading-relaxed mb-3">{ar ? item.evidenceAr : item.evidenceEn}</p>
            {item.driftLevel !== "on_track" && (
              <div className="flex items-center gap-1.5 text-micro font-medium text-brand-ink">
...
function HealthTab({ ar }: { ar: boolean }) {
  ...
  const levelStyle: Record<string, { bg: string; text: string; labelEn: string; labelAr: string; bar: string }> = {
    ...
    at_risk:  { bg: "bg-warning/10",    text: "text-warning",   labelEn: "At Risk",  labelAr: "في خطر", bar: "bg-warning" },
        ...
        {(["critical", "at_risk", "healthy"] as const).map((level) => {
          ...
          return (
            <div key={level} className={`${s.bg} border border-current/10 rounded-xl px-4 py-3`}>
              <p className={`text-micro font-medium ${s.text} mb-1`}>{ar ? s.labelAr : s.labelEn}</p>
              <p className={`text-display font-medium ${s.text} tabular-nums leading-none`} style={{ fontFamily: "var(--app-font-serif)" }}>{grouped[level].length}</p>
            </div>
          );
        })}
      </div>
            ...
            return (
              <div key={p.id} className="border border-border/40 rounded-xl px-5 py-4 hover:shadow-sm transition-shadow bg-background">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <p className="text-body font-medium text-foreground leading-snug">{ar ? p.titleAr : p.titleEn}</p>
                      <Pill color={`${s.bg} ${s.text} border border-current/20`}>{ar ? s.labelAr : s.labelEn}</Pill>
                    </div>
                    <div className="grid grid-cols-3 gap-3 mb-2.5">
                      <div>
                        <p className="text-micro text-muted-foreground/50 mb-1">{ar ? "الزخم" : "Momentum"}</p>
                        <ScoreBar score={p.momentum} color="bg-blue-500" />
                      </div>
                      <div>
                        <p className="text-micro text-muted-foreground/50 mb-1">{ar ? "النشاط" : "Activity"}</p>
                        <ScoreBar score={p.activity} color="bg-chart-4" />
                      </div>
                      <div>
                        <p className="text-micro text-muted-foreground/50 mb-1">{ar ? "التقدم" : "Progress"}</p>
...
function FocusTab({ ar, navigate }: { ar: boolean; navigate: (p: string) => void }) {
  ...
  const moduleBg: Record<string, string> = {
    finance: "bg-emerald-50", sales: "bg-warning/10", work: "bg-blue-50",
    resources: "bg-chart-4/10", organizations: "bg-cyan-50", people: "bg-rose-50",
  };
  ...
  return (
    <div className="space-y-3">
      <p className="text-caption text-muted-foreground mb-4">{ar ? "أعلى الإجراءات تأثيراً لليوم — مرتبة حسب الرافعة المالية" : "Today's highest-leverage actions — ranked by impact"}</p>
      {items.length === 0 ? (
        ...
        items.map((item, i) => {
          ...
          const iconCl = moduleIconColor[item.module] || "text-brand-ink";
          return (
            <div key={item.id} className="border border-border/40 rounded-xl px-5 py-4 hover:shadow-sm transition-shadow bg-background cursor-pointer group" onClick={() => handleClick(item)}>
              <div className="flex items-start gap-4">
                <div className="flex flex-col items-center gap-2 shrink-0">
                  <span className="text-micro text-muted-foreground/40 tabular-nums font-medium">{String(i + 1).padStart(2, "0")}</span>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 mb-1.5">
                    <p className="text-body font-medium text-foreground group-hover:text-brand-ink transition-colors leading-snug">{ar ? item.titleAr : item.titleEn}</p>
                    <div className="flex items-center gap-2 shrink-0">
                  ...
                  <p className="text-micro text-muted-foreground/70 leading-relaxed mb-2">{ar ? item.whyAr : item.whyEn}</p>
                  <Pill color="bg-primary/8 text-brand-ink border border-primary/20">{ar ? item.leverageAr : item.leverageEn}</Pill>
                </div>
                <ChevronRight size={14} strokeWidth={1.75} className="text-muted-foreground/20 group-hover:text-brand-ink/50 transition-colors shrink-0 mt-1" />
              </div>
            </div>
          );
        })
...
function RelationshipsTab({ ar }: { ar: boolean }) {
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-background border border-border/40 rounded-xl px-4 py-3.5">
          <p className="text-micro text-muted-foreground mb-1">{ar ? "إجمالي العلاقات" : "Total Relationships"}</p>
          <p className="text-display font-medium text-foreground tabular-nums" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.02em" }}>{data.totalRelationships}</p>
        </div>
        <div className="bg-background border border-border/40 rounded-xl px-4 py-3.5">
          <p className="text-micro text-muted-foreground mb-1">{ar ? "كثافة الشبكة" : "Network Density"}</p>
          <p className="text-display font-medium text-foreground tabular-nums" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.02em" }}>{data.networkDensity}%</p>
        </div>
        <div className="bg-background border border-border/40 rounded-xl px-4 py-3.5">
          <p className="text-micro text-muted-foreground mb-1">{ar ? "كيانات معزولة" : "Isolated Entities"}</p>
          <p className="text-display font-medium text-foreground tabular-nums" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.02em" }}>{data.isolatedCount}</p>
        </div>
      </div>
      ...
      {data.hubs.length > 0 && (
        <div>
          <h3 className="text-body font-medium text-foreground mb-3">{ar ? "مراكز الشبكة" : "Network Hubs"}</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {data.hubs.map((hub) => {
              ...
              return (
                <div key={hub.id} className="border border-border/40 rounded-xl px-4 py-3.5 bg-background hover:shadow-sm transition-shadow">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-body font-medium text-foreground truncate">{ar ? hub.nameAr : hub.nameEn}</p>
                      <p className="text-micro text-muted-foreground">{ar ? (typeLabel[hub.type]?.ar || hub.type) : (typeLabel[hub.type]?.en || hub.type)}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-title font-medium text-foreground tabular-nums" style={{ fontFamily: "var(--app-font-serif)" }}>{hub.connectionCount}</p>
                      <p className="text-micro text-muted-foreground/50">{ar ? "اتصال" : "connections"}</p>
                    </div>
                  </div>
      ...
      {/* Bridge insights */}
      <div>
        <h3 className="text-body font-medium text-foreground mb-3">{ar ? "رؤى الشبكة" : "Network Insights"}</h3>
        <div className="border border-border/40 rounded-xl overflow-hidden divide-y divide-border/25">
          {data.keyBridges.map((b, i) => (
            <div key={i} className="flex items-start gap-3.5 px-5 py-3.5">
              <div className="w-7 h-7 rounded-lg bg-primary/8 flex items-center justify-center shrink-0 mt-0.5">
                <Network size={12} strokeWidth={1.75} className="text-brand-ink" />
              </div>
              <div>
                <p className="text-caption font-medium text-foreground">{ar ? b.titleAr : b.titleEn}</p>
                <p className="text-micro text-muted-foreground/70 mt-0.5">{ar ? b.descAr : b.descEn}</p>
              </div>
            </div>
          ))}
        </div>
    ...
    <div className="space-y-3">
      {points.length === 0 ? (
        ...
        points.map((f) => {
          ...
          return (
            <div key={f.id} className={`border ${f.severity === "high" ? "border-rose-200/60" : "border-warning/30"} rounded-xl px-5 py-4`}>
              <div className="flex items-start gap-3.5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <p className="text-body font-medium text-foreground">{ar ? f.titleAr : f.titleEn}</p>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <Pill color={f.severity === "high" ? "bg-rose-50 text-rose-700 border border-rose-200" : "bg-warning/10 text-warning border border-warning/30"}>
                        {ar ? (f.severity === "high" ? "عالي" : "متوسط") : f.severity}
                      </Pill>
                    </div>
                  </div>
                  <p className="text-micro text-muted-foreground/70 leading-relaxed mb-1.5">{ar ? f.descAr : f.descEn}</p>
      ...
      <div className="flex items-center gap-6 p-5 border border-border/40 rounded-xl bg-background">
        <div className="relative shrink-0" style={{ width: 108, height: 108 }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-micro text-muted-foreground/50 mt-1">/100</span>
          </div>
        </div>
        <div>
          <p className="text-micro text-muted-foreground/60 tracking-wide uppercase mb-1">{ar ? "التقرير الأسبوعي" : "Weekly Report"}</p>
          <p className="text-title font-medium text-foreground leading-snug mb-2" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.02em" }}>
        ...
        return (
          <div key={sec.key}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-body font-medium text-foreground">{ar ? sec.titleAr : sec.titleEn}</h3>
              <span className={`text-micro px-2 py-0.5 rounded-full font-medium ${sec.pillCl}`}>{items.length}</span>
            </div>
            <div className="border border-border/40 rounded-xl overflow-hidden divide-y divide-border/25">
              {items.map((item) => (
                <div key={item.id} className="flex items-start gap-3.5 px-5 py-3.5 hover:bg-muted/10 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline justify-between gap-2">
                      <p className="text-body font-medium text-foreground">{ar ? item.titleAr : item.titleEn}</p>
                      {item.value && <span className="text-micro font-medium text-brand-ink shrink-0">{item.value}</span>}
                    </div>
                    <p className="text-micro text-muted-foreground/70 mt-0.5">{ar ? item.descAr : item.descEn}</p>
                  </div>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex items-center gap-5 p-5 border border-border/40 rounded-xl bg-background">
        ...
        <div>
          <p className="text-micro text-muted-foreground/60 tracking-wide uppercase mb-1">{ar ? "توافق الحياة" : "Life Alignment"}</p>
            ...
            <div key={d.id} className="border border-border/40 rounded-xl px-5 py-4 bg-background hover:shadow-sm transition-shadow">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-micro text-muted-foreground/70 mb-2 leading-relaxed">{ar ? d.insightAr : d.insightEn}</p>
                  <div className="mb-2">
                    <ScoreBar score={d.score} color={d.score >= 65 ? "bg-emerald-500" : d.score >= 40 ? "bg-warning" : "bg-rose-500"} />
                  </div>
                  <p className="text-micro text-muted-foreground/50 italic mb-1.5">
                    {ar ? d.becomingGoalAr : d.becomingGoalEn}
                  </p>
                  {d.score < 70 && (
                    <div className="flex items-center gap-1.5 text-micro font-medium text-brand-ink mt-1">
                      <ArrowRight size={11} strokeWidth={2} />
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-1">
        <p className="text-micro text-muted-foreground/60 mb-3">{ar ? "القرارات المطلوبة" : "Decisions Required"} — {templates.length}</p>
            ...
            <div key={t.id}
              ...
              <div className="flex items-start justify-between gap-2 mb-1.5">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className={`text-micro font-medium px-1.5 py-0.5 rounded-full ${catColor[t.category] || "bg-muted text-muted-foreground"}`}>{t.category}</span>
                </div>
              </div>
              <p className="text-caption font-medium text-foreground leading-snug">{ar ? t.titleAr : t.titleEn}</p>
              <p className="text-micro text-muted-foreground/60 mt-1 line-clamp-2">{ar ? t.contextAr : t.contextEn}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="lg:col-span-2">
        {!selectedItem ? (
          <div className="flex flex-col items-center justify-center h-64 border-2 border-dashed border-border/40 rounded-2xl text-center">
            ...
            <p className="text-body text-muted-foreground/60">{ar ? "اختر قراراً من القائمة" : "Select a decision from the list"}</p>
            <p className="text-micro text-muted-foreground/40 mt-1">{ar ? "سيظهر هنا ورقة العمل الكاملة" : "The full decision workspace will appear here"}</p>
          </div>
        ) : (
          <div className="border border-border/40 rounded-2xl overflow-hidden">
            ...
            <div className="px-6 py-4 border-b border-border/30 bg-muted/20">
              <div className="flex items-center gap-2 mb-2">
                <span className={`text-micro font-medium px-1.5 py-0.5 rounded-full ${catColor[selectedItem.category] || "bg-muted text-muted-foreground"}`}>{selectedItem.category}</span>
                <div className={`w-1.5 h-1.5 rounded-full ${urgencyDot[selectedItem.urgency]}`} />
                <span className="text-micro text-muted-foreground/60 capitalize">{selectedItem.urgency} urgency</span>
              </div>
              <h3 className="text-title font-medium text-foreground" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.01em" }}>
                {ar ? selectedItem.titleAr : selectedItem.titleEn}
              </h3>
              <p className="text-caption text-muted-foreground/70 mt-1">{ar ? selectedItem.contextAr : selectedItem.contextEn}</p>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-caption font-semibold text-foreground">{ar ? "التوصيات" : "Recommendations"}</p>
                </div>
                <div className="space-y-2">
                  {(ar ? selectedItem.recommendationsAr : selectedItem.recommendationsEn).map((r, i) => (
                    <div key={i} className="flex items-start gap-2.5 text-caption text-foreground/80">
                      <span className="w-5 h-5 rounded-full bg-warning/10 text-warning border border-warning/30 text-micro font-bold flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
              ...
              <div className="grid grid-cols-2 gap-5">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <Activity size={13} strokeWidth={1.75} className="text-brand-ink" />
                    <p className="text-caption font-semibold text-foreground">{ar ? "التبعيات" : "Dependencies"}</p>
                  </div>
                  <div className="space-y-1">
                    {(ar ? selectedItem.dependenciesAr : selectedItem.dependenciesEn).map((d, i) => (
                      <div key={i} className="flex items-center gap-1.5 text-micro text-muted-foreground/70">
      ...
      <div className="border-b border-border/40 px-8 md:px-10 py-8"
        ...
        <div className="max-w-[1100px]">
          <div className="flex items-center gap-2.5 mb-2">
            <Target size={14} strokeWidth={1.75} className="text-brand-ink" />
            <p className="text-micro text-muted-foreground/60 tracking-[0.08em] uppercase">{ar ? "مركز القرارات" : "Decision Center"}</p>
            <span className="text-micro text-muted-foreground/50 px-2 py-0.5 rounded-full bg-muted border border-border/40">{ar ? "Sprint 21" : "Sprint 21"}</span>
          </div>
            ...
            {TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`
                    flex items-center gap-1.5 px-3.5 py-3.5 text-caption whitespace-nowrap shrink-0
