...
const SEV_STYLE: Record<string, { border: string; icon: string; bg: string }> = {
  ...
  neutral:  { border: "border-border/40",      icon: "text-brand-ink",     bg: "bg-primary/8" },
...
const PRIORITY_STYLE: Record<string, { dot: string; label: string; labelAr: string }> = {
  ...
  medium: { dot: "bg-warning",   label: "Medium", labelAr: "متوسط" },
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className="text-micro text-muted-foreground mt-1">/ 100</span>
      </div>
    </div>
  );
}
    ...
    <div className={`border ${sev.border} rounded-xl bg-background p-5 hover:shadow-sm transition-shadow`}>
      <div className="flex items-start gap-3.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-body font-medium text-foreground leading-snug mb-1.5" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.01em" }}>
...
function SectionHeader({ icon: Icon, titleEn, titleAr, ar, color }: {
  ...
  return (
    <div className="flex items-center gap-2.5 mb-4 mt-10 first:mt-0">
      ...
      <h2 className="text-body-lg font-medium text-foreground" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.01em" }}>
        {ar ? titleAr : titleEn}
      </h2>
    </div>
  );
    ...
    <div className="min-h-full flex items-center justify-center py-24">
      <div className="text-center max-w-[400px]">
        <div className="w-12 h-12 rounded-2xl bg-primary/8 mx-auto flex items-center justify-center mb-4">
          <SparklesEmpty size={22} className="text-brand-ink/50" />
        </div>
        <h2 className="text-title font-medium mb-2" style={{ fontFamily: "var(--app-font-serif)" }}>
  ...
  if (dataLoading) return (
    <div className="min-h-full flex items-center justify-center py-24">
      <div className="flex flex-col items-center gap-3">
        <p className="text-heading font-bold tracking-tight" style={{ fontFamily: "var(--app-font-serif)" }}>Bumblebee</p>
  ...
  return (
    <div className="min-h-full">
      ...
      <div className="border-b border-border/40 px-8 md:px-10 py-8" style={{ background: "linear-gradient(160deg, hsl(var(--muted)/0.3) 0%, hsl(var(--background)) 60%)" }}>
        <div className="max-w-[1100px]">
          <div className="flex items-center gap-2.5 mb-2">
            ...
            <p className="text-micro text-muted-foreground/60 tracking-[0.08em] uppercase">{ar ? "الذكاء" : "Intelligence"}</p>
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start">
            ...
            <div className="flex flex-col items-center gap-3 shrink-0">
              ...
              <div className={`px-3 py-1 rounded-full text-micro font-semibold ${hm.color} ${hm.bg} border border-current/20`}>
                {ar ? hm.ar : hm.en}
              </div>
              <p className="text-micro text-muted-foreground">{ar ? "صحة الأعمال" : "Business Health"}</p>
            </div>
              ...
              {[
                ...
                <div key={i} className="bg-background border border-border/40 rounded-xl px-4 py-3.5">
                  ...
                  <div className="flex items-end gap-2">
                    <p className="text-heading font-medium text-foreground leading-none tabular-nums" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.02em" }}>{s.score}</p>
                    <span className="text-micro text-muted-foreground/50 mb-0.5">/100</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mt-6">
            {[
              ...
              <div key={i} className="bg-background border border-border/40 rounded-xl px-4 py-3">
                ...
                <p className="text-title font-medium text-foreground leading-none tabular-nums" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.02em" }}>{m.value}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="px-8 md:px-10 py-8 max-w-[1100px]">
            ...
            return (
              <div key={action.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-muted/15 transition-colors cursor-pointer group"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-micro text-muted-foreground truncate">{ar ? action.descAr : action.descEn}</p>
                </div>
                <div className="flex items-center gap-1.5 shrink-0">
                  <div className={`w-1.5 h-1.5 rounded-full ${ps.dot}`} />
                  <span className="text-micro text-muted-foreground">{ar ? ps.labelAr : ps.label}</span>
                </div>
        ...
        {/* Sales Intelligence */}
        <SectionHeader icon={ShoppingBag} titleEn="Sales Intelligence" titleAr="ذكاء المبيعات" ar={ar} color="text-warning" />
