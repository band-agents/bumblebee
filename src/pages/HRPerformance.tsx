...
function statusBadge(status: string, ar: boolean) {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-caption font-medium border ${s.cls}`}>
      {ar ? s.ar : s.en}
    </span>
  );
}
...
export default function HRPerformance() {
  ...
  const distribution = useMemo(() => {
    const ranges = [
      ...
      { label: "70-79", min: 70, max: 80, color: "bg-warning" },
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-chart-4/20 to-indigo-600 flex items-center justify-center shadow-lg shadow-chart-4/20">
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10">
                ...
                <select
                  ...
                  className="bg-transparent text-caption text-white/80 outline-none cursor-pointer appearance-none pr-1"
                >
                  {DEPTS.map((d) => (
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6 space-y-6">
          ...
          {[
            ...
            >
              ...
              <div className="flex items-baseline gap-1">
                <span className="text-display font-bold tracking-tight">{kpi.value}</span>
                {kpi.suffix && <span className="text-body text-white/40">{kpi.suffix}</span>}
              </div>
              <p className="text-caption text-white/50 mt-1">{kpi.label}</p>
            </motion.div>
          ))}
        </div>
        ...
        {tab === "goals" && (
          <div className="space-y-3">
            {filteredGoals.length === 0 ? (
              <div className="text-center py-12 text-white/30 text-body">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start gap-3 mb-2">
                          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-body shrink-0" style={{ backgroundColor: (categoryMeta[g.category]?.color ?? "#666") + "20" }}>
                      ...
                      <div className="flex items-center gap-4 lg:gap-6 ml-11 lg:ml-0 shrink-0">
                        <div className="text-center">
                          <p className="text-micro text-white/30 uppercase tracking-wider">{ar ? "الوزن" : "Weight"}</p>
                          <p className="text-body font-semibold text-white/80">{g.weight}%</p>
                        </div>
                        <div className="text-center">
                          <p className="text-micro text-white/30 uppercase tracking-wider">{ar ? "النوع" : "Type"}</p>
                          <p className="text-caption text-white/70 capitalize">{ar ? (g.type === "individual" ? "فردي" : g.type === "team" ? "فريق" : "شركة") : g.type}</p>
                        </div>
                        <div className="text-center">
                          <p className="text-micro text-white/30 uppercase tracking-wider">{ar ? "الموعد" : "Due"}</p>
                          <p className="text-caption text-white/70">{g.due_date}</p>
                        </div>
        ...
        {tab === "reviews" && (
          <div className="grid gap-3 sm:grid-cols-2">
            {filteredReviews.length === 0 ? (
              <div className="text-center py-12 text-white/30 text-body col-span-2">
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-caption text-white/40">{r.period}</p>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-3">
                      {[
                        ...
                        <div key={s.label} className="text-center p-2 rounded-xl bg-white/[0.03]">
                          <p className="text-micro text-white/30 uppercase tracking-wider mb-1">{s.label}</p>
                          <p className={`text-title font-bold ${scoreColor(s.value)}`}>{s.value}</p>
                        </div>
                      ))}
                    </div>
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-3">
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        <div
                          className="w-9 h-9 rounded-full flex items-center justify-center text-caption font-bold text-white shrink-0"
                          style={{ backgroundColor: emp?.avatar_color ?? "#666" }}
                        >
                          {emp ? (ar ? emp.full_name_ar[0] : emp.full_name[0]) : "?"}
                        </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-0.5">
                          {Array.from({ length: 5 }, (_, si) => (
                            <Star
                              key={si}
                              className={`w-3.5 h-3.5 ${si < f.score ? "text-warning fill-warning" : "text-white/10"}`}
                            />
                          ))}
                        </div>
                        <span className="text-caption font-semibold text-white/60">{f.score}/5</span>
                      </div>
                    </div>
                    <div className="ml-12 p-3 rounded-xl bg-white/[0.03] border border-white/[0.05]">
                      <p className="text-caption text-white/60 leading-relaxed italic">
        ...
        {tab === "chart" && (
          <div className="rounded-2xl bg-white/[0.03] border border-white/[0.06] p-6">
            <h3 className="text-body font-semibold text-white/80 mb-6">
              {ar ? "توزيع درجات الأداء" : "Performance Score Distribution"}
            </h3>
            <div className="space-y-4">
              {distribution.map((d, i) => (
                <div key={d.label} className="flex items-center gap-4">
                  <span className="text-caption text-white/50 w-12 text-right font-mono">{d.label}</span>
                  <div className="flex-1 h-8 bg-white/5 rounded-lg overflow-hidden">
                    ...
                    >
                      {d.count > 0 && (
                        <span className="text-caption font-bold text-white">{d.count}</span>
                      )}
                    </motion.div>
                  </div>
                  <span className="text-caption text-white/40 w-8 text-center">
                    {d.count} {ar ? "موظف" : "emp"}
                  </span>
                </div>
              ))}
            ...
            <div className="mt-6 pt-4 border-t border-white/5 grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <div className="text-center">
                <p className="text-display font-bold text-warning">
              ...
              <div className="sticky top-0 z-10 flex items-center justify-between p-5 border-b border-white/5 bg-[#12162a]/95 backdrop-blur-sm rounded-t-2xl">
                <div className="flex items-center gap-3">
                  {(() => {
                    ...
                    return (
                      <div
                        className="w-10 h-10 rounded-full flex items-center justify-center text-body font-bold text-white"
                        style={{ backgroundColor: emp?.avatar_color ?? "#666" }}
                      >
                        {emp ? (ar ? emp.full_name_ar[0] : emp.full_name[0]) : "?"}
                      </div>
                  ...
                  <div>
                    <h2 className="text-body-lg font-bold text-white/90">
                      {empName(selectedReview.employee_id)}
                    </h2>
                    <p className="text-caption text-white/40">{selectedReview.period}</p>
                  </div>
                </div>
                <button
                  onClick={() => setSelectedReview(null)}
              ...
              <div className="p-5 space-y-5">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    <div key={s.label} className={`text-center p-4 rounded-xl border ${scoreBg(s.value)}`}>
                      <p className="text-micro text-white/40 uppercase tracking-wider mb-1">{s.label}</p>
                      <p className={`text-display font-bold ${scoreColor(s.value)}`}>{s.value}</p>
                    </div>
                  ))}
                </div>
                ...
                <div className="rounded-xl bg-warning/5 border border-warning/30 p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="text-caption font-semibold text-warning uppercase tracking-wider">
                ...
                {/* Timeline */}
                <div className="flex items-center justify-between text-caption text-white/30 pt-2 border-t border-white/5">
