  ...
  return (
    <div className="min-h-full px-6 md:px-8 py-6 max-w-[1440px] mx-auto space-y-5">
      ...
      <motion.div variants={cardV} custom={0} initial="hidden" animate="visible"
        ...
        <div>
          <h1 className="text-heading font-semibold tracking-tight" style={serif}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          ...
          <motion.div key={i} variants={cardV} custom={i + 1} initial="hidden" animate="visible"
            ...
            <p className="text-heading font-bold text-foreground" style={serif}>{kpi.value}</p>
            <p className="text-micro text-muted-foreground mt-0.5 leading-tight">{kpi.label}</p>
          </motion.div>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
          {HR_JOB_OPENINGS.map((job, idx) => (
            <motion.div key={job.id} variants={cardV} custom={idx} initial="hidden" animate="visible"
              ...
              <div className="flex items-center justify-between mb-2.5">
                <span className={`text-micro px-2 py-0.5 rounded-full font-medium ${
                  job.status === "open" ? "bg-emerald-100 text-emerald-700"
                    ...
                    : job.status === "on_hold" ? "bg-warning/15 text-warning"
              ...
              <h4 className="text-caption font-medium mb-1.5 leading-snug group-hover:text-brand-ink transition-colors">
                {ar ? job.title_ar : job.title}
              </h4>
              <div className="flex items-center gap-1.5 text-micro text-muted-foreground mb-2">
                <Building2 size={9} />
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input
                ...
                className="w-48 h-8 pl-8 pr-3 rounded-lg border border-border bg-background text-micro placeholder:text-muted-foreground/40 focus:outline-none focus:ring-1 focus:ring-brand-ink/30"
            ...
            <select
              ...
              className="h-8 px-2 rounded-lg border border-border bg-background text-micro cursor-pointer focus:outline-none focus:ring-1 focus:ring-brand-ink/30"
            >
            ...
            <div key={stage.key} className="w-[260px] shrink-0">
              ...
              <div className="flex items-center justify-between mb-2.5 px-1">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-caption font-semibold">{ar ? stage.ar : stage.en}</span>
                </div>
                <span className="text-micro text-muted-foreground bg-muted px-1.5 py-0.5 rounded-full font-medium">
              ...
              >
                      ...
                      <div className="flex items-start justify-between mb-2">
                        ...
                        {candidate.rating != null && candidate.rating > 0 && (
                          <div className="flex items-center gap-0.5 shrink-0">
                            ...
                            <span className="text-micro font-medium">{candidate.rating}</span>
                          </div>
                        )}
                      </div>

                      <p className="text-micro text-muted-foreground line-clamp-2 mb-2 leading-relaxed">
                        {ar ? candidate.notes_ar : candidate.notes}
                      </p>

                      <div className="flex items-center justify-between pt-2 border-t border-border/30">
                        <span className={`inline-flex items-center gap-1 text-micro font-medium px-1.5 py-0.5 rounded-full ${source.color} ${source.bg}`}>
                          <SourceIcon size={8} />
                          {ar ? source.ar : source.en}
                        </span>
                        <span className="text-micro text-muted-foreground">{fmtDate(candidate.applied_at)}</span>
                      </div>
                    </div>
                  );
                })}
                ...
                {stage.candidates.length === 0 && (
                  <div className="text-center py-10 text-micro text-muted-foreground/40">
          ...
          <div className="w-[260px] shrink-0">
            <div className="flex items-center justify-between mb-2.5 px-1">
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-caption font-semibold">{ar ? STAGE_META.rejected.ar : STAGE_META.rejected.en}</span>
              </div>
              <span className="text-micro text-muted-foreground bg-muted px-1.5 py-0.5 rounded-full font-medium">
              ...
              {rejectedCandidates.map(candidate => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="min-w-0">
                        <p className="text-caption font-medium truncate">{ar ? candidate.name_ar : candidate.name}</p>
                        <p className="text-micro text-muted-foreground truncate">{candidate.phone}</p>
                      </div>
                    </div>
                    <p className="text-micro text-muted-foreground line-clamp-2 mb-2">{ar ? candidate.notes_ar : candidate.notes}</p>
                    <div className="flex items-center justify-between pt-2 border-t border-border/30">
                      <span className={`inline-flex items-center gap-1 text-micro font-medium px-1.5 py-0.5 rounded-full ${source.color} ${source.bg}`}>
                        <SourceIcon size={8} />
                        {ar ? source.ar : source.en}
                ...
                return (
                  <motion.div key={iv.id} variants={cardV} custom={idx} initial="hidden" animate="visible"
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <span className={`text-micro px-2 py-0.5 rounded-full font-medium ${typeMeta.color}`}>
                        {ar ? typeMeta.ar : typeMeta.en}
                      </span>
                      <span className="text-micro px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 font-medium">
                        {ar ? "قادمة" : "Scheduled"}
                      </span>
                    </div>
              ...
              {completedInterviews.map(iv => {
                ...
                return (
                  <div key={iv.id}
                    ...
                    <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-brand-ink text-micro font-bold shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-micro px-1.5 py-0.5 rounded-full font-medium ${typeMeta.color}`}>
                          {ar ? typeMeta.ar : typeMeta.en}
                        </span>
                      </div>
                      <p className="text-micro text-muted-foreground truncate">
                    ...
                    <div className="text-right shrink-0">
                      {iv.rating != null && (
                        <div className="flex items-center gap-0.5 justify-end mb-0.5">
                          {[1, 2, 3, 4, 5].map(n => (
                            <Star key={n} size={8} className={n <= iv.rating! ? "text-warning fill-warning" : "text-muted-foreground/20"} />
                          ))}
                        </div>
                      )}
                      <p className="text-micro text-muted-foreground">{fmtDate(iv.date)}</p>
                    </div>
                  </div>
                );
              })}
            ...
            >
              ...
              <div className="p-6 space-y-5 overflow-y-auto flex-1">
                ...
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-xl bg-primary/10 flex items-center justify-center text-brand-ink text-title font-bold shrink-0">
                    {selectedCandidate.name.split(" ").map(w => w[0]).join("")}
                  </div>
                  <div className="min-w-0">
                    <p className="text-body-lg font-medium">{ar ? selectedCandidate.name_ar : selectedCandidate.name}</p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="p-3 rounded-xl bg-muted/30">
                    <p className="text-micro text-muted-foreground mb-1">{ar ? "المرحلة" : "Stage"}</p>
                    <span className={`text-micro font-medium ${STAGE_META[selectedCandidate.stage]?.color}`}>
                      {ar ? STAGE_META[selectedCandidate.stage]?.ar : STAGE_META[selectedCandidate.stage]?.en}
                    </span>
                  </div>
                  {/* Rating */}
                  <div className="p-3 rounded-xl bg-muted/30">
                    <p className="text-micro text-muted-foreground mb-1">{ar ? "التقييم" : "Rating"}</p>
                  ...
                  {/* Source */}
                  <div className="p-3 rounded-xl bg-muted/30">
                    <p className="text-micro text-muted-foreground mb-1">{ar ? "المصدر" : "Source"}</p>
                    <span className={`inline-flex items-center gap-1 text-micro font-medium ${SOURCE_META[selectedCandidate.source]?.color}`}>
                  ...
                  {/* Applied Date */}
                  <div className="p-3 rounded-xl bg-muted/30">
                    <p className="text-micro text-muted-foreground mb-1">{ar ? "تاريخ التقديم" : "Applied"}</p>
                    <p className="text-micro font-medium">{fmtDate(selectedCandidate.applied_at)}</p>
                  </div>
                  {/* Offer Amount */}
                  {selectedCandidate.offer_amount != null && (
                    <div className="p-3 rounded-xl bg-muted/30">
                      <p className="text-micro text-muted-foreground mb-1">{ar ? "مبلغ العرض" : "Offer Amount"}</p>
                      <p className="text-micro font-medium text-emerald-600">{formatEGP(selectedCandidate.offer_amount)}</p>
                    </div>
                  )}
                  {/* Offer Status */}
                  {selectedCandidate.offer_status != null && (
                    <div className="p-3 rounded-xl bg-muted/30">
                      <p className="text-micro text-muted-foreground mb-1">{ar ? "حالة العرض" : "Offer Status"}</p>
                ...
                {/* Stage Selector */}
                <div>
                  <p className="text-micro text-muted-foreground mb-2">{ar ? "تغيير المرحلة" : "Change Stage"}</p>
                  <div className="flex flex-wrap gap-1.5">
                    {ALL_STAGES.map(stage => (
                      <button
                        key={stage}
                        onClick={() => setModalStage(stage)}
                        className={`text-micro px-2.5 py-1 rounded-lg font-medium transition-all ${
                          modalStage === stage
                ...
                {/* Notes */}
                <div>
                  <p className="text-micro text-muted-foreground mb-1.5 flex items-center gap-1.5">
                    <MessageSquare size={11} />
                    {ar ? "ملاحظات" : "Notes"}
                  </p>
                  <p className="text-caption text-foreground leading-relaxed bg-muted/20 p-3 rounded-xl">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {selectedCandidate.interview_dates.map((d, i) => (
                        <span key={i} className="text-micro px-2 py-0.5 rounded-full bg-cyan-50 text-cyan-700 font-medium">
                          {fmtDate(d)}
                        </span>
                      ))}
                    </div>
