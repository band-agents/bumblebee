    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-foreground/25 backdrop-blur-[2px]" onClick={onClose}>
      <div className="w-full max-w-[420px] rounded-2xl bg-background border border-border/60 shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-4 border-b border-border/40 flex items-center justify-between">
          <h3 className="text-body-lg font-medium" style={{ fontFamily: "var(--app-font-serif)" }}>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => setMode("add")}
              className={`h-10 rounded-xl border text-caption font-medium flex items-center justify-center gap-1.5 transition-colors ${mode === "add" ? "border-emerald-300 bg-emerald-50 text-emerald-700" : "border-border/60 text-muted-foreground hover:bg-muted"}`}>
              <Plus size={13} />{ar ? "إضافة رصيد" : "Add credit"}
            </button>
            <button onClick={() => setMode("deduct")}
              className={`h-10 rounded-xl border text-caption font-medium flex items-center justify-center gap-1.5 transition-colors ${mode === "deduct" ? "border-rose-300 bg-rose-50 text-rose-700" : "border-border/60 text-muted-foreground hover:bg-muted"}`}>
              <Minus size={13} />{ar ? "خصم رصيد" : "Deduct"}
            </button>
          </div>
          <div>
            <label className="block text-micro font-medium text-muted-foreground mb-1.5">{ar ? "المبلغ" : "Amount"}</label>
            <input type="number" min="0" value={amount} onChange={e => setAmount(e.target.value)}
              className={`${inputCls} w-full`} placeholder="0" autoFocus />
          </div>
          <div>
            <label className="block text-micro font-medium text-muted-foreground mb-1.5">{ar ? "السبب" : "Reason"}</label>
          ...
          {mode === "deduct" && amt > customer.balance && (
            <p className="text-micro text-rose-600 flex items-center gap-1.5"><AlertTriangle size={11} />{ar ? "المبلغ أكبر من الرصيد المتاح" : "Amount exceeds available balance"}</p>
          )}
        </div>
    ...
    <div className={`relative w-full h-[420px] rounded-2xl border border-border/50 overflow-hidden ${frameBg} transition-colors`}>
      ...
      <div className={`absolute ${panelV} ${panelSide} w-[240px] shadow-xl border ${dark ? "border-zinc-700 bg-zinc-800" : "border-zinc-200 bg-white"} overflow-hidden`}
        ...
        <div className="px-4 py-3 text-white flex items-center gap-2" style={{ background: cfg.color }}>
          <Icon size={14} />
          <span className="text-caption font-medium">{ar ? cfg.titleAr : cfg.titleEn}</span>
        </div>
        <div className="p-4">
          <p className={`text-micro ${softText}`}>{ar ? "رصيدك" : "Your balance"}</p>
          <p className={`text-heading font-semibold tabular-nums ${frameText}`}>{fmtMoney(1840, cfg.currency, ar)}</p>
          <div className={`mt-3 rounded-lg px-3 py-2 text-micro flex items-center gap-1.5 ${dark ? "bg-zinc-700/60 text-zinc-300" : "bg-zinc-50 text-zinc-600"}`}>
          ...
          <button className="mt-3 w-full h-8 text-micro font-medium text-white transition-opacity hover:opacity-90"
      ...
      <div className={`absolute ${posCls} flex items-center gap-2 px-3 h-10 text-white shadow-lg`}
        ...
        {cfg.launcherStyle === "icon_balance" && (
          <span className="text-micro font-medium tabular-nums">{fmtMoney(1840, cfg.currency, ar)}</span>
        )}
      </div>
    </div>
  );
    ...
    <div className="min-h-full py-8 px-7 md:px-10 max-w-[1020px] mx-auto">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex items-center gap-2.5">
          <span className={`text-micro ${enabled ? "text-emerald-600" : "text-muted-foreground"}`}>
        ...
        <div className="space-y-6">
          ...
          <div className={`${cardCls} p-5`}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-body font-medium">{ar ? "الصادر مقابل المستخدم" : "Issued vs Spent"}</h3>
            ...
            <div className="flex items-end gap-3">
              {ISSUED.map((v, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
                  ...
                  <span className="text-micro text-muted-foreground">{ar ? MONTHS_AR[i] : MONTHS[i]}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className={`${cardCls} overflow-hidden`}>
            <div className="px-5 py-3.5 border-b border-border/30">
              <h3 className="text-body font-medium">{ar ? "أحدث النشاط" : "Recent activity"}</h3>
            </div>
            {ACTIVITY.map((a, i) => {
              ...
              return (
                <div key={i} className={`px-5 py-3 flex items-center gap-3 ${i > 0 ? "border-t border-border/20" : ""}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-caption text-foreground truncate">{ar ? a.whoAr : a.whoEn}</p>
                    <p className="text-micro text-muted-foreground truncate">{ar ? a.noteAr : a.noteEn}</p>
                  </div>
                  <span className={`text-caption font-medium tabular-nums ${a.amount > 0 ? "text-emerald-600" : "text-muted-foreground"}`}>
                    {a.amount > 0 ? "+" : ""}{fmtMoney(Math.abs(a.amount), cfg.currency, ar).replace(/^/, a.amount < 0 ? "−" : "")}
                  </span>
                  <span className="text-micro text-muted-foreground/60 w-6 text-end">{a.when}</span>
                </div>
              );
            })}
          </div>
      ...
      {tab === "customers" && (
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-micro text-muted-foreground ms-auto">{filtered.length} {ar ? "محفظة" : "wallets"}</span>
          </div>
          ...
          <div className={`${cardCls} overflow-hidden`}>
                ...
                <div key={c.id} className={`px-5 py-3.5 flex items-center gap-3.5 group hover:bg-muted/30 transition-colors ${i > 0 ? "border-t border-border/20" : ""}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-body font-medium text-foreground truncate">{ar ? c.nameAr : c.nameEn}</p>
                      <span className={`text-micro font-medium px-1.5 py-0.5 rounded-full ${tp.cls} flex items-center gap-0.5`}>
                        {c.tier === "vip" && <Crown size={8} />}{ar ? tp.ar : tp.en}
                      </span>
                    </div>
                    <p className="text-micro text-muted-foreground truncate">{c.email}</p>
                  </div>
                  {c.expiringSoon && (
                    <span className="hidden md:flex items-center gap-1 text-micro text-warning bg-warning/10 px-2 py-1 rounded-md">
                      <Clock size={10} />{fmtMoney(c.expiringSoon, cfg.currency, ar)} {ar ? "تنتهي قريبًا" : "expiring soon"}
                    </span>
                  )}
                  <div className="text-end w-24">
                    <p className="text-body font-medium tabular-nums text-emerald-600">{fmtMoney(c.balance, cfg.currency, ar)}</p>
                    <p className="text-micro text-muted-foreground">{ar ? "الرصيد" : "balance"}</p>
                  </div>
                  <div className="hidden md:block text-end w-24">
                    <p className="text-caption tabular-nums text-muted-foreground">{fmtMoney(c.lifetimeEarned, cfg.currency, ar)}</p>
                    <p className="text-micro text-muted-foreground/60">{ar ? "مكتسب إجمالًا" : "lifetime earned"}</p>
                  </div>
                  <button onClick={() => setAdjusting(c)}
                    className="h-8 px-3 rounded-lg border border-border/60 text-micro text-muted-foreground hover:text-foreground hover:bg-muted transition-colors opacity-0 group-hover:opacity-100">
                    {ar ? "تعديل" : "Adjust"}
                  </button>
                </div>
              );
            ...
            {filtered.length === 0 && (
              <div className="py-12 text-center">
                ...
                <p className="text-caption text-muted-foreground">{ar ? "لا توجد نتائج" : "No wallets match"}</p>
              </div>
            )}
          </div>
        </div>
          ...
          <RuleRow icon={Coins} ar={ar}
            ...
            <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-caption text-muted-foreground">{ar ? "مضاعفات المستويات" : "Tier multipliers"}</span>
                {(Object.keys(cfg.tierMultipliers) as (keyof WalletConfig["tierMultipliers"])[]).map(t => (
                  <label key={t} className={`flex items-center gap-1.5 px-2 py-1 rounded-lg ${TIER_PILL[t].cls} text-micro font-medium`}>
                    ...
                    <input type="number" min="1" max="5" step="0.25" value={cfg.tierMultipliers[t]}
                      ...
                      className="w-12 h-6 px-1 rounded-md border border-border/60 bg-white/70 text-micro tabular-nums text-center focus:outline-none" />×
                  </label>
                ))}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] gap-6 items-start">
          ...
          <div className="space-y-4">
            <div className={`${cardCls} p-5 space-y-4`}>
              <h3 className="text-body font-medium">{ar ? "المُشغّل" : "Launcher"}</h3>
              <div>
                <p className="text-micro font-medium text-muted-foreground mb-2">{ar ? "الموضع" : "Position"}</p>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => update({ launcherStyle: "icon" })}
                    className={`h-9 rounded-lg border text-micro transition-colors ${cfg.launcherStyle === "icon" ? "border-primary bg-primary/5 text-foreground" : "border-border/60 text-muted-foreground hover:bg-muted"}`}>
                    {ar ? "أيقونة فقط" : "Icon only"}
                  </button>
                  <button onClick={() => update({ launcherStyle: "icon_balance" })}
                    className={`h-9 rounded-lg border text-micro transition-colors ${cfg.launcherStyle === "icon_balance" ? "border-primary bg-primary/5 text-foreground" : "border-border/60 text-muted-foreground hover:bg-muted"}`}>
                    {ar ? "أيقونة + الرصيد" : "Icon + balance"}
                  </button>
                </div>
              </div>
              <div>
                <p className="text-micro font-medium text-muted-foreground mb-2">{ar ? "الأيقونة" : "Icon"}</p>
                <div className="flex gap-2">
                  {([["wallet", Wallet], ["gift", Gift], ["sparkles", Sparkles]] as [LauncherIcon, React.ElementType][]).map(([k, I]) => (
                    <button key={k} onClick={() => update({ launcherIcon: k })}
                      className={`w-10 h-10 rounded-lg border flex items-center justify-center transition-colors ${cfg.launcherIcon === k ? "border-primary bg-primary/5 text-brand-ink" : "border-border/60 text-muted-foreground hover:bg-muted"}`}>
                      <I size={15} />
                    </button>
                  ))}
                </div>
            ...
            <div className={`${cardCls} p-5 space-y-4`}>
              ...
              <div>
                <p className="text-micro font-medium text-muted-foreground mb-2">{ar ? "اللون الأساسي" : "Brand color"}</p>
                <div className="flex items-center gap-2 flex-wrap">
                  {SWATCHES.map(c => (
              ...
              <div>
                <p className="text-micro font-medium text-muted-foreground mb-2 flex justify-between">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-micro font-medium text-muted-foreground mb-1.5">{ar ? "العنوان (EN)" : "Title (EN)"}</label>
                  <input value={cfg.titleEn} onChange={e => update({ titleEn: e.target.value })} className={`${inputCls} w-full`} />
                </div>
                <div>
                  <label className="block text-micro font-medium text-muted-foreground mb-1.5">{ar ? "العنوان (AR)" : "Title (AR)"}</label>
      ...
      {tab === "settings" && (
        <div className="space-y-4 max-w-[620px]">
          <div className={`${cardCls} p-5 space-y-4`}>
            <h3 className="text-body font-medium">{ar ? "عام" : "General"}</h3>
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-caption text-foreground">{ar ? "العملة" : "Currency"}</p>
                <p className="text-micro text-muted-foreground">{ar ? "تظهر في الودجت وكل الرسائل" : "Shown in the widget and all messages"}</p>
              </div>
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-caption text-foreground">{ar ? "الحد الأقصى للرصيد" : "Max wallet balance"}</p>
                <p className="text-micro text-muted-foreground">{ar ? "حماية من التراكم غير الطبيعي" : "Guard against abnormal accumulation"}</p>
              </div>
          ...
          <div className={`${cardCls} p-5 space-y-4`}>
            <h3 className="text-body font-medium flex items-center gap-1.5"><CreditCard size={13} />{ar ? "الدفع" : "Checkout"}</h3>
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-caption text-foreground">{ar ? "الدفع الجزئي" : "Partial payments"}</p>
                <p className="text-micro text-muted-foreground">{ar ? "استخدام الرصيد + بطاقة في نفس الطلب" : "Combine wallet credit + card on one order"}</p>
              </div>
              <Toggle on={cfg.partialPay} onChange={v => update({ partialPay: v })} />
            </div>
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-caption text-foreground">{ar ? "المحفظة في صفحة الحساب" : "Wallet in account page"}</p>
                <p className="text-micro text-muted-foreground">{ar ? "تبويب محفظة داخل حساب العميل في المتجر" : "A wallet tab inside the customer's store account"}</p>
              </div>
          ...
          <div className={`${cardCls} p-5 space-y-4`}>
            <h3 className="text-body font-medium flex items-center gap-1.5"><Bell size={13} />{ar ? "الإشعارات" : "Notifications"}</h3>
            {[
              ...
              <div key={n.key} className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-caption text-foreground">{ar ? n.ar : n.en}</p>
                    <p className="text-micro text-muted-foreground">{ar ? n.descAr : n.descEn}</p>
                  </div>
                </div>
