      ...
      <div className="relative bg-background border border-border/40 rounded-2xl shadow-2xl w-full max-w-[600px] max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-4 border-b border-border/40 flex items-center justify-between sticky top-0 bg-background z-10">
          <h2 className="text-title font-medium flex items-center gap-2" style={{ fontFamily: "var(--app-font-serif)" }}>
            <GitMerge size={16} className="text-brand-ink" />
        ...
        <div className="px-6 py-5">
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            {[{ m: m1, t: t1, label: ar ? "الملف الأول (أساسي)" : "Profile A (Primary)" }, { m: m2, t: t2, label: ar ? "الملف الثاني" : "Profile B" }].map(({ m, t, label }, i) => (
              <div key={i} className="border border-border/40 rounded-xl p-4">
                <p className="text-micro text-muted-foreground uppercase tracking-wide mb-2">{label}</p>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div>
                    <p className="text-caption font-medium">{ar ? m.nameAr : m.nameEn}</p>
                    <span className={`text-micro font-medium px-1.5 py-0.5 rounded-full ${t.pill}`}>{ar ? t.ar : t.en}</span>
                  </div>
                </div>
          ...
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center">
              <GitMerge size={14} className="text-brand-ink" />
            </div>
            <div className="h-px flex-1 bg-border/40" />
          </div>
          ...
          <div className="border-2 border-primary/30 rounded-xl p-5 bg-primary/3">
            <p className="text-micro text-brand-ink uppercase tracking-wide font-medium mb-3">{ar ? "النتيجة بعد الدمج" : "Merged Result"}</p>
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center text-caption font-semibold text-white" style={{ background: m1.avatarColor }}>
                {m1.nameEn.split(" ").map(w => w[0]).join("")}
              </div>
              <div>
                <p className="text-body-lg font-medium" style={{ fontFamily: "var(--app-font-serif)" }}>{ar ? m1.nameAr : m1.nameEn}</p>
                <span className={`text-micro font-medium px-2 py-0.5 rounded-full ${mt.pill}`}>{ar ? mt.ar : mt.en}</span>
              </div>
            </div>

            <div className="grid grid-cols-4 gap-3">
              {[
                { label: ar ? "النقاط" : "Points", value: fmtPts(mergedPoints), color: "text-brand-ink" },
                ...
                <div key={i} className="text-center">
                  ...
                  <p className={`text-body-lg font-medium tabular-nums ${item.color}`} style={{ fontFamily: "var(--app-font-serif)" }}>{item.value}</p>
                </div>
              ))}
            </div>

            <div className="mt-3 pt-3 border-t border-primary/15 text-micro text-muted-foreground space-y-1">
        ...
        <div className="px-6 py-4 border-t border-border/40 flex items-center justify-between sticky bottom-0 bg-background">
          <button onClick={onDismiss}
            className="h-9 px-4 rounded-xl border border-border text-caption text-muted-foreground hover:text-foreground hover:bg-muted transition-colors flex items-center gap-1.5">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={onMerge}
              className="h-9 px-5 rounded-xl bg-primary text-primary-foreground text-caption font-medium hover:opacity-90 transition-opacity flex items-center gap-2">
...
export default function LoyaltyMergePage() {
  ...
  const confMeta = {
    ...
    medium: { en: "Medium", ar: "متوسط", color: "text-warning", bg: "bg-warning/10 border-warning/30" },
  ...
  return (
    <div className="min-h-full py-8 px-7 md:px-10 max-w-[960px] mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-caption text-muted-foreground mt-1">{ar ? "اكتشاف ودمج الملفات المكررة في برنامج الولاء" : "Detect and merge duplicate loyalty profiles"}</p>
      </div>

      {/* Summary */}
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="border border-border/40 rounded-xl p-4 bg-background">
          <AlertTriangle size={14} className="text-warning mb-2" />
          <p className="text-heading font-medium tabular-nums text-warning" style={{ fontFamily: "var(--app-font-serif)" }}>{pending.length}</p>
        ...
        <div className="border border-border/40 rounded-xl p-4 bg-background">
          ...
          <p className="text-heading font-medium tabular-nums" style={{ fontFamily: "var(--app-font-serif)" }}>{dismissed.length}</p>
          <p className="text-micro text-muted-foreground">{ar ? "تم التجاهل" : "Dismissed"}</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {pending.length === 0 && merged.length === 0 && dismissed.length === 0 ? (
          <div className="py-14 text-center border border-border/40 rounded-xl">
            ...
            <p className="text-body text-muted-foreground">{ar ? "لا توجد تكرارات محتملة" : "No potential duplicates found"}</p>
          </div>
        ) : (
          <>
            {/* Pending */}
            {pending.length > 0 && (
              <div>
                <h3 className="text-micro font-medium text-muted-foreground uppercase tracking-wide mb-2">{ar ? "يحتاج مراجعة" : "Needs Review"}</h3>
                {pending.map(c => {
                    ...
                    <button key={c.id} onClick={() => setSelectedCandidate(c)}
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="relative w-12 h-9 shrink-0">
                          <div className="absolute left-0 top-0 w-8 h-8 rounded-lg flex items-center justify-center text-micro font-semibold text-white border-2 border-background z-10" style={{ background: c.member1.avatarColor }}>
                            {c.member1.nameEn.split(" ").map(w => w[0]).join("")}
                          </div>
                          <div className="absolute left-4 top-1 w-7 h-7 rounded-lg flex items-center justify-center text-micro font-semibold text-white border-2 border-background" style={{ background: c.member2.avatarColor }}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            <p className="text-caption font-medium">{ar ? c.member1.nameAr : c.member1.nameEn}</p>
                            <ArrowRight size={10} className="text-muted-foreground/40" />
                            <p className="text-caption font-medium">{ar ? c.member2.nameAr : c.member2.nameEn}</p>
                          </div>
                          <p className="text-micro text-muted-foreground truncate">{ar ? c.matchReasonAr : c.matchReason}</p>
                        </div>

                        <span className={`text-micro font-semibold px-2 py-1 rounded-full border ${cm.bg} ${cm.color}`}>
                          {ar ? cm.ar : cm.en}
                        </span>
            ...
            {(merged.length > 0 || dismissed.length > 0) && (
              <div className="mt-4">
                <h3 className="text-micro font-medium text-muted-foreground uppercase tracking-wide mb-2">{ar ? "تم المعالجة" : "Resolved"}</h3>
                {[...merged, ...dismissed].map(c => (
                  <div key={c.id} className="border border-border/25 rounded-xl p-3 bg-muted/10 mb-2 opacity-60">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-micro text-muted-foreground">{c.status === "merged" ? (ar ? "مدمج" : "Merged") : (ar ? "مُتجاهل" : "Dismissed")}</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className="mt-6 bg-muted/15 border border-border/30 rounded-xl p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-micro">
          <div className="flex gap-2">
            <div className="w-5 h-5 rounded-full bg-primary/10 flex items-center justify-center text-micro font-bold text-brand-ink shrink-0">1</div>
            <div>
          ...
          <div className="flex gap-2">
            <div className="w-5 h-5 rounded-full bg-primary/10 flex items-center justify-center text-micro font-bold text-brand-ink shrink-0">2</div>
            <div>
          ...
          <div className="flex gap-2">
            <div className="w-5 h-5 rounded-full bg-primary/10 flex items-center justify-center text-micro font-bold text-brand-ink shrink-0">3</div>
            <div>
