...
const DEMO_REDEMPTIONS: RedemptionRecord[] = [
  { id: "rd-001", memberId: "lm-002", memberName: "Sara Al-Rashid", channel: "online", pointsRedeemed: 10000, discountAmount: 100, discountCode: "BEE-LM00-R1A2B3", status: "used", shopifyOrderId: "SHP-4528", createdAt: "2026-06-04T12:00:00Z" },
  { id: "rd-002", memberId: "lm-001", memberName: "Ahmed Mohamed", channel: "online", pointsRedeemed: 2000, discountAmount: 20, discountCode: "BEE-LM00-X4Y5Z6", status: "active", expiresAt: "2026-07-04T12:00:00Z", createdAt: "2026-06-03T10:15:00Z" },
  ...
  { id: "rd-005", memberId: "lm-008", memberName: "Nour El-Din", channel: "online", pointsRedeemed: 3000, discountAmount: 30, discountCode: "BEE-LM00-W7V8U9", status: "expired", expiresAt: "2026-05-25T00:00:00Z", createdAt: "2026-04-25T10:00:00Z" },
  { id: "rd-006", memberId: "lm-003", memberName: "Omar Farouk", channel: "online", pointsRedeemed: 1500, discountAmount: 15, discountCode: "BEE-LM00-K1L2M3", status: "active", expiresAt: "2026-07-10T00:00:00Z", createdAt: "2026-06-10T09:00:00Z" },
];
...
const STATUS_META: Record<string, { en: string; ar: string; dot: string; pill: string }> = {
  ...
  used:      { en: "Used",      ar: "مستخدم",   dot: "bg-primary",     pill: "bg-primary/10 text-brand-ink" },
...
function GenerateCodeModal({ ar, members, onClose, onGenerate }: {
  ...
  function handleGenerate() {
    ...
    setTimeout(() => {
      const code = `BEE-${selectedMember.id.substring(3, 7).toUpperCase()}-${Date.now().toString(36).toUpperCase().substring(0, 6)}`;
      ...
      <div className="relative bg-background border border-border/40 rounded-2xl shadow-2xl w-full max-w-[480px] max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 py-5 space-y-4">
          {!generatedCode ? (
            <>
              ...
              {!selectedMember ? (
                <div>
                  <label className="block text-micro font-medium text-muted-foreground mb-1.5">{ar ? "ابحث عن العضو" : "Search Member"}</label>
                  <div className="relative">
                    <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground/50" />
                    <input value={search} onChange={e => setSearch(e.target.value)} autoFocus
                      placeholder={ar ? "اسم، هاتف، أو بريد..." : "Name, phone, or email..."}
                      className="w-full h-10 pl-9 pr-3 rounded-xl border border-border/80 bg-card text-body focus:outline-none focus:ring-2 focus:ring-brand-ink/20" />
                  </div>
                  {filtered.length > 0 && (
                      ...
                      {filtered.map(m => {
                        ...
                        return (
                          <button key={m.id} onClick={() => setSelectedMember(m)}
                            ...
                            <div className="w-8 h-8 rounded-lg flex items-center justify-center text-micro font-semibold text-white" style={{ background: m.avatarColor }}>
                              {m.nameEn.split(" ").map(w => w[0]).join("")}
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-caption font-medium truncate">{ar ? m.nameAr : m.nameEn}</p>
                              <p className="text-micro text-muted-foreground">{m.phone}</p>
                            </div>
                            <span className={`text-micro font-semibold px-2 py-0.5 rounded-full ${tier.pill}`}>{fmtPts(m.currentPoints)} {ar ? "نقطة" : "pts"}</span>
                          </button>
                        );
                      })}
                    </div>
                ...
                <>
                  ...
                  <div className="flex items-center gap-3 p-3 bg-muted/20 rounded-xl">
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center text-micro font-semibold text-white" style={{ background: selectedMember.avatarColor }}>
                      {selectedMember.nameEn.split(" ").map(w => w[0]).join("")}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-body font-medium">{ar ? selectedMember.nameAr : selectedMember.nameEn}</p>
                      <p className="text-micro text-muted-foreground">{fmtPts(selectedMember.currentPoints)} {ar ? "نقطة متاحة" : "points available"}</p>
                    </div>
                    <button onClick={() => setSelectedMember(null)} className="text-micro text-brand-ink hover:underline">{ar ? "تغيير" : "Change"}</button>
                  </div>

                  {/* Points input */}
                  <div>
                    <label className="block text-micro font-medium text-muted-foreground mb-1.5">{ar ? "النقاط للاستبدال" : "Points to Redeem"}</label>
                    <input type="number" value={points} onChange={e => setPoints(e.target.value)}
                      placeholder={ar ? "مثال: 5000" : "e.g. 5000"} min={500} max={selectedMember.currentPoints}
                      className="w-full h-10 px-3.5 rounded-xl border border-border/80 bg-card text-body-lg font-medium tabular-nums focus:outline-none focus:ring-2 focus:ring-brand-ink/20" />
                    <p className="text-micro text-muted-foreground mt-1">{ar ? "الحد الأدنى 500 نقطة" : "Minimum 500 points"} · {ar ? "الحد الأقصى" : "Max"} {fmtPts(selectedMember.currentPoints)}</p>
                  </div>

                  {/* Discount preview */}
                  {numPoints >= 500 && (
                    <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 text-center">
                      <p className="text-micro text-muted-foreground mb-1">{ar ? "قيمة الخصم" : "Discount Value"}</p>
                      <p className="text-display font-medium text-brand-ink tabular-nums" style={{ fontFamily: "var(--app-font-serif)" }}>{fmtCurrency(discount)}</p>
            ...
            <div className="text-center py-4">
              ...
              <div className="bg-muted/30 border border-border/40 rounded-xl p-4 mb-4">
                <p className="text-micro text-muted-foreground mb-2">{ar ? "كود الخصم" : "Discount Code"}</p>
        ...
        {!generatedCode ? (
          ...
          <div className="px-6 py-4 border-t border-border/40 flex justify-center sticky bottom-0 bg-background">
            <button onClick={onClose} className="h-9 px-6 rounded-xl bg-primary text-primary-foreground text-caption font-medium hover:opacity-90 transition-opacity">
              {ar ? "تم" : "Done"}
            </button>
          </div>
        )}
...
export default function LoyaltyRedemptionsPage() {
  ...
  function handleGenerate(memberId: string, points: number) {
    ...
    const newRedemption: RedemptionRecord = {
      ...
      discountCode: `BEE-${memberId.substring(3, 7).toUpperCase()}-${Date.now().toString(36).toUpperCase().substring(0, 6)}`,
      status: "active",
    ...
    <div className="min-h-full py-8 px-7 md:px-10 max-w-[960px] mx-auto">
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {[
          ...
          { value: fmtPts(totalRedeemed), label: ar ? "نقاط مستبدلة" : "Points Redeemed", icon: Gift, color: "text-brand-ink" },
          { value: fmtCurrency(totalDiscount), label: ar ? "خصومات" : "Discounts Given", icon: Ticket, color: "text-emerald-600" },
          { value: `${activeCount}`, label: ar ? "أكواد نشطة" : "Active Codes", icon: CheckCircle2, color: "text-warning" },
        ].map((c, i) => (
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <span className="text-micro text-muted-foreground ms-auto">{filtered.length} {ar ? "عملية" : "redemptions"}</span>
      </div>
      ...
      <div className="border border-border/40 rounded-xl overflow-hidden bg-background divide-y divide-border/25">
        {filtered.length === 0 ? (
          <div className="py-14 text-center">
            ...
            <p className="text-body text-muted-foreground">{ar ? "لا توجد عمليات استبدال" : "No redemptions found"}</p>
          </div>
        ) : filtered.map(rd => {
            ...
            <div key={rd.id} className="flex items-center gap-4 px-5 py-4 hover:bg-muted/10 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  <p className="text-body font-medium truncate">{rd.memberName}</p>
                </div>
                <div className="flex items-center gap-3 text-micro text-muted-foreground">
                  <span>{fmtPts(rd.pointsRedeemed)} {ar ? "نقطة" : "pts"} → {fmtCurrency(rd.discountAmount)}</span>
                  {rd.discountCode && <code className="font-mono text-micro bg-muted/30 px-1.5 py-0.5 rounded">{rd.discountCode}</code>}
              ...
              <div className="text-end shrink-0">
                <p className="text-micro text-foreground tabular-nums">
                  {new Date(rd.createdAt).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}
                </p>
                {rd.expiresAt && rd.status === "active" && (
                  <p className="text-micro text-muted-foreground flex items-center gap-1 justify-end mt-0.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={s.step} className="flex gap-3">
              <div className="w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center text-micro font-bold text-brand-ink shrink-0">{s.step}</div>
              <div>
                <p className="text-micro font-medium text-foreground mb-0.5">{ar ? s.titleAr : s.titleEn}</p>
                <p className="text-micro text-muted-foreground leading-relaxed">{ar ? s.descAr : s.descEn}</p>
              </div>
            </div>
          ))}
        </div>
