    ...
    <div ref={ref} className="relative">
      <div className="relative">
        ...
        <input
          ...
          className={inputCls + " h-9 text-caption pl-8"}
        />
      </div>
            ...
            return (
              ...
              >
                <div className="w-8 h-8 rounded-lg bg-primary/5 flex items-center justify-center shrink-0">
                  <Package size={13} className="text-brand-ink/60" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-caption font-medium truncate">{p.name_en}</p>
                ...
                {meta.suggested_price != null && meta.suggested_price > 0 && (
                  <span className="text-micro font-medium tabular-nums text-muted-foreground shrink-0">{meta.suggested_price.toLocaleString()}</span>
                )}
              </button>
            );
          })}
      ...
      {open && results.length === 0 && query.trim() && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-background border border-border/60 rounded-xl shadow-lg z-30 p-4 text-center text-caption text-muted-foreground">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-background border border-border/40 rounded-2xl shadow-2xl w-full max-w-[520px] max-h-[85vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border/40 shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-primary/10 flex items-center justify-center">
              <Edit3 size={14} className="text-brand-ink" />
            </div>
            <div>
              <h3 className="text-body-lg font-semibold">{ar ? "تعديل المنتج" : "Edit Product"}</h3>
              <p className="text-micro text-muted-foreground">{draft.product || (ar ? `صنف ${index + 1}` : `Item ${index + 1}`)}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-auto p-5 space-y-4">
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-micro font-medium text-muted-foreground">{ar ? "الأبعاد (سم)" : "Dimensions (cm)"}</span>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <div>
                <label className="text-micro text-muted-foreground">{ar ? "عرض" : "Width"}</label>
                <input type="number" value={draft.width || ""} onChange={e => set("width", parseFloat(e.target.value) || 0)} className={inputCls + " h-9"} />
              </div>
              <div>
                <label className="text-micro text-muted-foreground">{ar ? "ارتفاع" : "Height"}</label>
                <input type="number" value={draft.height || ""} onChange={e => set("height", parseFloat(e.target.value) || 0)} className={inputCls + " h-9"} />
              </div>
              <div>
                <label className="text-micro text-muted-foreground">{ar ? "عمق" : "Depth"}</label>
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-micro font-medium text-muted-foreground">{ar ? "الخامات والتشطيب" : "Material & Finish"}</span>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-micro text-muted-foreground">{ar ? "الخامة" : "Material"}</label>
              ...
              <div>
                <label className="text-micro text-muted-foreground">{ar ? "التشطيب" : "Finish"}</label>
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-micro font-medium text-muted-foreground">{ar ? "الكمية والسعر" : "Quantity & Price"}</span>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <div>
                <label className="text-micro text-muted-foreground">{ar ? "الكمية" : "Qty"}</label>
                <input type="number" value={draft.qty} onChange={e => set("qty", parseInt(e.target.value) || 1)} min={1} className={inputCls + " h-9"} />
              </div>
              <div>
                <label className="text-micro text-muted-foreground">{ar ? "سعر الوحدة" : "Unit Price"}</label>
              ...
              <div>
                <label className="text-micro text-muted-foreground">{ar ? "الإجمالي" : "Total"}</label>
                <div className="h-10 flex items-center px-3 text-body font-semibold bg-primary/5 rounded-xl tabular-nums text-brand-ink">
            ...
            <div className="grid grid-cols-3 gap-2 mt-2">
              <div className="col-span-2">
                <label className="text-micro text-muted-foreground">{ar ? "خصم الصنف" : "Line Discount"}</label>
              ...
              <div>
                <label className="text-micro text-muted-foreground">{ar ? "النوع" : "Type"}</label>
                <div className="flex h-9 rounded-xl border border-border/60 overflow-hidden">
                  {(["pct", "fixed"] as const).map((t) => (
                    <button key={t} type="button" onClick={() => set("discountType", t)}
                      className={`flex-1 text-caption font-medium transition-colors ${(draft.discountType ?? "pct") === t ? "bg-foreground text-background" : "text-muted-foreground hover:bg-muted/50"}`}>
                      {t === "pct" ? "%" : currency}
                    </button>
                  ))}
                </div>
        ...
        <div className="px-5 py-4 border-t border-border/40 shrink-0 flex gap-3">
          <button type="button" onClick={onClose} className="flex-1 h-10 rounded-xl border border-border/60 text-body font-medium hover:bg-muted/50">
            {ar ? "إلغاء" : "Cancel"}
          </button>
  ...
  return (
    <div className="border border-border/40 rounded-xl p-4 bg-muted/10 space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-micro font-medium text-muted-foreground">{ar ? `صنف ${index + 1}` : `Item ${index + 1}`}</span>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="col-span-2">
          <input value={item.product} onChange={(e) => set("product", e.target.value)} className={inputCls + " h-9 text-caption"} placeholder={ar ? "اسم المنتج" : "Product name"} />
        </div>
        <div className="col-span-2">
          <input value={item.description} onChange={(e) => set("description", e.target.value)} className={inputCls + " h-9 text-caption"} placeholder={ar ? "الوصف" : "Description"} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div>
          <label className="text-micro text-muted-foreground">{ar ? "الكمية" : "Qty"}</label>
          <input type="number" value={item.qty} onChange={(e) => set("qty", parseInt(e.target.value) || 1)} min={1} className={inputCls + " h-8 text-micro"} />
        </div>
        <div>
          <label className="text-micro text-muted-foreground">{ar ? "سعر الوحدة" : "Unit Price"}</label>
          <input type="number" value={item.unitPrice} onChange={(e) => set("unitPrice", parseFloat(e.target.value) || 0)} min={0} className={inputCls + " h-8 text-micro"} />
        </div>
        <div>
          <label className="text-micro text-muted-foreground">{ar ? "الإجمالي" : "Total"}</label>
          <div className="h-8 flex items-center px-3 text-caption font-medium bg-muted/30 rounded-xl tabular-nums">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-background border border-border/60 rounded-2xl shadow-xl w-full max-w-[640px] max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-6 py-5 border-b border-border/40 shrink-0">
          <h2 className="text-title font-medium" style={{ fontFamily: "var(--app-font-serif)" }}>{ar ? "عرض سعر جديد" : "New Quotation"}</h2>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-auto p-6 space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-caption font-medium">{ar ? "الأصناف" : "Items"}</h3>
              <div className="flex items-center gap-2">
                {products.length > 0 && (
                  <div className="relative group">
                    <button type="button" className="flex items-center gap-1 text-micro text-emerald-600 font-medium hover:opacity-70">
                      ...
                      {products.slice(0, 10).map(p => {
                        ...
                        return (
                          <button key={p.id} type="button" onClick={() => addFromProduct(p)}
                            ...
                            <Package size={11} className="text-brand-ink/50 shrink-0" />
                            <div className="min-w-0 flex-1">
                              <p className="text-micro font-medium truncate">{p.name_en}</p>
                              <p className="text-micro text-muted-foreground">{meta.category || meta.main_material || ""}</p>
                            </div>
                            {(meta.suggested_price ?? 0) > 0 && (
                              <span className="text-micro tabular-nums text-muted-foreground">{meta.suggested_price?.toLocaleString()}</span>
                            )}
                          </button>
                        );
                      })}
                ...
                <button type="button" onClick={addItem} className="flex items-center gap-1 text-micro text-brand-ink font-medium hover:opacity-70">
          ...
          {/* Cost breakdown */}
          <div>
            <h3 className="text-caption font-medium mb-3">{ar ? "تكاليف التصنيع" : "Cost Breakdown"}</h3>
            <div className="grid grid-cols-5 gap-2">
              {[
                ...
                <div key={c.key}>
                  <label className="text-micro text-muted-foreground">{ar ? c.ar : c.en}</label>
                  <input type="number" value={(form as Record<string, string>)[c.key]} onChange={(e) => setForm((f) => ({ ...f, [c.key]: e.target.value }))} min={0} className={inputCls + " h-8 text-micro"} placeholder="0" />
                </div>
              ))}
            </div>
          </div>
                ...
                <div className="flex rounded-xl border border-border/60 overflow-hidden shrink-0">
                  {(["pct", "fixed"] as const).map((t) => (
                    <button key={t} type="button" onClick={() => setForm((f) => ({ ...f, orderDiscountType: t }))}
                      className={`px-2.5 text-caption font-medium transition-colors ${form.orderDiscountType === t ? "bg-foreground text-background" : "text-muted-foreground hover:bg-muted/50"}`}>
                      {t === "pct" ? "%" : currency}
                    </button>
                  ))}
                </div>
          ...
          <div className="bg-muted/30 rounded-xl p-4 space-y-2 border border-border/30">
            <div className="flex items-center gap-1.5 mb-1">
              ...
              <span className="text-micro font-medium text-muted-foreground uppercase tracking-wider">{ar ? "حساب مباشر" : "Live Total"}</span>
            </div>
            ...
            <div className="flex justify-between text-body-lg font-semibold border-t border-border/40 pt-2">
      ...
      <div className="border-b border-border/40 px-7 md:px-10 py-7" style={{ background: "linear-gradient(160deg, hsl(var(--muted)/0.3) 0%, hsl(var(--background)) 60%)" }}>
        <div className="max-w-[1100px]">
          <div className="flex items-center gap-2.5 mb-2">
            ...
            <p className="text-micro text-muted-foreground/60 tracking-[0.08em] uppercase">{ar ? "عروض الأسعار" : "Quotations"}</p>
          </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              {quotations.length > 0 && (
                <button onClick={() => {
                  ...
                  exportCSV(rows, `bumblebee-quotations-${new Date().toISOString().slice(0,10)}.csv`);
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-background border border-border/40 rounded-xl px-4 py-3.5">
              ...
              <p className="text-heading font-medium tabular-nums mb-1" style={{ fontFamily: "var(--app-font-serif)" }}>{draftCount}</p>
            ...
            <div className="bg-background border border-border/40 rounded-xl px-4 py-3.5">
              <DollarSign size={14} className="text-brand-ink mb-2" />
              <p className="text-title font-medium tabular-nums mb-1" style={{ fontFamily: "var(--app-font-serif)" }}>{fmt(totalValue)}</p>
              <p className="text-micro text-muted-foreground">{ar ? "إجمالي العروض" : "Total Value"}</p>
            </div>
          </div>
        </div>
      </div>
              ...
              return (
                <div key={q.id} className="bg-background border border-border/40 rounded-xl p-5 hover:shadow-sm hover:border-border/70 transition-all">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                        ...
                        {m.quotation_date && <span className="text-micro text-muted-foreground">{m.quotation_date}</span>}
                      </div>
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-border/30">
                    {q.status === "draft" && (
                      <button onClick={() => updateStatus(q.id, "sent")} className="text-micro text-blue-600 font-medium hover:opacity-70 flex items-center gap-1">
