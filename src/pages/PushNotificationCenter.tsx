    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-start gap-3 min-w-0 flex-1">
          <span className="text-heading shrink-0 mt-0.5">{tm.icon}</span>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 mb-0.5 flex-wrap">
              ...
              <span className={`text-micro font-semibold px-2 py-0.5 rounded-full shrink-0 ${sm.color} ${sm.bg}`}>
                {ar ? sm.ar : sm.en}
              </span>
            </div>
            <p className="text-caption text-muted-foreground line-clamp-2">{ar ? notif.message_ar : notif.message}</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-4 pt-3 border-t border-border/25">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-micro text-muted-foreground">{ar ? "مرسل" : "Sent"}</span>
          <span className="text-micro font-medium text-foreground tabular-nums">{formatNumber(notif.sent_count)}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <Eye size={10} className="text-muted-foreground" />
          <span className="text-micro text-muted-foreground">{ar ? "وصل" : "Delivered"}</span>
          <span className="text-micro font-medium text-foreground tabular-nums">{formatNumber(notif.delivered_count)}</span>
        </div>
        <div className="flex items-center gap-1.5">
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <span className="text-micro text-muted-foreground">{ar ? al.ar : al.en}</span>
        </div>
      </div>
    ...
    <div className="min-h-full px-6 md:px-8 py-6 max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="space-y-3">
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-body text-muted-foreground">{ar ? "لا توجد إشعارات" : "No notifications found"}</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-background border border-border/40 rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-border/30 flex items-center gap-2">
          <Users size={14} strokeWidth={1.75} className="text-brand-ink" />
          ...
          {NOTIFICATION_SEGMENTS.map((seg, i) => (
            <div key={seg.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-muted/15 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-body font-medium text-foreground">{ar ? seg.name_ar : seg.name}</p>
                <p className="text-micro text-muted-foreground">{ar ? seg.description_ar : seg.description}</p>
              </div>
              <span className="text-caption font-medium text-foreground tabular-nums">{formatNumber(seg.user_count)}</span>
            </div>
          ))}
        </div>
      </div>
