  ...
  if (!deal) {
    return (
      <div className="min-h-full flex flex-col items-center justify-center gap-4 p-8">
        ...
        <p className="text-body-lg font-medium text-foreground" style={{ fontFamily: "var(--app-font-serif)" }}>{ar ? "لم يُعثر على الصفقة" : "Deal not found"}</p>
        <button onClick={() => navigate("/sales")} className="flex items-center gap-1.5 text-caption text-brand-ink hover:underline">
    ...
    <div className="min-h-full">
      ...
      <div className="relative border-b border-border/40" style={{ background: "linear-gradient(160deg, hsl(var(--muted)/0.35) 0%, hsl(var(--background)) 65%)" }}>
        <div className="px-8 md:px-10 pt-6">
          <button onClick={() => navigate("/sales")} className="inline-flex items-center gap-1.5 text-caption text-muted-foreground hover:text-foreground transition-colors group">
        ...
        <div className="px-8 md:px-10 pt-5 pb-6 max-w-[960px]">
          ...
          <div className="flex items-center gap-2 flex-wrap mb-4">
            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-micro font-medium ${stageMeta.pill}`}>
              <div className={`w-1.5 h-1.5 rounded-full ${stageMeta.dot}`} />{ar ? stageMeta.ar : stageMeta.en}
            </div>
            <span className={`text-micro font-medium px-2.5 py-1 rounded-full ${priorityMeta.pill}`}>{ar ? priorityMeta.ar : priorityMeta.en}</span>
            <span className="text-micro font-medium px-2.5 py-1 rounded-full bg-primary/8 text-brand-ink border border-primary/20 tabular-nums">
              {deal.probability}%
            </span>
          </div>

          {/* Title */}
          <h1 className="text-display md:text-display font-medium text-foreground leading-tight mb-2" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.03em" }}>
            {ar ? deal.titleAr : deal.titleEn}
          </h1>

          {/* Value */}
          <p className="text-heading font-medium text-brand-ink mb-3 tabular-nums" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.02em" }}>
          ...
          <div className="flex items-center gap-2.5 mt-6 flex-wrap">
            <div className="relative">
              <button onClick={() => setStageMenuOpen(!stageMenuOpen)} className="h-8 px-3.5 rounded-xl border border-border text-caption font-medium text-foreground hover:bg-muted flex items-center gap-1.5 transition-colors">
            ...
            <button onClick={() => setNoteModalOpen(true)} className="h-8 px-3.5 rounded-xl border border-border text-caption font-medium text-foreground hover:bg-muted flex items-center gap-1.5 transition-colors">
      ...
      <div className="px-8 md:px-10 py-7 max-w-[960px]">
          ...
          <div className="space-y-6">
            <Section title={ar ? "الوصف" : "Description"}>
              <div className="px-6 py-5">
                <p className="text-body-lg text-foreground/85 leading-[1.8] max-w-[680px]" style={{ fontFamily: "var(--app-font-serif)" }}>
                ...
                <div className="divide-y divide-border/25">
                  ...
                  <DetailRow icon={User} label={ar ? "المسؤول" : "Owner"} value={
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-md bg-primary/10 flex items-center justify-center text-micro font-semibold text-brand-ink">{initials(ar ? deal.ownerAr : deal.ownerEn)}</div>
                      {ar ? deal.ownerAr : deal.ownerEn}
                    </div>
                  } />
                </div>
                ...
                return (
                  <div key={ev.id} className="flex gap-4">
                    ...
                    <div className={`flex-1 min-w-0 ${isLast ? "pb-4" : "pb-6"}`}>
                      ...
                      {ev.descEn && <p className="text-caption text-muted-foreground/80 mt-1 leading-relaxed">{ar ? ev.descAr : ev.descEn}</p>}
                      <p className="text-micro text-muted-foreground/60 mt-1">{ar ? ev.actorAr : ev.actorEn} · {ar ? ev.dateAr : ev.dateEn}</p>
                    </div>
                  </div>
                );
              })}
          ...
          <Section title={ar ? "الأشخاص" : "People"}>
            <div className="divide-y divide-border/25">
              <div className="flex items-center gap-3.5 px-6 py-4 hover:bg-muted/15 transition-colors">
                <div className="w-9 h-9 rounded-xl bg-primary/8 flex items-center justify-center text-micro font-semibold text-brand-ink shrink-0">{initials(ar ? deal.ownerAr : deal.ownerEn)}</div>
          ...
          <Section title={ar ? "المنظمة" : "Organization"}>
            {deal.orgNameEn ? (
              <div className="flex items-center gap-3.5 px-6 py-4">
                ...
                <div><p className="text-body font-medium text-foreground">{ar ? deal.orgNameAr : deal.orgNameEn}</p><p className="text-micro text-muted-foreground">{ar ? "العميل / المنظمة" : "Client / Organization"}</p></div>
              </div>
            ) : (
              <div className="px-6 py-14 text-center">
                ...
                <p className="text-body text-muted-foreground/60">{ar ? "لا توجد منظمة مرتبطة" : "No organization linked"}</p>
              </div>
            )}
          </Section>
        )}
        ...
        {activeTab === "notes" && (
          <Section title={ar ? "الملاحظات" : "Notes"} action={
            <button onClick={() => setNoteModalOpen(true)} className="flex items-center gap-1.5 text-micro font-medium text-brand-ink hover:text-brand-ink/80 transition-colors">
              ...
              {activity.map((ev, i) => {
                ...
                return (
                  <div key={ev.id} className="flex gap-4">
                    ...
                    <div className={`flex-1 min-w-0 ${isLast ? "pb-4" : "pb-6"}`}>
                      <p className="text-body font-medium text-foreground">{ar ? ev.titleAr : ev.titleEn}</p>
                      <p className="text-caption text-muted-foreground/70 mt-0.5 leading-relaxed">{ar ? ev.descAr : ev.descEn}</p>
          ...
          <Section title={ar ? "عمل مرتبط" : "Related Work"}>
            {relatedWork.length > 0 ? (
                    ...
                    <div key={w.id} onClick={() => navigate(`/work/${w.id}`)} className="flex items-center gap-4 px-6 py-4 hover:bg-muted/15 transition-colors cursor-pointer group">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <span className={`text-micro font-medium px-2 py-0.5 rounded-full shrink-0 ${wKind.pill}`}>{ar ? wKind.ar : wKind.en}</span>
              ...
              <div className="px-6 py-14 text-center">
                ...
                <p className="text-body text-muted-foreground/60">{ar ? "لا يوجد عمل مرتبط" : "No related work items"}</p>
                <button onClick={() => navigate("/work")} className="mt-3 text-caption text-brand-ink hover:underline">{ar ? "انتقل إلى العمل" : "Go to Work"}</button>
              </div>
            )}
          </Section>
        )}
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-px bg-border/20">
                {[
                  ...
                  <div key={i} className="bg-background px-6 py-6 flex flex-col gap-2">
                    <p className="text-display font-medium text-foreground leading-none tabular-nums" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.025em" }}>{s.value}</p>
                    <p className="text-micro text-muted-foreground">{s.label}</p>
                  </div>
                ))}
              </div>
