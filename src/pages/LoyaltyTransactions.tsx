    ...
    <div className="min-h-full py-8 px-7 md:px-10 max-w-[1100px] mx-auto">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="bg-background border border-border/40 rounded-xl px-4 py-3.5">
          <p className="text-micro text-muted-foreground mb-1">{ar ? "المعاملات" : "Transactions"}</p>
          <p className="text-heading font-medium text-foreground tabular-nums" style={{ fontFamily: "var(--app-font-serif)" }}>{filtered.length}</p>
        </div>
        <div className="bg-background border border-border/40 rounded-xl px-4 py-3.5">
          <p className="text-micro text-muted-foreground mb-1">{ar ? "نقاط صادرة" : "Points Issued"}</p>
          <p className="text-heading font-medium text-emerald-600 tabular-nums" style={{ fontFamily: "var(--app-font-serif)" }}>+{fmtPts(earnedTotal)}</p>
        </div>
        <div className="bg-background border border-border/40 rounded-xl px-4 py-3.5">
          <p className="text-micro text-muted-foreground mb-1">{ar ? "نقاط مخصومة" : "Points Deducted"}</p>
          <p className="text-heading font-medium text-foreground tabular-nums" style={{ fontFamily: "var(--app-font-serif)" }}>-{fmtPts(deductedTotal)}</p>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="border border-border/40 rounded-xl py-16 text-center bg-background">
          <p className="text-body font-medium text-foreground">{ar ? "لا توجد معاملات" : "No transactions found"}</p>
          <p className="text-caption text-muted-foreground mt-1">{ar ? "جرب تغيير الفلاتر" : "Try adjusting filters"}</p>
        </div>
      ) : (
          ...
          {filtered.map(tx => {
            ...
            return (
              <div key={tx.id} className="flex items-center gap-4 px-5 py-3.5">
                ...
                {member && (
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center text-micro font-semibold text-white shrink-0" style={{ backgroundColor: member.avatarColor }}>
                    {initials(ar ? member.nameAr : member.nameEn)}
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-body font-medium text-foreground truncate">{ar ? (member?.nameAr || "—") : (member?.nameEn || "—")}</span>
                    <span className="text-micro text-muted-foreground">{ar ? meta.ar : meta.en}</span>
                    {tx.orderId && <span className="text-micro text-muted-foreground font-mono hidden sm:inline">{tx.orderId}</span>}
                  </div>
                ...
                <span className="text-micro text-muted-foreground tabular-nums shrink-0 hidden md:block w-[100px] text-end">
