        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="px-6 py-5 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-micro font-medium text-muted-foreground mb-1.5">{ar ? "المرحلة" : "Stage"}</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-micro font-medium text-muted-foreground mb-1.5">{ar ? "جهة الاتصال" : "Contact"}</label>
              ...
              <div>
                <label className="block text-micro font-medium text-muted-foreground mb-1.5">{ar ? "المنظمة" : "Organization"}</label>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 mb-2.5">
        <span className="text-body-lg font-semibold text-foreground tabular-nums" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.02em" }}>
      ...
      {/* Title */}
      <h4
        className="text-body font-medium text-foreground leading-snug mb-2.5 line-clamp-2 group-hover:text-brand-ink transition-colors"
      ...
      {deal.org_name_en && (
        <div className="flex items-center gap-1.5 mb-2 text-muted-foreground/60">
          ...
          <span className="text-micro truncate">{ar ? (deal.org_name_ar || deal.org_name_en) : deal.org_name_en}</span>
        </div>
      )}
      ...
      <div className="flex items-center justify-between gap-2 mb-2.5">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span className="text-micro text-muted-foreground truncate">{ar ? ((m.ownerAr as string) || "") : ((m.ownerEn as string) || "")}</span>
        </div>
      ...
      <div className="flex items-center justify-between pt-2.5 border-t border-border/30">
        <span className="text-micro text-muted-foreground tabular-nums">{deal.probability}%</span>
        {deal.expected_close_date && (
          <div className="flex items-center gap-1 text-muted-foreground/50">
            <Calendar size={9} strokeWidth={1.75} />
            <span className="text-micro">{ar ? ((m.expectedCloseDateAr as string) || deal.expected_close_date) : ((m.expectedCloseDateEn as string) || deal.expected_close_date)}</span>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="flex gap-4 overflow-x-auto pb-4 -mx-2 px-2" style={{ minHeight: "400px" }}>
      {STAGE_ORDER.map((stage) => {
        ...
        return (
          <div
            key={stage}
            className={`flex flex-col min-w-[260px] w-[260px] shrink-0 rounded-2xl ${COL_COLORS[stage]} ${isDragOver ? "ring-2 ring-brand-ink/30 scale-[1.01]" : ""} transition-all duration-200`}
            ...
            <div className="flex-1 px-3 pb-3 space-y-2.5 overflow-y-auto" style={{ maxHeight: "calc(100vh - 420px)" }}>
              ...
              {colDeals.length === 0 && (
                <div className="py-8 text-center">
                  <p className="text-micro text-muted-foreground/40">{ar ? "لا توجد صفقات" : "No deals"}</p>
                </div>
              )}
            </div>
          </div>
                ...
                return (
                  <div key={deal.id} onClick={() => onNavigate(deal.id)}
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="text-body font-medium text-foreground truncate group-hover:text-brand-ink transition-colors mb-0.5" style={{ letterSpacing: "-0.01em" }}>
                        {ar ? (deal.title_ar || deal.title_en) : deal.title_en}
                      </h4>
                    ...
                    <div className="flex items-center gap-5 shrink-0">
                      <span className="text-body font-semibold text-foreground tabular-nums hidden sm:block" style={{ fontFamily: "var(--app-font-serif)" }}>
                        {fmtVal(deal.value, deal.currency)}
                      </span>
                      <span className="text-micro text-muted-foreground tabular-nums hidden md:block">{deal.probability}%</span>
      ...
      <div className="min-h-full py-8 px-7 md:px-10 max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-7">
          {[
            ...
            <div key={i} className="bg-background border border-border/40 rounded-xl px-4 py-3.5">
              ...
              <p className="text-title md:text-heading font-medium text-foreground leading-none tabular-nums" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.02em" }}>
                {m.value}
              </p>
            </div>
          ))}
        ...
        <div className="flex flex-wrap items-center gap-2.5 mb-5">
          ...
          <div className="flex items-center border border-border rounded-xl overflow-hidden bg-card">
            <button onClick={() => setView("pipeline")}
              className={`w-8 h-8 flex items-center justify-center transition-colors ${view === "pipeline" ? "bg-primary/10 text-brand-ink" : "text-muted-foreground hover:text-foreground"}`}>
        ...
        {filtered.length === 0 ? (
          <div className="border border-border/40 rounded-xl py-20 text-center bg-background">
            ...
            <p className="text-body font-medium text-foreground mb-1">{ar ? "لا توجد نتائج" : "No deals found"}</p>
