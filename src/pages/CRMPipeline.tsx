...
const STAGES = [
  ...
  { id: "qualified", en: "Qualified", ar: "مؤهل", color: "bg-chart-4", bgColor: "bg-chart-4/10" },
  { id: "meeting", en: "Meeting", ar: "اجتماع", color: "bg-warning", bgColor: "bg-warning/10" },
  ...
  return (
    <div className="min-h-full px-6 md:px-8 py-6">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <button className="h-9 px-4 rounded-xl bg-primary text-primary-foreground text-caption font-medium flex items-center gap-1.5 hover:opacity-90 transition-opacity">
          ...
          <div key={stage.id} className="w-[280px] shrink-0"
            ...
            <div className="flex items-center justify-between mb-2.5 px-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-micro text-muted-foreground bg-muted px-1.5 py-0.5 rounded-full">{stage.leads.length}</span>
              </div>
              <span className="text-micro text-muted-foreground font-medium">{formatEGP(stage.totalValue)}</span>
            </div>
            ...
            <div className={`space-y-2 min-h-[200px] p-2 rounded-xl ${stage.bgColor} border border-transparent transition-colors ${draggedLead ? "border-dashed border-border/60" : ""}`}>
                ...
                <motion.div key={lead.id} layout
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-caption font-medium text-foreground">{lead.customer_name}</span>
                    </div>
                    <span className="text-micro font-semibold text-foreground">{formatEGP(lead.value)}</span>
                  </div>
                  <p className="text-micro text-muted-foreground mb-2 pl-5">{ar ? lead.title_ar : lead.title}</p>
                  ...
                  <div className="flex items-center justify-between pl-5 pt-2 border-t border-border/30">
                    <div className="flex items-center gap-1.5">
                      <div className="w-5 h-5 rounded-full bg-primary/10 flex items-center justify-center text-micro font-bold text-brand-ink">
                        {lead.assigned_to.charAt(0)}
                      </div>
                      <span className="text-micro text-muted-foreground">{lead.assigned_to}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="text-micro text-muted-foreground">{lead.probability}%</span>
                  ...
                  {lead.next_followup && (
                    <div className="flex items-center gap-1 mt-2 pl-5">
                      ...
                      <span className="text-micro text-muted-foreground">{ar ? "متابعة" : "Follow-up"}: {lead.next_followup}</span>
                    </div>
                  )}
                </motion.div>
              ))}
              {stage.leads.length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-micro text-muted-foreground/50 mb-2">{ar ? "لا توجد صفقات" : "No deals yet"}</p>
                  <button className="text-micro text-brand-ink hover:underline flex items-center gap-1 mx-auto">
          ...
          <div className="relative w-full max-w-lg bg-background rounded-2xl border border-border shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-brand-ink text-micro font-bold">
                  {selectedLead.customer_name.split(" ").map(w => w[0]).join("")}
                </div>
                <div>
                  <p className="text-body font-medium">{selectedLead.customer_name}</p>
                  <p className="text-micro text-muted-foreground">{selectedLead.customer_name_ar}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-xl bg-muted/30">
                  <p className="text-micro text-muted-foreground uppercase tracking-wider mb-1">{ar ? "القيمة" : "Value"}</p>
                  <p className="text-title font-bold text-foreground" style={{ fontFamily: "var(--app-font-serif)" }}>{formatEGP(selectedLead.value)}</p>
                </div>
                <div className="p-3 rounded-xl bg-muted/30">
                  <p className="text-micro text-muted-foreground uppercase tracking-wider mb-1">{ar ? "المرحلة" : "Stage"}</p>
                  <p className="text-body font-medium capitalize">{ar ? STAGES.find(s => s.id === selectedLead.stage)?.ar : selectedLead.stage}</p>
                </div>
              </div>
              ...
              <div className="space-y-2.5">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between py-1.5 border-b border-border/20 last:border-0">
                    ...
                    <span className="text-micro font-medium text-foreground">{row.value}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {selectedLead.tags.map(tag => (
                  <span key={tag} className="text-micro px-2 py-0.5 rounded-full bg-primary/10 text-brand-ink font-medium">{tag}</span>
                ))}
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-micro text-muted-foreground">{ar ? "احتمال الإغلاق" : "Close Probability"}</span>
                  <span className="text-micro font-medium">{selectedLead.probability}%</span>
                </div>
              ...
              <div className="p-3 rounded-xl bg-primary/5 border border-primary/10">
                <p className="text-micro text-muted-foreground mb-1">{ar ? "القيمة المتوقعة" : "Expected Value"}</p>
                <p className="text-title font-bold text-brand-ink" style={{ fontFamily: "var(--app-font-serif)" }}>
