...
function DetailRow({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-center gap-3.5 px-6 py-3.5">
      ...
      <div className="flex-1 min-w-0"><p className="text-micro text-muted-foreground/70 mb-0.5">{label}</p><div className="text-body text-foreground">{value}</div></div>
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" aria-modal="true">
      ...
      <div className="relative bg-background border border-border/60 rounded-2xl shadow-xl w-full max-w-[480px] overflow-hidden">
        ...
        <div className="px-6 py-3 border-t border-border/40 flex justify-end gap-3">
          ...
          <button onClick={() => { if (text.trim()) { onAdd(text.trim()); setText(""); onClose(); } }} disabled={!text.trim()} className="h-8 px-4 rounded-xl bg-primary text-primary-foreground text-caption font-medium hover:opacity-90 transition-opacity disabled:opacity-40">{ar ? "إضافة" : "Add"}</button>
        </div>
      </div>
    </div>
  );
...
export default function FinanceDetail() {
  ...
  if (!inv) {
    return (
      <div className="min-h-full flex flex-col items-center justify-center gap-4 p-8">
        ...
        <p className="text-body-lg font-medium text-foreground" style={{ fontFamily: "var(--app-font-serif)" }}>{ar ? "لم يُعثر على الفاتورة" : "Invoice not found"}</p>
        <button onClick={() => navigate("/finance")} className="flex items-center gap-1.5 text-caption text-brand-ink hover:underline"><ArrowLeft size={12} strokeWidth={2} />{ar ? "العودة" : "Back"}</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-full">
      {actionToast && (
        <div className="fixed top-4 start-1/2 -translate-x-1/2 z-50 px-5 py-2.5 rounded-xl bg-foreground text-background text-body font-medium shadow-lg flex items-center gap-2 animate-in fade-in slide-in-from-top-2 duration-200"><Check size={14} strokeWidth={2.5} />{actionToast}</div>
      )}
      ...
      <div className="relative border-b border-border/40" style={{ background: "linear-gradient(160deg, hsl(var(--muted)/0.35) 0%, hsl(var(--background)) 65%)" }}>
        <div className="px-8 md:px-10 pt-6">
          <button onClick={() => navigate("/finance")} className="inline-flex items-center gap-1.5 text-caption text-muted-foreground hover:text-foreground transition-colors group">
        ...
        <div className="px-8 md:px-10 pt-5 pb-6 max-w-[960px]">
          <div className="flex items-center gap-2.5 flex-wrap mb-4">
            ...
            <span className="text-micro font-mono text-muted-foreground px-2.5 py-1 rounded-full bg-muted border border-border/40">{inv.number}</span>
          </div>

          <h1 className="text-display md:text-display font-medium text-foreground leading-tight mb-2" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.03em" }}>{ar ? inv.titleAr : inv.titleEn}</h1>
          <p className="text-display font-medium text-brand-ink mb-3 tabular-nums" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.02em" }}>{fmt(inv.amount)}</p>
          ...
          {inv.status !== "cancelled" && (
            <div className="mt-5 flex items-center gap-4">
              ...
              <span className="text-body font-medium text-foreground tabular-nums" style={{ fontFamily: "var(--app-font-serif)" }}>{Math.round(inv.paidAmount / inv.amount * 100)}%</span>
            </div>
          )}
                ...
                <>
                  ...
                  <div className="absolute top-full mt-1.5 start-0 z-50 bg-background border border-border/60 rounded-xl shadow-lg py-1.5 min-w-[180px]">
                    {(["draft", "sent", "paid", "overdue", "cancelled"] as InvoiceStatus[]).map((s) => {
                      ...
                      return (<button key={s} onClick={() => changeStatus(s)} disabled={isCurrent} className={`w-full flex items-center gap-2.5 px-4 py-2 text-caption text-start transition-colors ${isCurrent ? "text-brand-ink bg-primary/5 font-medium" : "text-foreground hover:bg-muted"}`}><div className={`w-2 h-2 rounded-full ${m.dot}`} />{ar ? m.ar : m.en}{isCurrent && <Check size={12} strokeWidth={2.5} className="ms-auto text-brand-ink" />}</button>);
                    })}
                  </div>
                </>
              )}
      ...
      <div className="border-b border-border/40 bg-background sticky top-0 z-10">
        <div className="px-8 md:px-10 flex items-center gap-0 overflow-x-auto">
          {TABS.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-4 py-3 text-caption font-medium whitespace-nowrap border-b-2 transition-all ${activeTab === tab.id ? "border-primary text-brand-ink" : "border-transparent text-muted-foreground hover:text-foreground hover:border-border/60"}`}>{ar ? tab.ar : tab.en}</button>
          ))}
        </div>
      </div>
      ...
      <div className="px-8 md:px-10 py-7 max-w-[960px]">
        ...
        {activeTab === "overview" && (
          <div className="space-y-6">
            {inv.noteEn && <Section title={ar ? "ملاحظة" : "Note"}><div className="px-6 py-5"><p className="text-body-lg text-foreground/85 leading-[1.8] max-w-[680px]" style={{ fontFamily: "var(--app-font-serif)" }}>{ar ? inv.noteAr : inv.noteEn}</p></div></Section>}
            ...
            <Section title={ar ? "التفاصيل" : "Details"}>
              <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-border/25">
                <div className="divide-y divide-border/25">
                  ...
                  <DetailRow icon={AlertTriangle} label={ar ? "المتبقي" : "Balance"} value={<span className={`font-semibold tabular-nums ${balance > 0 ? "text-warning" : "text-emerald-600"}`} style={{ fontFamily: "var(--app-font-serif)" }}>{fmt(balance)}</span>} />
                </div>
                <div className="divide-y divide-border/25">
                  <DetailRow icon={Building2} label={ar ? "المنظمة" : "Organization"} value={ar ? inv.orgNameAr : inv.orgNameEn} />
                  <DetailRow icon={User} label={ar ? "جهة الاتصال" : "Contact"} value={<div className="flex items-center gap-2"><div className="w-5 h-5 rounded-md bg-primary/10 flex items-center justify-center text-micro font-semibold text-brand-ink">{initials(ar ? inv.contactNameAr : inv.contactNameEn)}</div>{ar ? inv.contactNameAr : inv.contactNameEn}</div>} />
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2.5 mb-2"><Sparkles size={14} strokeWidth={1.75} className="text-brand-ink" /><span className="text-body font-medium text-foreground">{ar ? "رؤى مالية" : "Finance Insights"}</span><span className="text-micro text-muted-foreground/50 px-2 py-0.5 rounded-full bg-muted border border-border/40">{ar ? "ذكاء اصطناعي" : "AI"}</span></div>
              {insights.map((card, i) => (
        ...
        {activeTab === "organization" && (
          <Section title={ar ? "المنظمة" : "Organization"}>
            <div className="flex items-center gap-3.5 px-6 py-4">
              ...
              <div><p className="text-body font-medium text-foreground">{ar ? inv.orgNameAr : inv.orgNameEn}</p><p className="text-micro text-muted-foreground">{ar ? "العميل" : "Client"}</p></div>
            </div>
          </Section>
        )}
        ...
        {activeTab === "contact" && (
          <Section title={ar ? "جهة الاتصال" : "Contact"}>
            <div className="flex items-center gap-3.5 px-6 py-4">
              <div className="w-9 h-9 rounded-xl bg-primary/8 flex items-center justify-center text-micro font-semibold text-brand-ink shrink-0">{initials(ar ? inv.contactNameAr : inv.contactNameEn)}</div>
              <div><p className="text-body font-medium text-foreground">{ar ? inv.contactNameAr : inv.contactNameEn}</p><p className="text-micro text-muted-foreground">{ar ? "جهة اتصال الفاتورة" : "Invoice Contact"}</p></div>
            </div>
          </Section>
        )}
        ...
        {activeTab === "payments" && (
          <Section title={ar ? "المدفوعات" : "Payments"}>
            {invPayments.length > 0 ? (
                ...
                {invPayments.map((p) => {
                  ...
                  return (
                    <div key={p.id} className="flex items-center gap-4 px-6 py-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-body font-medium text-foreground">{fmt(p.amount)}</p>
                        <p className="text-micro text-muted-foreground">{ar ? mm.ar : mm.en}{p.referenceEn ? ` · ${p.referenceEn}` : ""} · {ar ? p.dateAr : p.dateEn}</p>
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="px-6 py-14 text-center"><p className="text-body text-muted-foreground/60">{ar ? "لا مدفوعات بعد" : "No payments recorded"}</p></div>
            )}
          </Section>
        )}
              ...
              {allNotes.map((n) => (
                <div key={n.id} className="px-6 py-4">
                  <div className="flex items-center gap-2.5 mb-2.5">
                    <div className="w-6 h-6 rounded-md bg-primary/8 flex items-center justify-center text-micro font-semibold text-brand-ink">{initials(ar ? n.authorAr : n.authorEn)}</div>
                    <span className="text-caption font-medium text-foreground">{ar ? n.authorAr : n.authorEn}</span>
                    <span className="text-micro text-muted-foreground/50 ms-auto">{ar ? n.dateAr : n.dateEn}</span>
                  </div>
                  <p className="text-body text-foreground/80 leading-[1.7]" style={{ paddingInlineStart: "2.125rem" }}>{ar ? n.contentAr : n.contentEn}</p>
                </div>
              ))}
            </div>
          </Section>
