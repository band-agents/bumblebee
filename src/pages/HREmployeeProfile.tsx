...
const STATUS_META: Record<string, { en: string; ar: string; pill: string }> = {
  ...
  on_leave: { en: "On Leave", ar: "إجازة", pill: "bg-warning/15 text-warning" },
...
const TIMELINE_ICONS: Record<string, { icon: React.ElementType; color: string; bg: string }> = {
  ...
  bonus_added: { icon: Gift, color: "text-brand-ink", bg: "bg-primary/10" },
  ...
  attendance_issue: { icon: AlertTriangle, color: "text-warning", bg: "bg-warning/15" },
...
export default function HREmployeeProfile() {
  ...
  if (!employee) {
    return (
      <div className="min-h-full flex items-center justify-center">
        <p className="text-body text-muted-foreground">{ar ? "الموظف غير موجود" : "Employee not found"}</p>
      </div>
    );
  }
  ...
  const aiSummary = `${employee.full_name} has been with Bumblebee since ${new Date(employee.hire_date).getFullYear()}. ${employee.status === "active" ? "Currently active" : employee.status}. Performance score: ${employee.performance_score || "N/A"}/100. Attendance risk: ${employee.attendance_risk}.`;
    ...
    <div className="min-h-full">
      ...
      <div className="px-6 md:px-8 py-5 border-b border-border/40" style={{ background: "linear-gradient(160deg, hsl(var(--muted)/0.2) 0%, hsl(var(--background)) 60%)" }}>
        <div className="flex items-center gap-4">
          ...
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-title font-bold text-white shrink-0" style={{ backgroundColor: employee.avatar_color }}>
            {employee.full_name.split(" ").map(w => w[0]).join("")}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <h1 className="text-title font-semibold" style={{ fontFamily: "var(--app-font-serif)" }}>
                {ar && employee.full_name_ar ? employee.full_name_ar : employee.full_name}
              </h1>
              <span className={`text-micro px-2 py-0.5 rounded-full font-semibold ${st.pill}`}>{ar ? st.ar : st.en}</span>
            </div>
            <div className="flex items-center gap-3 text-micro text-muted-foreground mt-1">
              ...
              <span className="font-mono text-micro">{employee.employee_number}</span>
            </div>
          </div>
      ...
      <div className="px-6 md:px-8 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6 max-w-[1200px]">
                      ...
                      <div key={event.id} className="flex gap-3 relative">
                        ...
                        <div className="pb-5 flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-0.5">
                            ...
                            <span className="text-micro text-muted-foreground shrink-0">{new Date(event.timestamp).toLocaleDateString(ar ? "ar-EG" : "en-US", { month: "short", day: "numeric", year: "numeric" })}</span>
                          </div>
                          <p className="text-micro text-muted-foreground">{ar ? event.details_ar : event.details}</p>
                          <span className="text-micro text-muted-foreground/60 mt-1 block">{event.staff}</span>
                        </div>
                      </div>
                    );
                  })
              ...
              <div className="space-y-2">
                {attendance.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-caption text-muted-foreground">{ar ? "لا توجد سجلات حضور" : "No attendance records"}</p>
                  </div>
                ) : (
                  attendance.slice(0, 30).map(a => {
                    const statusMeta: Record<string, { en: string; ar: string; pill: string }> = {
                      ...
                      late: { en: "Late", ar: "متأخر", pill: "bg-warning/15 text-warning" },
              ...
              <div className="space-y-2">
                {leaves.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-caption text-muted-foreground">{ar ? "لا توجد طلبات إجازات" : "No leave requests"}</p>
                  </div>
                ) : (
                  leaves.map(l => {
                    ...
                    const statusMeta: Record<string, { en: string; ar: string; pill: string }> = {
                      pending: { en: "Pending", ar: "معلق", pill: "bg-warning/15 text-warning" },
                    ...
                    return (
                      <div key={l.id} className="flex items-center gap-3 p-3 rounded-xl border border-border/30">
                        ...
                        <span className="text-micro bg-muted px-2 py-0.5 rounded-full text-muted-foreground">{ar ? lt.ar : lt.en}</span>
                        <span className="text-caption flex-1">{l.start_date} → {l.end_date}</span>
              ...
              <div className="space-y-2">
                {payroll.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-caption text-muted-foreground">{ar ? "لا توجد سجلات رواتب" : "No payroll records"}</p>
                  </div>
                ) : (
                  payroll.map(p => {
                    const statusMeta: Record<string, { en: string; ar: string; pill: string }> = {
                    ...
                    return (
                      <div key={p.id} className="p-4 rounded-xl border border-border/30">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                            <span className="text-caption font-medium">{p.period}</span>
                            <span className={`text-micro px-2 py-0.5 rounded-full font-medium ${pst.pill}`}>{ar ? pst.ar : pst.en}</span>
                          </div>
                          <span className="text-body-lg font-bold text-foreground" style={{ fontFamily: "var(--app-font-serif)" }}>{formatEGP(p.net_salary)}</span>
                        </div>
              ...
              <div className="space-y-4">
                {reviews.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-caption text-muted-foreground">{ar ? "لا توجد مراجعات أداء" : "No performance reviews"}</p>
                  </div>
                ) : (
                  reviews.map(r => (
                    <div key={r.id} className="p-5 rounded-xl border border-border/30">
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          <span className="text-caption font-medium">{r.review_period}</span>
                      ...
                      <div className="grid grid-cols-2 gap-3 text-micro">
                        ...
                        <div className="p-3 rounded-lg bg-warning/10">
                          <p className="font-medium text-warning mb-1">{ar ? "مجالات التحسين" : "Improvements"}</p>
                          <p className="text-muted-foreground">{r.improvements}</p>
                        </div>
                      </div>
                      <div className="mt-3 p-3 rounded-lg bg-primary/5">
                        <p className="font-medium text-brand-ink text-micro mb-1">{ar ? "الأهداف القادمة" : "Next Goals"}</p>
                        <p className="text-micro text-muted-foreground">{r.goals}</p>
                      </div>
                    </div>
                  ))
                )}
              ...
              <div className="space-y-2">
                {employee.documents.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-caption text-muted-foreground">{ar ? "لا توجد مستندات" : "No documents"}</p>
                  </div>
                ) : (
                  employee.documents.map((doc, i) => (
                    <div key={i} className="flex items-center gap-3 p-3 rounded-xl border border-border/30 hover:bg-muted/20 transition-colors cursor-pointer">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-caption font-medium">{doc.name}</p>
                        <p className="text-micro text-muted-foreground">{doc.type} · {doc.uploaded_at}</p>
                      </div>
              ...
              <div className="space-y-2">
                {employee.equipment.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-caption text-muted-foreground">{ar ? "لا توجد أصول مخصصة" : "No assets assigned"}</p>
                  </div>
                ) : (
                  employee.equipment.map((eq, i) => (
                    <div key={i} className="flex items-center gap-3 p-3 rounded-xl border border-border/30">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-caption font-medium">{eq.item}</p>
                        <p className="text-micro text-muted-foreground">Assigned: {eq.assigned_at}</p>
                      </div>
                      <span className={`text-micro px-2 py-0.5 rounded-full font-medium ${eq.status === "assigned" ? "bg-emerald-100 text-emerald-700" : "bg-zinc-100 text-zinc-500"}`}>
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-xl border border-border/40 space-y-3">
              ...
              {[
                ...
                <div key={i} className="flex items-center justify-between">
                  ...
                  <span className="text-micro font-medium text-foreground text-right max-w-[140px] truncate">{m.value}</span>
                </div>
              ))}
            </div>
            ...
            {/* Risk & Performance */}
            <div className="p-4 rounded-xl border border-border/40 space-y-3">
              <h4 className="text-micro font-semibold text-muted-foreground uppercase tracking-wider">{ar ? "تحليل الأداء" : "Performance"}</h4>
              <div className="flex items-center justify-between">
                <span className="text-micro text-muted-foreground">{ar ? "خطر الحضور" : "Attendance Risk"}</span>
                <span className={`text-micro px-2 py-0.5 rounded-full font-medium ${employee.attendance_risk === "high" ? "bg-rose-100 text-rose-600" : employee.attendance_risk === "medium" ? "bg-warning/15 text-warning" : "bg-emerald-100 text-emerald-700"}`}>
              ...
              {employee.performance_score && (
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-micro text-muted-foreground">{ar ? "درجة الأداء" : "Performance Score"}</span>
              ...
              {employee.contract_end_date && (
                <div className="flex items-center justify-between">
                  <span className="text-micro text-muted-foreground">{ar ? "انتهاء العقد" : "Contract Expires"}</span>
                  <span className="text-micro font-medium text-warning">{employee.contract_end_date}</span>
                </div>
              )}
              {employee.probation_end_date && (
                <div className="flex items-center justify-between">
                  <span className="text-micro text-muted-foreground">{ar ? "انتهاء التجربة" : "Probation Ends"}</span>
                  <span className="text-micro font-medium text-blue-600">{employee.probation_end_date}</span>
                </div>
              )}
            </div>
