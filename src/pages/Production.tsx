...
function ProgressBar({ pct, size = "md" }: { pct: number; size?: "sm" | "md" | "lg" }) {
  ...
  const color = pct >= 100 ? "bg-emerald-500" : pct >= 70 ? "bg-blue-500" : pct >= 40 ? "bg-warning" : "bg-rose-500";
  return (
...
function Badge({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return <span className={`text-micro px-2 py-0.5 rounded-full font-medium inline-flex items-center ${className}`}>{children}</span>;
}
...
function StatCard({ label, value, sub, icon: Icon, color = "text-foreground" }: {
  ...
  return (
    <div className="bg-background border border-border/40 rounded-xl p-4 hover:shadow-sm transition-shadow">
      ...
      <p className={`text-heading font-semibold tabular-nums ${color}`} style={{ fontFamily: "var(--app-font-serif)" }}>{value}</p>
...
function DashboardView({ ar, onSelectOrder }: { ar: boolean; onSelectOrder: (id: string) => void }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-body-lg font-semibold" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "Active Production", "الإنتاج النشط")}</h3>
            <span className="text-micro text-muted-foreground">{activeOrders.length} {t(ar, "orders", "أوامر")}</span>
          </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <span className="text-micro font-mono text-muted-foreground">{order.order_number}</span>
                    ...
                    <p className="text-body font-medium" style={{ fontFamily: "var(--app-font-serif)" }}>{order.product_name}</p>
                  ...
                  <div className="shrink-0 flex flex-col items-end gap-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-micro tabular-nums font-medium w-8 text-right">{order.progress_pct}%</span>
                    </div>
                    <span className="text-micro text-muted-foreground">ETA: {order.estimated_completion}</span>
                  </div>
                </div>
              </button>
            ))}
            {activeOrders.length === 0 && (
              <div className="py-8 text-center text-caption text-muted-foreground/50">{t(ar, "No active orders", "لا أوامر نشطة")}</div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-body font-semibold" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "Alerts", "التنبيهات")}</h3>
            ...
            <div className="space-y-2">
              {recentAlerts.map(alert => (
                <div key={alert.id} className={`p-3 rounded-lg border text-micro ${ALERT_SEVERITY[alert.severity]}`}>
                  <div className="flex items-start gap-2">
                    <AlertTriangle size={12} className="shrink-0 mt-0.5" />
                    <div>
                      <span className="font-medium">{alert.order_number}</span>
                      <p className="mt-0.5 text-micro opacity-80">{ar ? alert.message_ar : alert.message_en}</p>
                    </div>
                  </div>
                </div>
              ))}
          ...
          {/* Workstations */}
          <div>
            <h3 className="text-body font-semibold mb-3" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "Workstations", "محطات العمل")}</h3>
            <div className="space-y-1.5">
              {workstations.map(ws => (
                <div key={ws.id} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-muted/20 border border-border/20">
                  <div className={`w-2 h-2 rounded-full shrink-0 ${ws.status === "active" ? "bg-emerald-500" : ws.status === "maintenance" ? "bg-warning" : ws.status === "down" ? "bg-rose-500" : "bg-zinc-300"}`} />
                  <div className="flex-1 min-w-0">
                    <p className="text-micro font-medium truncate">{ws.name}</p>
                    <p className="text-micro text-muted-foreground">{ws.operator || t(ar, "No operator", "بلا مشغل")}</p>
                  </div>
                  <span className="text-micro text-muted-foreground">{ws.capacity}%</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div>
        <h3 className="text-body-lg font-semibold mb-3" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "Production Rate Overview", "نظرة عامة على معدل الإنتاج")}</h3>
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={i} className="bg-background border border-border/40 rounded-xl p-4">
              ...
              <p className="text-title font-semibold tabular-nums" style={{ fontFamily: "var(--app-font-serif)" }}>{s.value}</p>
              <p className="text-micro text-muted-foreground mt-0.5">{s.sub}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input className="w-full h-9 px-3 pl-9 rounded-xl border border-border/60 bg-background text-caption focus:outline-none focus:ring-2 focus:ring-brand-ink/20"
        ...
        <div className="flex gap-1 flex-wrap">
          {LIST_FILTERS.map(f => (
            <button key={f.id} onClick={() => setFilter(f.id)}
              className={`px-3 py-1.5 rounded-lg text-micro font-medium transition-colors ${filter === f.id ? "bg-primary/10 text-brand-ink" : "text-muted-foreground hover:bg-muted/50"}`}>
              {ar ? f.ar : f.en}
            </button>
          ))}
        </div>
      ...
      <div className="bg-background border border-border/40 rounded-xl overflow-hidden">
        {/* Header */}
        <div className="grid grid-cols-[120px_1fr_80px_100px_80px_80px_90px_80px_80px] gap-2 px-4 py-2.5 bg-muted/30 text-micro text-muted-foreground font-medium border-b border-border/30">
        ...
        <div className="divide-y divide-border/20">
          {filtered.map(order => (
            <button key={order.id} onClick={() => onSelectOrder(order.id)}
              ...
              <div className="min-w-0">
                <p className="text-caption font-medium truncate">{order.product_name}</p>
                <p className="text-micro text-muted-foreground truncate">{order.customer_name}</p>
              </div>
              ...
              <span className={`text-micro px-2 py-0.5 rounded-full font-medium ${STAGE_COLORS[order.current_stage] || "bg-zinc-100 text-zinc-600"}`}>
                {order.current_stage_en}
              </span>
              <span className="text-micro tabular-nums flex items-center gap-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-micro tabular-nums">{order.progress_pct}%</span>
              </div>
          ...
          {filtered.length === 0 && (
            <div className="py-12 text-center text-caption text-muted-foreground/50">{t(ar, "No orders found", "لا توجد أوامر")}</div>
          )}
        </div>
      </div>
    </div>
        ...
        <div key={key} className="min-w-[260px] flex-1">
          <div className="flex items-center justify-between mb-3 px-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-caption font-medium">{ar ? stageLabels[key]?.ar : stageLabels[key]?.en}</span>
              <span className="text-micro text-muted-foreground bg-muted px-1.5 py-0.5 rounded-full">{grouped[key].length}</span>
            </div>
          </div>
          <div className="space-y-2">
            {grouped[key].map(order => (
              <button key={order.id} onClick={() => onSelectOrder(order.id)}
                ...
                <p className="text-caption font-medium mb-1.5" style={{ fontFamily: "var(--app-font-serif)" }}>{order.product_name}</p>
                ...
                <div className="flex items-center justify-between mt-2 text-micro">
            ...
            {grouped[key].length === 0 && (
              <div className="py-8 text-center text-micro text-muted-foreground/30 border border-dashed border-border/30 rounded-xl">
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            <Badge className={PRIORITY_COLORS[order.priority]}>{t(ar, order.priority, order.priority)}</Badge>
            {order.is_delayed && <Badge className="bg-rose-100 text-rose-600">{t(ar, `${order.delay_days}d late`, `تأخر ${order.delay_days} يوم`)}</Badge>}
            <Badge className={order.material_status === "shortage" ? "bg-rose-100 text-rose-600" : order.material_status === "partial" ? "bg-warning/15 text-warning" : "bg-emerald-100 text-warning"}>
          ...
          <h2 className="text-title font-semibold" style={{ fontFamily: "var(--app-font-serif)" }}>{order.product_name}</h2>
          <div className="flex items-center gap-3 mt-1 text-micro text-muted-foreground flex-wrap">
        ...
        <div className="shrink-0 text-right">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-body font-semibold tabular-nums">{order.progress_pct}%</span>
          </div>
          <p className="text-micro text-muted-foreground">{t(ar, "المعدل:", "Rate:")} {order.production_rate_per_hour}/hr ({order.efficiency_pct}% {t(ar, "كفاءة", "eff")})</p>
        </div>
      </div>
      ...
      <div className="flex gap-1 border-b border-border/30 pb-2 overflow-x-auto">
        {detailTabs.map(t2 => (
          <button key={t2.id} onClick={() => setTab(t2.id)}
            className={`px-3 py-2 rounded-lg text-micro font-medium transition-colors flex items-center gap-1.5 whitespace-nowrap ${tab === t2.id ? "bg-primary/10 text-brand-ink" : "text-muted-foreground hover:bg-muted/50"}`}>
      ...
      {tab === "overview" && (
        <div className="space-y-5">
          ...
          <div className="bg-background border border-border/40 rounded-xl p-5">
            <h3 className="text-body font-semibold mb-3" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "Quantity Summary", "ملخص الكميات")}</h3>
            <div className="grid grid-cols-4 sm:grid-cols-8 gap-3">
              {[
                ...
                { label: t(ar, "Remaining", "المتبقي"), value: fmt(order.remaining_qty), color: "text-warning" },
                ...
                { label: t(ar, "Efficiency", "الكفاءة"), value: `${order.efficiency_pct}%`, color: order.efficiency_pct >= 85 ? "text-emerald-600" : "text-warning" },
              ].map((s, i) => (
                <div key={i} className="text-center">
                  <p className="text-micro text-muted-foreground mb-1">{s.label}</p>
                  <p className={`text-title font-semibold tabular-nums ${s.color}`} style={{ fontFamily: "var(--app-font-serif)" }}>{s.value}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-background border border-border/40 rounded-xl p-5">
            <h3 className="text-body font-semibold mb-3" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "Production Rate", "معدل الإنتاج")}</h3>
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
              {[
                ...
                <div key={i}>
                  <p className="text-micro text-muted-foreground mb-1">{s.label}</p>
                  <p className="text-body-lg font-semibold tabular-nums" style={{ fontFamily: "var(--app-font-serif)" }}>{s.value}</p>
                </div>
              ))}
            </div>
            {order.is_delayed && (
              <div className="mt-3 p-3 rounded-lg bg-rose-50 border border-rose-200 text-micro text-rose-700 flex items-start gap-2">
          ...
          {/* Cost Summary */}
          <div className="bg-background border border-border/40 rounded-xl p-5">
            <h3 className="text-body font-semibold mb-3" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "Cost Tracking", "تتبع التكاليف")}</h3>
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
              {[
                ...
                <div key={i}>
                  <p className="text-micro text-muted-foreground mb-1">{s.label}</p>
                  <p className={`text-body-lg font-semibold tabular-nums ${s.color || "text-foreground"}`} style={{ fontFamily: "var(--app-font-serif)" }}>{s.value}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {tab === "stages" && (
        <div className="space-y-3">
          ...
          <div className="bg-background border border-border/40 rounded-xl p-5">
            <h3 className="text-body font-semibold mb-4" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "Manufacturing Stages", "مراحل التصنيع")}</h3>
                  ...
                  <div key={stage.id} className="flex items-center gap-3">
                    ...
                    <div className={`flex-1 flex items-center justify-between px-3 py-2 rounded-lg ${isCompleted ? "bg-emerald-50/50" : isActive ? "bg-blue-50/50" : "bg-muted/20"}`}>
                      <div className="flex items-center gap-2.5">
                        <span className={`text-caption font-medium ${isCompleted ? "text-emerald-700" : isActive ? "text-blue-700" : "text-muted-foreground"}`}>
                          {ar ? stage.stage_name_ar : stage.stage_name_en}
                        </span>
                        {stage.assigned_operator && <span className="text-micro text-muted-foreground flex items-center gap-1"><Users size={9} />{stage.assigned_operator}</span>}
                      </div>
          ...
          {/* Stage Details Table */}
          <div className="bg-background border border-border/40 rounded-xl overflow-hidden">
            <div className="grid grid-cols-[1fr_80px_80px_80px_80px_80px_80px] gap-2 px-4 py-2 bg-muted/30 text-micro text-muted-foreground font-medium border-b border-border/30">
            ...
            <div className="divide-y divide-border/20">
              {order.stages.sort((a, b) => a.sequence - b.sequence).map(stage => (
                <div key={stage.id} className="grid grid-cols-[1fr_80px_80px_80px_80px_80px_80px] gap-2 px-4 py-2.5 items-center text-micro">
      ...
      {tab === "materials" && (
        <div className="bg-background border border-border/40 rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-border/30">
            <h3 className="text-body font-semibold" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "Materials", "المواد")}</h3>
          </div>
          <div className="grid grid-cols-[1fr_80px_80px_80px_80px_80px_100px] gap-2 px-4 py-2 bg-muted/30 text-micro text-muted-foreground font-medium border-b border-border/30">
          ...
          <div className="divide-y divide-border/20">
            {order.materials.map(mat => (
              <div key={mat.id} className="grid grid-cols-[1fr_80px_80px_80px_80px_80px_100px] gap-2 px-4 py-2.5 items-center text-micro">
                <div>
                  <span className="font-medium">{mat.name}</span>
                  {mat.warehouse_location && <span className="text-micro text-muted-foreground block">{mat.warehouse_location}</span>}
                </div>
                ...
                <span className={`tabular-nums ${mat.remaining_qty > 0 && mat.status !== "available" ? "text-warning" : ""}`}>{fmt(mat.remaining_qty)}</span>
                <Badge className={mat.status === "available" ? "bg-emerald-100 text-emerald-600" : mat.status === "partial" ? "bg-warning/15 text-warning" : "bg-rose-100 text-rose-600"}>
                  {mat.status}
                </Badge>
            ...
            {order.materials.length === 0 && (
              <div className="py-8 text-center text-micro text-muted-foreground/50">{t(ar, "No materials tracked", "لا مواد متعقبة")}</div>
            )}
          </div>
        </div>
      )}
      ...
      {tab === "qc" && (
        <div className="space-y-4">
          <div className="bg-background border border-border/40 rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-body font-semibold" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "QC Status", "حالة الجودة")}</h3>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="text-center p-3 bg-muted/20 rounded-lg">
                <p className="text-micro text-muted-foreground">{t(ar, "ناجح", "Passed")}</p>
                <p className="text-title font-semibold text-emerald-600">{fmt(order.passed_qty)}</p>
              </div>
              <div className="text-center p-3 bg-muted/20 rounded-lg">
                <p className="text-micro text-muted-foreground">{t(ar, "مرفوض", "Failed")}</p>
                <p className="text-title font-semibold text-rose-600">{fmt(order.rejected_qty)}</p>
              </div>
              <div className="text-center p-3 bg-muted/20 rounded-lg">
                <p className="text-micro text-muted-foreground">{t(ar, "نسبة العيوب", "Defect %")}</p>
                <p className="text-title font-semibold">{order.planned_qty > 0 ? ((order.rejected_qty / order.planned_qty) * 100).toFixed(1) : 0}%</p>
              </div>
            </div>
          </div>
          ...
          {order.qc_checks.length > 0 && (
            <div className="bg-background border border-border/40 rounded-xl overflow-hidden">
              <div className="px-4 py-3 border-b border-border/30">
                <h3 className="text-body font-semibold" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "QC Checks", "فحوصات الجودة")}</h3>
              </div>
              <div className="divide-y divide-border/20">
                {order.qc_checks.map(qc => (
                  <div key={qc.id} className="px-4 py-3">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-caption font-medium">{qc.stage} — {qc.inspector}</span>
                    ...
                    {qc.notes && <p className="text-micro text-muted-foreground mt-1">{qc.notes}</p>}
                  </div>
                ))}
              </div>
            </div>
      ...
      {tab === "activity" && (
        <div className="bg-background border border-border/40 rounded-xl p-5">
          <h3 className="text-body font-semibold mb-4" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "Activity Log", "سجل النشاط")}</h3>
          <div className="space-y-3">
            {order.activity_log.map(entry => {
              ...
              return (
                <div key={entry.id} className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-caption">{ar ? entry.description_ar : entry.description_en}</p>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-body-lg font-semibold" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "Production Alerts", "تنبيهات الإنتاج")}</h3>
        <span className="text-micro text-muted-foreground">{alerts.length} {t(ar, "active", "نشطة")}</span>
      </div>
      <div className="space-y-2">
        {alerts.map(alert => (
          <div key={alert.id} className={`p-4 rounded-xl border ${ALERT_SEVERITY[alert.severity]}`}>
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-micro font-mono font-medium">{alert.order_number}</span>
                    <Badge className={alert.severity === "critical" ? "bg-rose-200 text-rose-800" : alert.severity === "warning" ? "bg-warning/20 text-warning" : "bg-blue-200 text-blue-800"}>
                      {alert.severity}
                    </Badge>
                  </div>
                  <p className="text-caption font-medium">{ar ? alert.message_ar : alert.message_en}</p>
                  <p className="text-micro text-muted-foreground mt-1">{new Date(alert.created_at).toLocaleString()}</p>
                </div>
              </div>
              <button onClick={() => handleDismiss(alert.id)} className="text-micro text-muted-foreground hover:text-foreground px-2 py-1 rounded-lg hover:bg-muted/50">
                {t(ar, "تجاهل", "Dismiss")}
              </button>
            </div>
          </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-body-lg font-semibold" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "Buzz — Production Intelligence", "Buzz — ذكاء الإنتاج")}</h3>
      </div>
      ...
      <div className="bg-gradient-to-br from-primary/5 to-primary/10 border border-primary/20 rounded-xl p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-caption font-medium text-brand-ink">{t(ar, "ملخص AI", "AI Summary")}</span>
        </div>
          ...
          return (
            <div key={insight.id} className={`bg-background border border-border/40 rounded-xl p-4 border-l-4 ${AI_SEVERITY[insight.severity]}`}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-caption font-semibold mb-1">{ar ? insight.title_ar : insight.title_en}</p>
                  <p className="text-micro text-muted-foreground leading-relaxed">{ar ? insight.detail_ar : insight.detail_en}</p>
                </div>
              </div>
            </div>
          );
...
function ReportsView({ ar }: { ar: boolean }) {
  ...
  function handleExport(type: string) {
    ...
    a.href = url;
    a.download = `bumblebee-production-${type}-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-body-lg font-semibold" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "Production Reports", "تقارير الإنتاج")}</h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {REPORTS.map(report => (
          <button key={report.id} onClick={() => handleExport(report.id)}
            ...
            <div className="flex-1 min-w-0">
              <p className="text-caption font-medium">{ar ? report.ar : report.en}</p>
              <p className="text-micro text-muted-foreground mt-0.5">{t(ar, "تصدير CSV", "Export CSV")}</p>
            </div>
      ...
      {/* Quick Stats */}
      <div className="bg-background border border-border/40 rounded-xl p-5">
        <h4 className="text-body font-semibold mb-3" style={{ fontFamily: "var(--app-font-serif)" }}>{t(ar, "ملخص سريع", "Quick Summary")}</h4>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div>
            <p className="text-micro text-muted-foreground">{t(ar, "إجمالي الأوامر", "Total Orders")}</p>
            <p className="text-title font-semibold">{stats.totalOrders}</p>
          </div>
          <div>
            <p className="text-micro text-muted-foreground">{t(ar, "معدل الإنجاز", "Completion Rate")}</p>
            <p className="text-title font-semibold">{stats.totalPlannedQty > 0 ? Math.round((stats.totalCompletedQty / stats.totalPlannedQty) * 100) : 0}%</p>
          </div>
          <div>
            <p className="text-micro text-muted-foreground">{t(ar, "متوسط الكفاءة", "Avg Efficiency")}</p>
            <p className="text-title font-semibold">{stats.avgEfficiency}%</p>
          </div>
          <div>
            <p className="text-micro text-muted-foreground">{t(ar, "معدل التسليم", "On-Time Rate")}</p>
            <p className="text-title font-semibold">{stats.onTimeRate}%</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="flex gap-1 mb-5 border-b border-border/30 pb-2 overflow-x-auto">
        {VIEW_TABS.map(tab => (
          <button key={tab.id} onClick={() => setView(tab.id)}
            className={`px-3 py-2 rounded-lg text-caption font-medium transition-colors flex items-center gap-1.5 whitespace-nowrap
