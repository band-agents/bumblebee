        ...
        <div className="px-6 py-5 space-y-4">
              ...
              {(["campaign", "birthday", "first_purchase"] as const).map(t => {
                ...
                return (
                  <button key={t} onClick={() => setType(t)}
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-micro font-medium transition-all ${type === t ? "border-primary bg-primary/5 text-brand-ink" : "border-border/60 bg-card text-muted-foreground hover:border-border"}`}>
                    <Icon size={13} />
                    {ar ? meta.ar : meta.en}
                  </button>
                );
          ...
          {type === "campaign" ? (
            <div>
              ...
              <p className="text-micro text-muted-foreground mt-1">{ar ? "1.0 = عادي، 2.0 = مضاعف" : "1.0 = normal, 2.0 = double"}</p>
            </div>
          ) : (
            <div>
  ...
  return (
    <div className="min-h-full py-8 px-7 md:px-10 max-w-[960px] mx-auto">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="border border-border/40 rounded-xl p-4 bg-background">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-micro text-muted-foreground">{ar ? "نشطة الآن" : "Active Now"}</span>
          </div>
          <p className="text-heading font-medium tabular-nums" style={{ fontFamily: "var(--app-font-serif)" }}>{activeCampaigns}</p>
        </div>
        <div className="border border-border/40 rounded-xl p-4 bg-background">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-2 h-2 rounded-full bg-warning" />
            <span className="text-micro text-muted-foreground">{ar ? "مجدولة" : "Scheduled"}</span>
          </div>
          <p className="text-heading font-medium tabular-nums" style={{ fontFamily: "var(--app-font-serif)" }}>{scheduledCampaigns}</p>
        </div>
        <div className="border border-border/40 rounded-xl p-4 bg-background">
          <div className="flex items-center gap-2 mb-2">
            <Calendar size={10} className="text-muted-foreground" />
            <span className="text-micro text-muted-foreground">{ar ? "حملات زمنية" : "Time-Bounded"}</span>
          </div>
          <p className="text-heading font-medium tabular-nums" style={{ fontFamily: "var(--app-font-serif)" }}>{timeBoundedActive}</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <span className="text-micro text-muted-foreground ms-auto">{filtered.length} {ar ? "حملة" : "campaigns"}</span>
      </div>
      ...
      <div className="space-y-3">
        {sortedCampaigns.length === 0 ? (
          <div className="py-14 text-center border border-border/40 rounded-xl">
            ...
            <p className="text-body text-muted-foreground">{ar ? "لا توجد حملات" : "No campaigns found"}</p>
          </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-body-lg font-medium truncate" style={{ fontFamily: "var(--app-font-serif)" }}>
                      {ar ? cp.nameAr : cp.nameEn}
                    </h3>
                    <span className={`text-micro font-semibold px-2 py-0.5 rounded-full flex items-center gap-1 ${RULE_STATUS_META[cp.status].dot.replace("bg-", "text-")} bg-opacity-10`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${sm.dot}`} />
                      {ar ? sm.ar : sm.en}
                    </span>
                  </div>

                  <p className="text-micro text-muted-foreground mb-3">{ar ? cp.descAr : cp.descEn}</p>

                  {/* Metadata chips */}
                  <div className="flex flex-wrap items-center gap-2">
                    {cp.multiplier && (
                      <span className="text-micro font-medium px-2 py-1 rounded-lg bg-primary/5 text-brand-ink flex items-center gap-1">
                    ...
                    {cp.startsAt && cp.endsAt && (
                      <span className="text-micro font-medium px-2 py-1 rounded-lg bg-muted/40 text-muted-foreground flex items-center gap-1">
                        <Calendar size={9} />
                    ...
                    {cp.startsAt && cp.endsAt && (
                      <span className={`text-micro px-2 py-1 rounded-lg flex items-center gap-1 ${isActive ? "bg-emerald-50 text-emerald-700" : isScheduled ? "bg-warning/10 text-warning" : "bg-muted text-muted-foreground"}`}>
                        <Clock size={9} />{daysLabel(cp.startsAt, cp.endsAt)}
                      </span>
                    )}
                    {cp.categoryFilter && (
                      <span className="text-micro font-medium px-2 py-1 rounded-lg bg-blue-50 text-blue-700 flex items-center gap-1">
                  ...
                  {cp.estimatedImpactEn && (
                    <p className="text-micro text-muted-foreground/70 mt-2 flex items-center gap-1">
              ...
              return (
                <div key={cp.id}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-micro font-medium">{ar ? cp.nameAr : cp.nameEn}</span>
                  ...
                  <div className="h-3 bg-muted/30 rounded-full relative overflow-hidden">
                    <div
                      className={`absolute top-0 h-full rounded-full transition-all ${isNowIn ? "bg-primary" : cp.status === "expired" ? "bg-muted-foreground/25" : "bg-warning"}`}
      ...
      <div className="mt-4 text-center">
        <button onClick={() => navigate("/loyalty/rules")} className="text-micro text-brand-ink hover:underline flex items-center gap-1 mx-auto">
