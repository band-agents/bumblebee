      ...
      <motion.div variants={cardV} custom={6} initial="hidden" animate="visible" className="border border-border/40 rounded-xl overflow-hidden bg-background">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border/30">
                ...
                <th className="text-right px-4 py-3 text-micro font-semibold text-muted-foreground">{ar ? "الإجمالي" : "Net"}</th>
                  ...
                  <tr key={record.id} className="border-b border-border/20 hover:bg-muted/20 cursor-pointer transition-colors"
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded-full flex items-center justify-center text-micro font-bold text-white shrink-0" style={{ backgroundColor: emp?.avatar_color || "#6B7280" }}>
                          {emp?.full_name.split(" ").map(w => w[0]).join("")}
                        </div>
                        <div>
                          <p className="text-caption font-medium">{emp ? (ar && emp.full_name_ar ? emp.full_name_ar : emp.full_name) : "—"}</p>
                          <p className="text-micro text-muted-foreground font-mono">{emp?.employee_number}</p>
                        </div>
                      </div>
                    </td>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-caption text-muted-foreground">{ar ? "لا توجد سجلات" : "No records found"}</p>
          </div>
        )}
      </motion.div>
              ...
              {(() => {
                ...
                return emp ? (
                  <div className="flex items-center gap-3 pb-4 border-b border-border/30">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center text-caption font-bold text-white" style={{ backgroundColor: emp.avatar_color }}>
                      {emp.full_name.split(" ").map(w => w[0]).join("")}
                    </div>
                    <div>
                      <p className="text-body font-medium">{ar && emp.full_name_ar ? emp.full_name_ar : emp.full_name}</p>
                      <p className="text-micro text-muted-foreground">{emp.job_title} · {emp.employee_number}</p>
                    </div>
                  </div>
                ) : null;
              })()}
              <div className="space-y-2">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between py-1.5 border-b border-border/20 last:border-0">
                    ...
                    <span className={`text-caption font-medium ${row.color}`}>{formatEGP(Math.abs(row.value))}</span>
                  </div>
                ))}
              </div>
              <div className="flex items-center justify-between pt-3 border-t border-border/40">
                <span className="text-body font-semibold">{ar ? "صافي الراتب" : "Net Salary"}</span>
                <span className="text-title font-bold text-brand-ink" style={{ fontFamily: "var(--app-font-serif)" }}>{formatEGP(selectedRecord.net_salary)}</span>
              </div>
              <div className="flex gap-2 pt-2">
                <button className="flex-1 h-10 rounded-xl bg-primary text-primary-foreground text-caption font-medium hover:opacity-90 transition-opacity flex items-center justify-center gap-1.5">
