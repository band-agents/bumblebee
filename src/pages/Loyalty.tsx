...
function TierBar({ dist, total, lang }: { dist: Record<LoyaltyTierSlug, number>; total: number; lang: "en" | "ar" }) {
  ...
  return (
    <div className="bg-background border border-border/40 rounded-xl p-5">
      <h3 className="text-body font-medium text-foreground mb-4" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.01em" }}>
      ...
      <div className="flex items-center gap-4 mt-3">
        {TIER_ORDER.map(slug => (
          <div key={slug} className="flex items-center gap-1.5">
            ...
            <span className="text-micro text-muted-foreground">{ar ? TIER_META[slug].ar : TIER_META[slug].en}</span>
            <span className="text-micro font-medium text-foreground tabular-nums">{dist[slug]}</span>
          </div>
        ))}
      </div>
    </div>
        ...
        {top.map((m, i) => {
          ...
          return (
            <div key={m.id} onClick={() => onNavigate(m.id)} className="flex items-center gap-3.5 px-5 py-3 hover:bg-muted/20 transition-colors cursor-pointer group">
              <span className="text-micro text-muted-foreground/50 tabular-nums w-4 shrink-0">{i + 1}</span>
              <div className="w-8 h-8 rounded-lg flex items-center justify-center text-micro font-semibold text-white shrink-0" style={{ backgroundColor: m.avatarColor }}>
                {initials(ar ? m.nameAr : m.nameEn)}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-body font-medium text-foreground truncate group-hover:text-brand-ink transition-colors">
                  {ar ? m.nameAr : m.nameEn}
                </p>
                <p className="text-micro text-muted-foreground">{m.phone}</p>
              </div>
              <span className={`text-micro font-semibold px-2 py-0.5 rounded-full ${tm.pill}`}>
                {ar ? tm.ar : tm.en}
              </span>
              <div className="text-end shrink-0">
                <p className="text-body font-medium text-foreground tabular-nums">{fmtPts(m.currentPoints)}</p>
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
      {actions.map(a => (
        <button key={a.path} onClick={() => onNavigate(a.path)}
          ...
          <div>
            <p className="text-body font-medium text-foreground group-hover:text-brand-ink transition-colors">{ar ? a.labelAr : a.labelEn}</p>
          </div>
          <ArrowUpRight size={12} className="text-muted-foreground/30 group-hover:text-brand-ink/50 ms-auto transition-colors" />
        </button>
      ))}
    </div>
  );
      ...
      <div className="border-b border-border/40 px-8 md:px-10 py-8" style={{ background: "linear-gradient(160deg, hsl(var(--muted)/0.3) 0%, hsl(var(--background)) 60%)" }}>
        <div className="max-w-[1100px]">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <MetricCard icon={Users} label={ar ? "إجمالي الأعضاء" : "Total Members"} value={String(stats.totalMembers)} sub={`${stats.activeMembers} ${ar ? "نشط" : "active"}`} color="text-brand-ink" />
