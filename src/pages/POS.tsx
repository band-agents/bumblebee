    ...
    return (
      <div className="h-full flex items-center justify-center bg-background">
        <div className="text-center space-y-2">
          ...
          <p className="text-body text-muted-foreground">{lang === "ar" ? "جاري التحميل..." : "Loading POS..."}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="h-full flex items-center justify-center bg-background p-6">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-sm">
          <div className="bg-card border border-border rounded-2xl p-6 text-center space-y-4">
            ...
            <div>
              <p className="text-title font-semibold text-foreground" style={{ fontFamily: "var(--app-font-serif)" }}>
                {lang === "ar" ? "تم بنجاح" : "Payment Successful"}
              </p>
              <p className="text-caption text-muted-foreground mt-1">{receipt.txnNumber}</p>
            </div>
            <div className="border-t border-border pt-3">
              <p className="text-micro text-muted-foreground uppercase tracking-wider">{lang === "ar" ? "المبلغ" : "Total"}</p>
              <p className="text-heading font-bold text-foreground mt-1" style={{ fontFamily: "var(--app-font-serif)" }}>
    ...
    <div className="h-[calc(100dvh-56px)] flex overflow-hidden bg-background">
      ...
      <div className="flex-1 flex flex-col min-w-0 border-r border-border/40">
        ...
        <div className="shrink-0 px-4 py-3 border-b border-border/40 bg-card/50">
          ...
          <div className="flex items-center gap-1.5 mb-3">
            <button onClick={() => setShowCustomerDrawer(true)} className="h-8 px-3 rounded-lg bg-muted/50 hover:bg-muted text-micro font-medium text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1.5">
              <Users size={12} /> {lang === "ar" ? "العملاء" : "Customers"}
            </button>
            <button onClick={() => setShowReports(true)} className="h-8 px-3 rounded-lg bg-muted/50 hover:bg-muted text-micro font-medium text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1.5">
              <BarChart3 size={12} /> {lang === "ar" ? "التقارير" : "Reports"}
            </button>
            <button onClick={() => setShowTransactions(true)} className="h-8 px-3 rounded-lg bg-muted/50 hover:bg-muted text-micro font-medium text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1.5">
              <History size={12} /> {lang === "ar" ? "السجل" : "History"}
            </button>
            <button onClick={() => { holdTransaction(); }} className="h-8 px-3 rounded-lg bg-muted/50 hover:bg-muted text-micro font-medium text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1.5" disabled={cart.length === 0}>
              <Pause size={12} /> {lang === "ar" ? "تعليق" : "Hold"}
            </button>
            {heldTransactions.length > 0 && (
              <button onClick={() => setShowHeld(true)} className="h-8 px-3 rounded-lg bg-warning/10 hover:bg-warning/15 text-micro font-medium text-warning transition-colors flex items-center gap-1.5 relative">
                <Pause size={12} />
            ...
            <div className="ml-auto flex items-center gap-1.5">
              ...
              <button onClick={() => setShowFilters(!showFilters)} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${showFilters ? "bg-primary/10 text-brand-ink" : "bg-muted/50 hover:bg-muted text-muted-foreground hover:text-foreground"}`}>
                <Filter size={13} />
              </button>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <select
                ...
                className="h-9 pl-8 pr-8 rounded-lg border border-border bg-background text-body text-foreground appearance-none cursor-pointer focus:outline-none focus:ring-1 focus:ring-brand-ink/30"
              >
                {branches.map((b) => (
            ...
            <div className="ml-auto flex items-center gap-2">
              {isOffline ? (
                <span className="flex items-center gap-1.5 text-micro text-warning bg-warning/10 px-2.5 py-1 rounded-full border border-warning/30">
                  <WifiOff size={11} />
                  {lang === "ar" ? "غير متصل" : "Offline"}
                </span>
              ) : (
                <span className="flex items-center gap-1.5 text-micro text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-200">
                  <Wifi size={11} />
                  {lang === "ar" ? "متصل" : "Online"}
                </span>
              )}
          ...
          <div className="flex items-center gap-2 mt-3">
            ...
            <select
              ...
              className="h-9 px-2 rounded-lg border border-border bg-background text-micro appearance-none cursor-pointer focus:outline-none focus:ring-1 focus:ring-brand-ink/30"
            >
            ...
            {showFilters && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                <div className="pt-3 space-y-2">
                  <div className="flex items-center gap-3">
                    <span className="text-micro text-muted-foreground w-16">{lang === "ar" ? "السعر" : "Price"}</span>
                    <input
                      type="range"
                      min={0}
                      max={10000}
                    ...
                    />
                    <span className="text-micro text-foreground font-medium w-20 text-right">≤ {formatEGP(priceRange[1])}</span>
                  </div>
                  <div className="flex gap-1.5 overflow-x-auto no-scrollbar">
                    {categories.map((cat) => (
                      <button
                        ...
                        className={`shrink-0 h-7 px-3 rounded-full text-micro font-medium transition-all ${
                          activeCategory === cat
                ...
                return (
                  ...
                  >
                    <div className="w-full aspect-square rounded-lg bg-muted/50 mb-2 flex items-center justify-center relative">
                      <ShoppingCart size={20} className="text-muted-foreground/30 group-hover:text-brand-ink/40 transition-colors" />
                      <button
                    ...
                    <p className="text-caption font-medium text-foreground leading-tight truncate">{product.product_name}</p>
                    {product.sku && <p className="text-micro text-muted-foreground/60 mt-0.5 font-mono">{product.sku}</p>}
                    <div className="flex items-center justify-between mt-2">
                      <p className="text-body font-semibold text-foreground" style={{ fontFamily: "var(--app-font-serif)" }}>
                        {formatEGP(product.unit_price)}
                      </p>
                      {inCart && (
                        <span className="absolute top-2 right-2 w-5 h-5 rounded-full bg-primary text-primary-foreground text-micro font-bold flex items-center justify-center">
                          {inCart.quantity}
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-1 mt-1">
                      <div className={`w-1.5 h-1.5 rounded-full ${available <= product.reorder_level ? "bg-warning" : "bg-emerald-500"}`} />
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-caption font-medium text-foreground truncate">{product.product_name}</p>
                      <p className="text-micro text-muted-foreground font-mono">{product.sku || "—"}</p>
                    </div>
                    <div className="flex items-center gap-3 shrink-0">
                      <div className="text-right">
                        <p className="text-caption font-semibold text-foreground">{formatEGP(product.unit_price)}</p>
                      ...
                      {inCart && (
                        <span className="w-5 h-5 rounded-full bg-primary text-primary-foreground text-micro font-bold flex items-center justify-center">
                          {inCart.quantity}
                        </span>
                      )}
                    </div>
      ...
      <div className="w-[380px] shrink-0 flex flex-col bg-card">
        ...
        <div className="shrink-0 px-4 py-3 border-b border-border/40">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              {cart.length > 0 && (
                <span className="w-5 h-5 rounded-full bg-primary/10 text-brand-ink text-micro font-bold flex items-center justify-center">
        ...
        <div className="shrink-0 px-4 py-3 border-b border-border/40 space-y-2">
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full h-8 pl-8 pr-3 rounded-lg border border-border bg-background text-caption placeholder:text-muted-foreground/40 focus:outline-none focus:ring-1 focus:ring-brand-ink/30"
              />
            </div>
          ...
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground/50 text-caption">+20</span>
            <input
              type="tel"
              value={customerPhone}
              onChange={(e) => setCustomerPhone(e.target.value)}
              placeholder={lang === "ar" ? "رقم الهاتف" : "Phone number"}
              className="w-full h-8 pl-12 pr-3 rounded-lg border border-border bg-background text-caption placeholder:text-muted-foreground/40 focus:outline-none focus:ring-1 focus:ring-brand-ink/30"
            />
          </div>
        </div>
          ...
          <AnimatePresence mode="popLayout">
            {cart.length === 0 ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center h-full text-center">
                ...
                <p className="text-caption text-muted-foreground">{lang === "ar" ? "أضف منتجات للبدء" : "Add products to start"}</p>
                <p className="text-micro text-muted-foreground/60 mt-1">{lang === "ar" ? "انقر على منتج لإضافته" : "Click a product to add"}</p>
              </motion.div>
            ) : (
              cart.map((item, idx) => (
                <motion.div
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      {item.discount_percent > 0 && (
                        <span className="text-micro px-1.5 py-0.5 rounded-full bg-rose-100 text-rose-600 shrink-0 ml-1">-{item.discount_percent}%</span>
                      )}
                    </div>
                    {item.sku && <p className="text-micro text-muted-foreground/60 font-mono">{item.sku}</p>}
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      <span className="text-caption font-medium w-6 text-center">{item.quantity}</span>
                      <button onClick={() => updateQuantity(idx, 1)} className="w-6 h-6 rounded-md border border-border flex items-center justify-center hover:bg-muted transition-colors">
                        <Plus size={11} />
                      </button>
                      <div className="ml-auto flex items-center gap-1">
                        ...
                        <input
                          ...
                          className="w-10 h-5 px-1 rounded border border-border bg-background text-micro text-center focus:outline-none focus:ring-1 focus:ring-brand-ink/30"
                        />
                      </div>
                    </div>
                  </div>
          ...
          <div className="shrink-0 border-t border-border/40 px-4 py-3 space-y-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <div className="flex gap-1 overflow-x-auto no-scrollbar flex-1">
                {DISCOUNT_PRESETS.map((pct) => (
                  <button
                    ...
                    className={`shrink-0 h-6 px-2 rounded-md text-micro font-medium transition-all ${
                      discountPercent === pct
                        ? "bg-primary text-primary-foreground"
                        : "bg-muted text-muted-foreground hover:bg-muted/80"
                    }`}
              ...
              <input
                ...
                className="w-12 h-6 px-1 rounded-md border border-border bg-background text-micro text-right focus:outline-none focus:ring-1 focus:ring-brand-ink/30"
              />
            </div>
            ...
            <input
              ...
              className="w-full h-8 px-3 rounded-lg border border-border bg-background text-caption placeholder:text-muted-foreground/40 focus:outline-none focus:ring-1 focus:ring-brand-ink/30"
            />
            ...
            <div className="flex items-center gap-1.5 text-micro text-brand-ink">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="h-10 px-3 rounded-xl border border-border text-caption font-medium text-muted-foreground hover:bg-muted transition-colors flex items-center gap-1.5 shrink-0"
              >
                <Pause size={12} /> {lang === "ar" ? "تعليق" : "Hold"}
              </button>
              <button
                onClick={() => setShowPayment(true)}
                className="flex-1 h-10 rounded-xl bg-primary text-primary-foreground text-body font-semibold hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
              >
                <CreditCard size={15} />
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={() => !processing && setShowPayment(false)}>
            ...
            >
              <div className="flex items-center justify-between px-5 py-4 border-b border-border/40">
                <h3 className="text-body-lg font-semibold text-foreground" style={{ fontFamily: "var(--app-font-serif)" }}>
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="text-center py-3 bg-muted/30 rounded-xl">
                  <p className="text-micro text-muted-foreground uppercase tracking-wider">{lang === "ar" ? "المبلغ المستحق" : "Amount Due"}</p>
                  <p className="text-display font-bold text-foreground mt-1" style={{ fontFamily: "var(--app-font-serif)" }}>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {PAYMENT_METHODS.map((pm) => {
                    ...
                    return (
                      ...
                      >
                        <Icon size={18} className={paymentMethod === pm.id ? "text-brand-ink" : "text-muted-foreground"} />
                        <span className={`text-micro font-medium ${paymentMethod === pm.id ? "text-brand-ink" : "text-muted-foreground"}`}>
                ...
                {paymentMethod === "cash" && (
                  <div className="space-y-2">
                    <label className="text-micro text-muted-foreground uppercase tracking-wider">{lang === "ar" ? "المبلغ المستلم" : "Cash Received"}</label>
                    <input
                      type="number"
                      value={cashReceived}
                      onChange={(e) => setCashReceived(e.target.value)}
                      placeholder={cartTotals.total.toFixed(2)}
                      className="w-full h-11 px-4 rounded-xl border border-border bg-background text-title font-semibold text-foreground focus:outline-none focus:ring-2 focus:ring-brand-ink/30"
                      autoFocus
                    />
                    {Number(cashReceived) >= cartTotals.total && (
                      <div className="flex justify-between text-body bg-emerald-50 text-emerald-700 px-3 py-2 rounded-lg border border-emerald-200">
                    ...
                    <div className="flex gap-1.5">
                      {[cartTotals.total, Math.ceil(cartTotals.total / 50) * 50, Math.ceil(cartTotals.total / 100) * 100].filter((v, i, a) => a.indexOf(v) === i).map((amt) => (
                        <button
                          ...
                          className="flex-1 h-7 rounded-md bg-muted text-micro font-medium text-muted-foreground hover:bg-muted/80 transition-colors"
                        >
                          {formatEGP(amt)}
                        </button>
                      ))}
                ...
                {paymentMethod === "split" && (
                  <div className="space-y-3">
                    <div>
                      <label className="text-micro text-muted-foreground uppercase tracking-wider mb-1 block">{lang === "ar" ? "نقداً" : "Cash Amount"}</label>
                      <input
                        type="number"
                        value={splitCash}
                        onChange={(e) => { setSplitCash(e.target.value); setSplitCard(String(Math.max(0, cartTotals.total - Number(e.target.value)))); }}
                        placeholder="0"
                        className="w-full h-10 px-3 rounded-xl border border-border bg-background text-body-lg font-semibold focus:outline-none focus:ring-2 focus:ring-brand-ink/30"
                      />
                    </div>
                    <div>
                      <label className="text-micro text-muted-foreground uppercase tracking-wider mb-1 block">{lang === "ar" ? "بطاقة" : "Card Amount"}</label>
                      <input
                        type="number"
                        value={splitCard}
                ...
                <button
                  ...
                  className="w-full h-12 rounded-xl bg-primary text-primary-foreground text-body-lg font-semibold hover:opacity-90 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {processing ? (
                    <>
