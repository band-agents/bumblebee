...
function Section({ title, children, action }: { title: string; children: React.ReactNode; action?: React.ReactNode }) {
  return (<div className="border border-border/40 rounded-xl bg-background overflow-hidden"><div className="flex items-center justify-between px-6 py-4 border-b border-border/30"><h3 className="text-micro font-semibold text-muted-foreground tracking-[0.08em] uppercase">{title}</h3>{action}</div>{children}</div>);
}

function DetailRow({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: React.ReactNode }) {
  return (<div className="flex items-center gap-3.5 px-6 py-3.5"><div className="w-7 h-7 rounded-lg bg-muted/40 flex items-center justify-center shrink-0"><Icon size={13} strokeWidth={1.75} className="text-muted-foreground" /></div><div className="flex-1 min-w-0"><p className="text-micro text-muted-foreground/70 mb-0.5">{label}</p><div className="text-body text-foreground">{value}</div></div></div>);
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" aria-modal="true">
      ...
      <div className="relative bg-background border border-border/60 rounded-2xl shadow-xl w-full max-w-[480px] overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border/40"><h2 className="text-body-lg font-medium text-foreground" style={{ fontFamily: "var(--app-font-serif)" }}>{ar ? "إضافة ملاحظة" : "Add Note"}</h2><button onClick={onClose} className="w-7 h-7 rounded-lg flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"><X size={14} strokeWidth={2} /></button></div>
        <div className="px-6 py-4"><textarea value={text} onChange={(e) => setText(e.target.value)} placeholder={ar ? "اكتب…" : "Write…"} rows={4} autoFocus className="w-full px-3 py-2.5 rounded-xl border border-border/80 bg-card text-body text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/40 resize-none" /></div>
        <div className="px-6 py-3 border-t border-border/40 flex justify-end gap-3"><button onClick={onClose} className="h-8 px-4 rounded-xl border border-border text-caption text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">{ar ? "إلغاء" : "Cancel"}</button><button onClick={() => { if (text.trim()) { onAdd(text.trim()); setText(""); onClose(); } }} disabled={!text.trim()} className="h-8 px-4 rounded-xl bg-primary text-primary-foreground text-caption font-medium hover:opacity-90 transition-opacity disabled:opacity-40">{ar ? "إضافة" : "Add"}</button></div>
      </div>
    </div>
  );
}
...
export default function ResourceDetail() {
  ...
  if (!res) {
    return (<div className="min-h-full flex flex-col items-center justify-center gap-4 p-8"><div className="w-12 h-12 rounded-2xl bg-muted flex items-center justify-center"><Package size={20} className="text-muted-foreground" strokeWidth={1.5} /></div><p className="text-body-lg font-medium text-foreground" style={{ fontFamily: "var(--app-font-serif)" }}>{ar ? "لم يُعثر على المورد" : "Resource not found"}</p><button onClick={() => navigate("/resources")} className="flex items-center gap-1.5 text-caption text-brand-ink hover:underline"><ArrowLeft size={12} strokeWidth={2} />{ar ? "العودة" : "Back"}</button></div>);
  }
  ...
  const insights = [
    {
      icon: Gauge, color: res.utilization >= 60 ? "text-emerald-600" : "text-warning", bg: res.utilization >= 60 ? "bg-emerald-50" : "bg-warning/10",
  ...
  return (
    <div className="min-h-full">
      {actionToast && (<div className="fixed top-4 start-1/2 -translate-x-1/2 z-50 px-5 py-2.5 rounded-xl bg-foreground text-background text-body font-medium shadow-lg flex items-center gap-2 animate-in fade-in slide-in-from-top-2 duration-200"><Check size={14} strokeWidth={2.5} />{actionToast}</div>)}
      ...
      <div className="relative border-b border-border/40" style={{ background: "linear-gradient(160deg, hsl(var(--muted)/0.35) 0%, hsl(var(--background)) 65%)" }}>
        <div className="px-8 md:px-10 pt-6">
          <button onClick={() => navigate("/resources")} className="inline-flex items-center gap-1.5 text-caption text-muted-foreground hover:text-foreground transition-colors group">
        ...
        <div className="px-8 md:px-10 pt-5 pb-6 max-w-[960px]">
          <div className="flex items-start gap-5 mb-5">
            <div className="w-14 h-14 rounded-2xl bg-primary/8 flex items-center justify-center shrink-0"><Icon size={22} strokeWidth={1.75} className="text-brand-ink" /></div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2.5 flex-wrap mb-2">
                <span className={`text-micro font-medium px-2.5 py-1 rounded-full ${tm.pill}`}>{ar ? tm.ar : tm.en}</span>
                <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-micro font-medium ${sm.pill}`}><div className={`w-1.5 h-1.5 rounded-full ${sm.dot}`} />{ar ? sm.ar : sm.en}</div>
              </div>
              <h1 className="text-display md:text-display font-medium text-foreground leading-tight mb-1" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.03em" }}>{ar ? res.nameAr : res.nameEn}</h1>
          ...
          {res.status !== "retired" && (
            <div className="flex items-center gap-4 mb-5">
              <div className="flex-1 h-[5px] rounded-full bg-border/40 overflow-hidden">
                <div className={`h-full rounded-full transition-all ${res.utilization >= 70 ? "bg-emerald-500" : res.utilization >= 40 ? "bg-warning" : "bg-rose-400"}`} style={{ width: `${res.utilization}%` }} />
              </div>
              <span className="text-body font-medium text-foreground tabular-nums" style={{ fontFamily: "var(--app-font-serif)" }}>{res.utilization}%</span>
            </div>
          )}
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            <div className="relative">
              <button onClick={() => setStatusMenuOpen(!statusMenuOpen)} className="h-8 px-3.5 rounded-xl border border-border text-caption font-medium text-foreground hover:bg-muted flex items-center gap-1.5 transition-colors"><ArrowRightCircle size={13} strokeWidth={1.75} />{ar ? "تغيير الحالة" : "Change Status"}</button>
              {statusMenuOpen && (<><div className="fixed inset-0 z-40" onClick={() => setStatusMenuOpen(false)} /><div className="absolute top-full mt-1.5 start-0 z-50 bg-background border border-border/60 rounded-xl shadow-lg py-1.5 min-w-[180px]">
                {(["active", "idle", "maintenance", "retired"] as ResourceStatus[]).map((s) => { const m = RESOURCE_STATUS_META[s]; const c = s === res.status; return (<button key={s} onClick={() => changeStatus(s)} disabled={c} className={`w-full flex items-center gap-2.5 px-4 py-2 text-caption text-start transition-colors ${c ? "text-brand-ink bg-primary/5 font-medium" : "text-foreground hover:bg-muted"}`}><div className={`w-2 h-2 rounded-full ${m.dot}`} />{ar ? m.ar : m.en}{c && <Check size={12} strokeWidth={2.5} className="ms-auto text-brand-ink" />}</button>); })}
              </div></>)}
            </div>
            ...
            {res.status !== "retired" && <button onClick={() => changeStatus("retired")} className="h-8 px-3.5 rounded-xl border border-rose-300 text-rose-600 text-caption font-medium hover:bg-rose-50 flex items-center gap-1.5 transition-colors"><AlertTriangle size={13} strokeWidth={1.75} />{ar ? "إيقاف" : "Retire"}</button>}
          </div>
        </div>
      </div>
      ...
      <div className="border-b border-border/40 bg-background sticky top-0 z-10">
        <div className="px-8 md:px-10 flex items-center gap-0 overflow-x-auto">
          {TABS.map((tab) => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-4 py-3 text-caption font-medium whitespace-nowrap border-b-2 transition-all ${activeTab === tab.id ? "border-primary text-brand-ink" : "border-transparent text-muted-foreground hover:text-foreground hover:border-border/60"}`}>{ar ? tab.ar : tab.en}</button>))}
        </div>
      </div>
      ...
      <div className="px-8 md:px-10 py-7 max-w-[960px]">
        ...
        {activeTab === "overview" && (
          <div className="space-y-6">
            <Section title={ar ? "الوصف" : "Description"}><div className="px-6 py-5"><p className="text-body-lg text-foreground/85 leading-[1.8] max-w-[680px]" style={{ fontFamily: "var(--app-font-serif)" }}>{ar ? res.descAr : res.descEn}</p></div></Section>
            ...
            <Section title={ar ? "التفاصيل" : "Details"}>
              <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-border/25">
                <div className="divide-y divide-border/25">
                  <DetailRow icon={Icon} label={ar ? "النوع" : "Type"} value={<span className={`text-micro font-medium px-2.5 py-0.5 rounded-full ${tm.pill}`}>{ar ? tm.ar : tm.en}</span>} />
                  ...
                  {res.assignedToEn && <DetailRow icon={User} label={ar ? "مُعيّن إلى" : "Assigned To"} value={<div className="flex items-center gap-2"><div className="w-5 h-5 rounded-md bg-primary/10 flex items-center justify-center text-micro font-semibold text-brand-ink">{initials(ar ? res.assignedToAr! : res.assignedToEn)}</div>{ar ? res.assignedToAr : res.assignedToEn}</div>} />}
                </div>
                <div className="divide-y divide-border/25">
            ...
            <div className="border border-border/40 rounded-xl overflow-hidden bg-background">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-border/20">
                {[{ value: fmt(res.value), label: ar ? "القيمة" : "Value" }, { value: `${res.utilization}%`, label: ar ? "الاستخدام" : "Utilization" }, { value: String(res.maintenance.length), label: ar ? "سجلات الصيانة" : "Maintenance" }, { value: ar ? sm.ar : sm.en, label: ar ? "الحالة" : "Status" }].map((s, i) => (
                  <div key={i} className="bg-background px-6 py-5 flex flex-col gap-1.5"><p className="text-heading font-medium text-foreground leading-none tabular-nums" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.025em" }}>{s.value}</p><p className="text-micro text-muted-foreground">{s.label}</p></div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2.5 mb-2"><Sparkles size={14} strokeWidth={1.75} className="text-brand-ink" /><span className="text-body font-medium text-foreground">{ar ? "رؤى الموارد" : "Resource Insights"}</span><span className="text-micro text-muted-foreground/50 px-2 py-0.5 rounded-full bg-muted border border-border/40">{ar ? "ذكاء اصطناعي" : "AI"}</span></div>
              {insights.map((c, i) => (
                <div key={i} className="border border-border/40 rounded-xl bg-background p-5 hover:shadow-sm transition-shadow">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-micro text-muted-foreground/60 tracking-wide uppercase mb-1">{ar ? c.titleAr : c.titleEn}</p>
                      <p className="text-body-lg font-medium text-foreground leading-snug mb-2" style={{ fontFamily: "var(--app-font-serif)", letterSpacing: "-0.01em" }}>{ar ? c.valueAr : c.valueEn}</p>
                      <p className="text-caption text-muted-foreground/70 leading-relaxed">{ar ? c.descAr : c.descEn}</p>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <Section title={ar ? "الجدول الزمني" : "Asset Timeline"}>
            <div className="px-6 py-2">
              {timeline.map((ev, i) => { const { Icon: TIcon, color, bg } = TL_ICONS[ev.kind] || TL_ICONS.purchase; const isLast = i === timeline.length - 1; return (
                <div key={ev.id} className="flex gap-4"><div className="flex flex-col items-center"><div className={`w-8 h-8 rounded-xl ${bg} flex items-center justify-center shrink-0`}><TIcon size={14} strokeWidth={1.75} className={color} /></div>{!isLast && <div className="w-px flex-1 bg-border/40 my-1" />}</div><div className={`flex-1 min-w-0 ${isLast ? "pb-4" : "pb-6"}`}><p className="text-body font-medium text-foreground">{ar ? ev.titleAr : ev.titleEn}</p>{ev.descEn && <p className="text-caption text-muted-foreground/80 mt-1">{ar ? ev.descAr : ev.descEn}</p>}<p className="text-micro text-muted-foreground/60 mt-1">{ar ? ev.dateAr : ev.dateEn}</p></div></div>
              ); })}
            </div>
          </Section>
        )}
        ...
        {activeTab === "assignments" && (
          <div className="space-y-6">
            <Section title={ar ? "التعيين الحالي" : "Current Assignment"}>
              {res.assignedToEn ? (
                <div className="flex items-center gap-3.5 px-6 py-4"><div className="w-9 h-9 rounded-xl bg-primary/8 flex items-center justify-center text-micro font-semibold text-brand-ink shrink-0">{initials(ar ? res.assignedToAr! : res.assignedToEn)}</div><div><p className="text-body font-medium text-foreground">{ar ? res.assignedToAr : res.assignedToEn}</p><p className="text-micro text-muted-foreground">{ar ? "مُعيّن حالياً" : "Currently assigned"}</p></div></div>
              ) : (<div className="px-6 py-10 text-center"><p className="text-body text-muted-foreground/60">{ar ? "غير مُعيّن" : "Not assigned"}</p></div>)}
            </Section>
            {relWork.length > 0 && (
                ...
                <div className="divide-y divide-border/25">
                  {relWork.map((w) => { const ws = W_SM[w.status]; const wk = KIND_META[w.kind]; return (
                    <div key={w.id} onClick={() => navigate(`/work/${w.id}`)} className="flex items-center gap-4 px-6 py-4 hover:bg-muted/15 transition-colors cursor-pointer group">
                      ...
                      <div className="flex-1 min-w-0"><div className="flex items-center gap-2 mb-0.5"><span className={`text-micro font-medium px-2 py-0.5 rounded-full ${wk.pill}`}>{ar ? wk.ar : wk.en}</span><h4 className="text-body font-medium text-foreground truncate group-hover:text-brand-ink transition-colors">{ar ? w.titleAr : w.titleEn}</h4></div><div className="flex items-center gap-2 text-micro text-muted-foreground"><div className={`w-1.5 h-1.5 rounded-full ${ws.dot}`} /><span>{ar ? ws.ar : ws.en}</span></div></div>
          ...
          <div className="space-y-6">
            ...
            {upcomingMaint.length > 0 && (
              <Section title={ar ? "قادمة" : "Upcoming"}>
                <div className="divide-y divide-border/25">{upcomingMaint.map((m) => { const ms = MAINT_STATUS_META[m.status]; return (<div key={m.id} className="flex items-center gap-4 px-6 py-4"><div className="w-9 h-9 rounded-xl bg-primary/8 flex items-center justify-center shrink-0"><Calendar size={14} strokeWidth={1.75} className="text-brand-ink" /></div><div className="flex-1"><p className="text-body font-medium text-foreground">{ar ? m.titleAr : m.titleEn}</p><p className="text-micro text-muted-foreground">{ar ? m.dateAr : m.dateEn}{m.costEn ? ` · ${ar ? m.costAr : m.costEn}` : ""}</p></div><div className={`w-2 h-2 rounded-full ${ms.dot}`} /></div>); })}</div>
              </Section>
            )}
        ...
        {activeTab === "files" && (
          <Section title={ar ? "الملفات" : "Files"}>
            <div className="divide-y divide-border/25">{sampleFiles.map((f) => { const fm = FILE_ICONS[f.kind] || FILE_ICONS.pdf; return (<div key={f.id} className="flex items-center gap-4 px-6 py-3.5 hover:bg-muted/15 transition-colors"><div className={`w-9 h-9 rounded-xl ${fm.bg} flex items-center justify-center shrink-0`}><fm.Icon size={15} strokeWidth={1.75} className={fm.color} /></div><div className="flex-1 min-w-0"><p className="text-body font-medium text-foreground truncate">{ar ? f.nameAr : f.nameEn}</p><p className="text-micro text-muted-foreground">{ar ? f.sizeAr : f.sizeEn}</p></div><button className="w-7 h-7 rounded-lg flex items-center justify-center text-muted-foreground/40 hover:text-foreground hover:bg-muted transition-colors shrink-0"><Download size={13} strokeWidth={1.75} /></button></div>); })}</div>
          </Section>
        )}

        {activeTab === "notes" && (
          <Section title={ar ? "الملاحظات" : "Notes"} action={<button onClick={() => setNoteModalOpen(true)} className="flex items-center gap-1.5 text-micro font-medium text-brand-ink hover:text-brand-ink/80 transition-colors"><Plus size={12} strokeWidth={2.5} />{ar ? "إضافة" : "Add"}</button>}>
            <div className="divide-y divide-border/25">{allNotes.map((n) => (<div key={n.id} className="px-6 py-4"><div className="flex items-center gap-2.5 mb-2.5"><div className="w-6 h-6 rounded-md bg-primary/8 flex items-center justify-center text-micro font-semibold text-brand-ink">{initials(ar ? n.authorAr : n.authorEn)}</div><span className="text-caption font-medium text-foreground">{ar ? n.authorAr : n.authorEn}</span><span className="text-micro text-muted-foreground/50 ms-auto">{ar ? n.dateAr : n.dateEn}</span></div><p className="text-body text-foreground/80 leading-[1.7]" style={{ paddingInlineStart: "2.125rem" }}>{ar ? n.contentAr : n.contentEn}</p></div>))}</div>
          </Section>
        )}
          ...
          <Section title={ar ? "النشاط" : "Activity"}>
            <div className="px-6 py-2">
              {[
                ...
                <div key={ev.id} className="flex gap-4"><div className="flex flex-col items-center"><div className={`w-8 h-8 rounded-xl ${bg} flex items-center justify-center shrink-0`}><TIcon size={14} strokeWidth={1.75} className={color} /></div>{!isLast && <div className="w-px flex-1 bg-border/40 my-1" />}</div><div className={`flex-1 min-w-0 ${isLast ? "pb-4" : "pb-6"}`}><p className="text-body font-medium text-foreground">{ar ? ev.titleAr : ev.titleEn}</p><p className="text-micro text-muted-foreground/50 mt-1">{ar ? ev.dateAr : ev.dateEn}</p></div></div>
              ); })}
            </div>
          </Section>
        )}
