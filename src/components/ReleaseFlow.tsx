...
export default function ReleaseFlow({ steps, history, status, ar, currentUserRole, onApprove, onReject }: Props) {
  ...
  return (
    <div className="border border-border/40 rounded-xl overflow-hidden">
      {/* Header */}
      <div className={`px-5 py-3 flex items-center justify-between ${released ? "bg-emerald-50 border-b border-emerald-200" : "bg-warning/10 border-b border-warning/30"}`}>
        <div className="flex items-center gap-2">
          {released ? <CheckCircle2 size={14} className="text-emerald-600" /> : <Clock size={14} className="text-warning" />}
          <span className={`text-body font-medium ${released ? "text-emerald-700" : "text-warning"}`}>
            {released ? (ar ? "تم الاعتماد ✓" : "Fully Released ✓") : (ar ? "مسار الاعتماد" : "Release Flow")}
          </span>
        </div>
        <span className={`text-micro px-2.5 py-1 rounded-full font-medium ${statusLabel.color}`}>
          ...
          return (
            <div key={step.key} className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className={`text-caption font-medium ${met ? "text-emerald-700" : "text-foreground"}`}>
                    {ar ? step.ar : step.en}
                  </span>
                  {roleDef && <span className={`text-micro px-1.5 py-0.5 rounded ${roleDef.color}`}>{ar ? roleDef.ar : roleDef.en}</span>}
                  {!step.required && <span className="text-micro text-muted-foreground">{ar ? "(اختياري)" : "(optional)"}</span>}
                </div>
                {met && historyEntry && (
                ...
                <div className="flex items-center gap-1.5">
                  {noteFor === step.key ? (
                    <div className="flex items-center gap-1">
                      <input value={note} onChange={e => setNote(e.target.value)} placeholder={ar ? "ملاحظة..." : "Note..."} className="h-7 px-2 text-micro rounded border border-border/60 w-24" />
                      <button onClick={() => { onApprove(step, note); setNoteFor(null); setNote(""); }} className="h-7 px-2 rounded bg-emerald-600 text-white text-micro">{ar ? "اعتمد" : "OK"}</button>
                      <button onClick={() => setNoteFor(null)} className="h-7 px-1.5 rounded hover:bg-muted"><X size={10} /></button>
                    </div>
                  ) : (
                    <>
                      <button onClick={() => setNoteFor(step.key)} className="h-7 px-2.5 rounded-lg bg-emerald-600 text-white text-micro font-medium hover:bg-emerald-700">
                        {ar ? "اعتمد" : "Approve"}
                      </button>
                      {onReject && (
                        <button onClick={() => onReject(step)} className="h-7 px-2.5 rounded-lg border border-rose-200 text-rose-600 text-micro font-medium hover:bg-rose-50">
                          {ar ? "ارفض" : "Reject"}
                        </button>
                      )}
                    </>
      ...
      <div className="px-5 py-2.5 bg-muted/20 border-t border-border/30 flex items-center justify-between">
        ...
        {!released && (
          <span className="text-micro text-warning font-medium">
