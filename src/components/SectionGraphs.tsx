...
export function MiniBarChart({ data, title, titleAr, height = 120 }: { data: BarItem[]; title: string; titleAr?: string; height?: number }) {
  ...
  return (
    <div className="bg-background border border-border/40 rounded-xl p-4">
      <p className="text-micro font-medium text-muted-foreground mb-3">{lang === "ar" && titleAr ? titleAr : title}</p>
      <div className="flex items-end gap-1.5" style={{ height }}>
        {data.map((d, i) => (
          <div key={i} className="flex-1 flex flex-col items-center gap-1">
            <span className="text-micro font-medium text-foreground/70">{d.value}</span>
            <div
            ...
            />
            <span className="text-micro text-muted-foreground/60 truncate w-full text-center">{d.label}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <div className="text-center -mt-[calc(50%+12px)] mb-4">
        <p className="text-title font-bold" style={{ color: clr }}>{Math.round(pct * 100)}%</p>
      </div>
      <p className="text-micro text-muted-foreground text-center">{lang === "ar" && labelAr ? labelAr : label}</p>
    </div>
  );
}
  ...
  return (
    <div className="bg-background border border-border/40 rounded-xl p-4">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-heading font-bold" style={{ color: color || "inherit", fontFamily: "var(--app-font-serif)" }}>{value}</p>
          <p className="text-micro text-muted-foreground mt-1">{lang === "ar" && labelAr ? labelAr : label}</p>
        </div>
        {Icon && (
        ...
        {steps.map((s, i) => (
          <div key={i} className="flex items-start gap-3">
            ...
            <div className="-mt-0.5">
              <p className={`text-micro font-medium ${s.status === "pending" ? "text-muted-foreground/50" : "text-foreground"}`}>{s.label}</p>
              {s.time && <p className="text-micro text-muted-foreground/40">{s.time}</p>}
            </div>
          </div>
        ))}
      </div>
