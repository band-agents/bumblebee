...
export function Topbar({ onMenuClick }: TopbarProps) {
  ...
  return (
    <header className="h-16 shrink-0 flex items-center px-4 gap-3 bb-glass border-b sticky top-0 z-30">
      ...
      <div className="flex items-center gap-2.5 min-w-0 flex-1">
        ...
        <HistoryNav ar={ar} />
        <SmartBreadcrumb ar={ar} />
        <PinsMenu ar={ar} />
      </div>

      {/* ── Center: Search ── */}
      <button onClick={openBar}
        className="hidden md:flex items-center gap-2 flex-1 max-w-[360px] h-8 ps-3 pe-3 rounded-xl border border-border/70 bg-card/80 text-body text-muted-foreground/70 hover:bg-card hover:border-border/90 hover:text-muted-foreground transition-all duration-150 cursor-text select-none">
        <Search size={13} strokeWidth={1.75} className="shrink-0" />
        <span className="flex-1 text-start truncate">{ar ? "ابحث في بامبلبي..." : "Search Bumblebee…"}</span>
        <kbd className="shrink-0 hidden lg:inline-flex items-center text-micro text-muted-foreground/40 border border-border/50 rounded px-1.5 py-0.5 font-sans">⌘K</kbd>
      </button>
      ...
      <div className="flex items-center gap-1 text-muted-foreground">
        ...
        <div className="relative">
          <button aria-label="Notifications" onClick={() => { setNotifOpen(v => !v); setProfileOpen(false); }}
            ...
            {unreadCount > 0 && (
              <span className="absolute top-1 end-1 min-w-[14px] h-[14px] rounded-full bg-primary ring-1 ring-background flex items-center justify-center text-micro text-primary-foreground font-bold">
        ...
        <button onClick={() => setLang(lang === "en" ? "ar" : "en")}
          className="h-7 px-2.5 rounded-md shrink-0 text-micro font-medium tracking-wide text-muted-foreground border border-border/60 bg-card/60 hover:text-foreground hover:bg-card hover:border-border transition-all select-none">
          {lang === "en" ? "AR" : "EN"}
        </button>
        ...
        {isAuthenticated ? (
          <div className="relative" ref={profileRef}>
            <button onClick={() => { setProfileOpen(v => !v); setNotifOpen(false); }}
              ...
              <div className="w-7 h-7 rounded-full bg-primary/10 flex items-center justify-center text-brand-ink text-micro font-bold shrink-0">
                {userInitials}
              </div>
              <div className="hidden md:flex flex-col items-start min-w-0">
                <span className="text-micro font-medium text-foreground truncate max-w-[100px] leading-tight">{userName}</span>
                <span className="text-micro text-muted-foreground truncate max-w-[100px] leading-tight">{userEmail}</span>
              </div>
                ...
                >
                  ...
                  <div className="px-4 py-3.5 border-b border-border/40 bg-muted/20">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-brand-ink text-body font-bold shrink-0">
                        {userInitials}
                      </div>
                      <div className="min-w-0">
                        <p className="text-body font-semibold text-foreground truncate">{userName}</p>
                        <p className="text-micro text-muted-foreground truncate">{userEmail}</p>
                        <div className="flex items-center gap-1.5 mt-1">
                          <span className="text-micro px-1.5 py-0.5 rounded-full bg-primary/10 text-brand-ink font-medium capitalize">
                            {workspace?.role || "owner"}
                          </span>
                          {workspace?.plan && (
                            <span className="text-micro px-1.5 py-0.5 rounded-full bg-warning/15 text-warning font-medium capitalize">
                              {workspace.plan}
                            </span>
                          )}
                        </div>
                  ...
                  <div className="px-2 py-2 border-b border-border/30">
                    <p className="px-2 py-1 text-micro text-muted-foreground/50 uppercase tracking-wider font-medium">{ar ? "سريع" : "Quick"}</p>
                    <Link href="/settings" onClick={() => setProfileOpen(false)}
                      className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-caption text-foreground hover:bg-muted/50 transition-colors">
                  ...
                  {/* Workspace */}
                  <div className="px-2 py-2 border-b border-border/30">
                    <p className="px-2 py-1 text-micro text-muted-foreground/50 uppercase tracking-wider font-medium">{ar ? "المساحة" : "Workspace"}</p>
                    <div className="px-2.5 py-2 flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-lg bg-primary/10 flex items-center justify-center text-brand-ink text-micro font-bold shrink-0">
                        {workspace?.name?.charAt(0) || "T"}
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="text-micro font-medium text-foreground truncate">{workspace?.name || "Bumblebee"}</p>
                        <p className="text-micro text-muted-foreground">{workspace?.plan || "pro"} plan</p>
                      </div>
                  ...
                  {/* Preferences */}
                  <div className="px-2 py-2 border-b border-border/30">
                    <p className="px-2 py-1 text-micro text-muted-foreground/50 uppercase tracking-wider font-medium">{ar ? "التفضيلات" : "Preferences"}</p>
                    <button onClick={() => { setLang(lang === "en" ? "ar" : "en"); }}
                      className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-caption text-foreground hover:bg-muted/50 transition-colors">
                      <Languages size={13} className="text-muted-foreground" />
                      <span className="flex-1 text-start">{ar ? "اللغة" : "Language"}</span>
                      <span className="text-micro text-muted-foreground font-medium">{lang === "en" ? "English" : "العربية"}</span>
                    </button>
                    <button onClick={() => openBar()}
                      className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-caption text-foreground hover:bg-muted/50 transition-colors">
                      ...
                      <kbd className="text-micro text-muted-foreground/50 border border-border/40 rounded px-1 py-0.5">⌘K</kbd>
                    </button>
                  </div>

                  {/* Help & Support */}
                  <div className="px-2 py-2 border-b border-border/30">
                    <a href="https://bumblebee.app/help" target="_blank" rel="noopener"
                      className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-caption text-foreground hover:bg-muted/50 transition-colors">
                    ...
                    <a href="https://bumblebee.app/docs" target="_blank" rel="noopener"
                      className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-caption text-foreground hover:bg-muted/50 transition-colors">
                      <BookOpen size={13} className="text-muted-foreground" />
                      <span className="flex-1 text-start">{ar ? "التوثيق" : "Documentation"}</span>
                      <ExternalLink size={10} className="text-muted-foreground/40" />
                    </a>
                    <a href="https://bumblebee.app/changelog" target="_blank" rel="noopener"
                      className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-caption text-foreground hover:bg-muted/50 transition-colors">
