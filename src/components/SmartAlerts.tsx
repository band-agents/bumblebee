  ...
  return (
    <div className={`flex items-start gap-3 px-4 py-3 rounded-xl border ${style.bg} ${style.text}`}>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-caption font-medium">{alert.message}</p>
        {alert.detail && <p className="text-micro opacity-80 mt-0.5">{alert.detail}</p>}
        {alert.action && (
          <button onClick={alert.action.onClick} className="text-micro font-medium underline mt-1 hover:opacity-70">{alert.action.label}</button>
        )}
      </div>
      {onDismiss && (
...
export function WorkflowBlockerCard({ blockers, ar, title }: {
  ...
  return (
    <div className={`rounded-xl border p-4 ${allMet ? "bg-emerald-50 border-emerald-200" : "bg-warning/10 border-warning/30"}`}>
      <div className="flex items-center gap-2 mb-3">
        {allMet ? <CheckCircle2 size={14} className="text-emerald-600" /> : <AlertTriangle size={14} className="text-warning" />}
        <span className={`text-body font-medium ${allMet ? "text-emerald-700" : "text-warning"}`}>
          {title || (allMet ? (ar ? "جاهز للتشغيل ✓" : "Ready for Production ✓") : (ar ? "مش جاهز للتشغيل" : "Not Ready for Production"))}
        </span>
        <span className="text-micro ml-auto tabular-nums opacity-60">{blockers.filter(b => b.met).length}/{blockers.length}</span>
      </div>
      <div className="space-y-1.5">
        {blockers.map(b => (
          <div key={b.key} className={`flex items-center gap-2 text-micro ${b.met ? "text-emerald-600" : "text-warning"}`}>
      ...
      {!allMet && unmet.length > 0 && (
        <div className="mt-3 pt-2 border-t border-warning/30">
          <p className="text-micro text-warning font-medium">
