        ...
        {stages.map((s, i) => {
          ...
          return (
            <div key={s.id} className="flex items-center gap-1">
              ...
              <div className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all ${isCurrent ? "ring-2 ring-brand-ink ring-offset-1 " + dept.bg : isPast ? "bg-emerald-100" : dept.bg}`}
                title={ar ? (s.name_ar || s.name) : s.name}>
                {isPast ? <CheckCircle2 size={12} className="text-emerald-600" /> : <Icon size={12} className={isCurrent ? "text-brand-ink" : dept.color} />}
              </div>
            </div>
          );
        })}
        ...
        return (
          <div key={s.id} className="flex gap-3">
            ...
            <div className="flex flex-col items-center">
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 transition-all ${isCurrent ? "ring-2 ring-brand-ink ring-offset-1 " + dept.bg : isPast ? "bg-emerald-100" : dept.bg}`}>
                {isPast ? <CheckCircle2 size={15} className="text-emerald-600" /> : <Icon size={15} className={isCurrent ? "text-brand-ink" : dept.color} />}
              </div>
              {!isLast && (
            ...
            <div className={`flex-1 min-w-0 ${isLast ? "pb-2" : "pb-3"}`}>
              <div className="flex items-center gap-2 mb-0.5">
                <span className="text-body font-medium">{ar ? (s.name_ar || s.name) : s.name}</span>
                {isCurrent && <span className="text-micro px-1.5 py-0.5 rounded bg-primary/10 text-brand-ink font-medium">{ar ? "حالي" : "Current"}</span>}
                {isPast && <span className="text-micro px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700 font-medium">{ar ? "تم" : "Done"}</span>}
              </div>
              <div className="flex items-center gap-3 text-micro text-muted-foreground flex-wrap">
                ...
                {s.quality_checkpoint && <span className="text-micro px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-700 font-medium">{ar ? "فحص جودة" : "QC"}</span>}
                {s.approval_required && <span className="text-micro px-1.5 py-0.5 rounded bg-warning/10 text-warning font-medium">{ar ? "موافقة" : "Approval"}</span>}
                {s.blocks_next && <span className="text-micro px-1.5 py-0.5 rounded bg-rose-50 text-rose-700 font-medium">{ar ? "حاجزة" : "Blocks"}</span>}
              </div>
              {s.checklist.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-1.5">
                  {s.checklist.slice(0, 4).map((c, ci) => (
                    <span key={ci} className="text-micro px-1.5 py-0.5 rounded bg-muted/50 text-muted-foreground">{c}</span>
                  ))}
                  {s.checklist.length > 4 && <span className="text-micro text-muted-foreground/50">+{s.checklist.length - 4}</span>}
                </div>
              )}
            </div>
          </div>
