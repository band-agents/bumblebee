    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-[440px] bg-background border-l border-border shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="shrink-0 px-5 py-4 border-b border-border/40">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center">
              <Package size={18} className="text-brand-ink" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-body-lg font-semibold text-foreground truncate">{product.product_name}</p>
        ...
        <div className="shrink-0 px-5 border-b border-border/40 flex gap-1">
          {tabs.map((tab) => (
            <button
              ...
              className={`px-3 py-2.5 text-caption font-medium border-b-2 transition-colors ${
                activeTab === tab.id
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-xl bg-muted/30">
                  <p className="text-micro text-muted-foreground uppercase tracking-wider mb-1">{lang === "ar" ? "سعر البيع" : "Retail Price"}</p>
                  <p className="text-title font-bold text-foreground" style={{ fontFamily: "var(--app-font-serif)" }}>{formatEGP(product.unit_price)}</p>
                </div>
                <div className="p-3 rounded-xl bg-muted/30">
                  <p className="text-micro text-muted-foreground uppercase tracking-wider mb-1">{lang === "ar" ? "تكلفة الشراء" : "Cost Price"}</p>
                  <p className="text-title font-bold text-foreground" style={{ fontFamily: "var(--app-font-serif)" }}>{formatEGP(product.unit_cost)}</p>
                </div>
              </div>
              ...
              <div className="p-3 rounded-xl border border-border/40">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-micro text-muted-foreground">{lang === "ar" ? "هامش الربح" : "Profit Margin"}</span>
                  <span className={`text-caption font-semibold ${margin > 50 ? "text-emerald-600" : margin > 30 ? "text-warning" : "text-rose-600"}`}>
                    {margin.toFixed(1)}%
                  </span>
                </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="p-3 rounded-xl bg-muted/30 text-center">
                  <p className="text-body-lg font-bold text-foreground">{totalStock}</p>
                  <p className="text-micro text-muted-foreground mt-0.5">{lang === "ar" ? "إجمالي" : "Total Stock"}</p>
                </div>
                <div className="p-3 rounded-xl bg-muted/30 text-center">
                  <p className="text-body-lg font-bold text-foreground">{totalReserved}</p>
                  <p className="text-micro text-muted-foreground mt-0.5">{lang === "ar" ? "محجوز" : "Reserved"}</p>
                </div>
                <div className="p-3 rounded-xl bg-muted/30 text-center">
                  <p className="text-body-lg font-bold text-foreground">{totalStock - totalReserved}</p>
                  <p className="text-micro text-muted-foreground mt-0.5">{lang === "ar" ? "متاح" : "Available"}</p>
                </div>
              </div>

              {/* Reorder Alert */}
              {sameProduct.some((i) => i.quantity <= i.reorder_level) && (
                <div className="flex items-center gap-2 p-3 rounded-xl bg-warning/10 border border-warning/30 text-warning">
                  <AlertTriangle size={14} />
                  <span className="text-micro">{lang === "ar" ? "بعض الفروع بحاجة لإعادة تعبئة" : "Some branches need restocking"}</span>
                </div>
              )}
            </>
          )}
          ...
          {activeTab === "stock" && (
            <>
              <p className="text-caption text-muted-foreground">{lang === "ar" ? "المخزون حسب الفرع" : "Stock by Branch"}</p>
              {sameProduct.map((inv) => {
                  ...
                  <div key={inv.id} className="p-3 rounded-xl border border-border/40">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-caption font-medium text-foreground">{inv.branch_id}</span>
                      </div>
                      {isLow && <span className="text-micro px-1.5 py-0.5 rounded-full bg-warning/15 text-warning">Low</span>}
                    </div>
                    <div className="grid grid-cols-4 gap-2 text-center">
                      <div>
                        <p className="text-caption font-semibold">{inv.quantity}</p>
                        <p className="text-micro text-muted-foreground">{lang === "ar" ? "إجمالي" : "Total"}</p>
                      </div>
                      <div>
                        <p className="text-caption font-semibold">{inv.reserved_quantity}</p>
                        <p className="text-micro text-muted-foreground">{lang === "ar" ? "محجوز" : "Held"}</p>
                      </div>
                      <div>
                        <p className="text-caption font-semibold text-emerald-600">{available}</p>
                        <p className="text-micro text-muted-foreground">{lang === "ar" ? "متاح" : "Free"}</p>
                      </div>
                      <div>
                        <p className="text-caption font-semibold">{inv.reorder_level}</p>
                        <p className="text-micro text-muted-foreground">{lang === "ar" ? "حد إعادة الطلب" : "Reorder"}</p>
                      </div>
                    </div>
                    <div className="mt-2 h-1.5 rounded-full bg-muted/60 overflow-hidden">
                      <div
                        className={`h-full rounded-full ${isLow ? "bg-warning" : "bg-emerald-500"}`}
          ...
          {activeTab === "sales" && (
            <>
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-xl bg-muted/30">
                  <p className="text-micro text-muted-foreground uppercase tracking-wider mb-1">{lang === "ar" ? "قيمة المخزون (التكلفة)" : "Inventory Value (Cost)"}</p>
                  <p className="text-body-lg font-bold text-foreground">{formatEGP(totalValue)}</p>
                </div>
                <div className="p-3 rounded-xl bg-muted/30">
                  <p className="text-micro text-muted-foreground uppercase tracking-wider mb-1">{lang === "ar" ? "قيمة المخزون ( البيع)" : "Inventory Value (Retail)"}</p>
                  <p className="text-body-lg font-bold text-foreground">{formatEGP(totalRetail)}</p>
                </div>
              </div>

              <div className="p-3 rounded-xl border border-border/40">
                <p className="text-micro text-muted-foreground mb-2">{lang === "ar" ? "الأداء" : "Performance"}</p>
              ...
              <div className="p-3 rounded-xl bg-primary/5 border border-primary/10">
                <p className="text-micro text-brand-ink font-medium mb-1">{lang === "ar" ? "اقتراح ذكي" : "Smart Suggestion"}</p>
