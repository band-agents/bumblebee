...
do $$ begin if to_regclass('access_role_templates') is not null then execute 'drop policy if exists "access_role_templates_read" on access_role_templates'; end if; end $$;
...
$$;
do $$ begin if to_regclass('invoices') is not null then execute 'drop trigger if exists guard_finance_write on invoices'; end if; end $$;
create trigger guard_finance_write before insert on invoices for each row execute function guard_finance_write();
do $$ begin if to_regclass('payments') is not null then execute 'drop trigger if exists guard_finance_write on payments'; end if; end $$;
